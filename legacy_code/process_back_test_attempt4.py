...
##
        sdde=1
        stddiff=np.std(diffmp2)
        if mp[-5]-mp[-1]>3*stddiff or mp[-2]-mp[-1]>3*stddiff or mp[-3]-mp[-1]>3*stddiff or mp[-4]-mp[-1]>3*stddiff or mp[-6]-mp[-1]>3*stddiff or mp[-7]-mp[-1]>3*stddiff or mp[-8]-mp[-1]>3*stddiff or mp[-9]-mp[-1]>3*stddiff :#or mp[i-4]-mp[i]>3*stddiff :
            sdde=0
...
##
        ...
        #    if calc_down(mp_list[current_price_index-20:current_price_index+1])<1 and calc_up(mp_list[current_price_index-20:current_price_index+1])>0.5:#0.7<p_c/n_c<1.3 :#and ks_p_value>0.5:#or (np.mean(diffmp[-5:])>0 and stab_de!=1):#self.pimf1[-1][1]>0.4:#self.cu[-1]>0.6 or (len(self.cu)>1 and self.cu[-1]>self.cu[-2]):
            if  p[0]>0 :#and not (sharp(s21)<0 and sharp(s22)<0):#len(ma3s1)>1 and ma3s1[-1]>ma3s1[-2]:#np.mean(s21)/np.std(s21)>np.mean(s22)/np.std(s22) and np.mean(s21)/np.std(s21)>0 and np.mean(s21)/np.std(s21)>np.mean(s11)/np.std(s11):#sum(s1)>sum(s2) and sum(s1)>0:#deltama10<0 :#and  s2[-1]>0:#self.mpdown[-1]<mampdown[-1] :#and lp2!=1:
                sel=1
            #elif p_c/n_c>1.3:
            #elif calc_up(mp_list[current_price_index-20:current_price_index+1])>1 :
            else:# not (sharp(s21)<0 and sharp(s22)<0):# len(ma3s2)>1 and ma3s2[-1]>ma3s2[-2]:#if np.mean(s21)/np.std(s21)<np.mean(s22)/np.std(s22) and np.mean(s22)/np.std(s22)>0 and np.mean(s22)/np.std(s22)>np.mean(s12)/np.std(s12): #sum(s1)<sum(s2) and sum(s2)>0:
                sel=2
...
##
        ...
        ma5=ma_func2(self.close_price,5) 
        m,s=montecarlo_simulate2(self.close_price[current_price_index-24:current_price_index-2],500,3)
        self.mclist.append(p)
        self.mclist3.append(m)
        if len(self.mclist3)>0:
            self.mclist2.append(self.close_price[current_price_index]-(m-2*s))
        #self.mclist3.append(m)
        self.mclist4.append(s)

##
        speed_de=1
        if self.close_price[current_price_index]-self.close_price[current_price_index-1]<mp_list[current_price_index]-mp_list[current_price_index-1]:
            speed_de=0
       # self.mde=1 
        mde=1
        if (self.fail_flag==0 and self.close_price[current_price_index]-(m-2*s)<0) or (self.fail_flag==1 and self.close_price[current_price_index]-(m-s)<0):
            mde=0
        if 0:#self.close_price[current_price_index]>self.close_price[current_price_index-1]:
            mde=1
##
        ...
        print "cp mclist3 monte {}".format(self.mclist3[-10:])
        ...
        mpm,mps=montecarlo_simulate2(mp_list[current_price_index-35:current_price_index-4],100,3)
        #p,m,s=montecarlo_simulate(ma5[current_price_index-20:current_price_index+1],10)
        self.mclist6.append(mpm)
        if len(self.mclist6)>0:
            self.mclist5.append(self.mclist6[-1]-mp_list[current_price_index])
        #self.mclist4.append(jm/vm)
        #self.mclist6.append(mpm)
        print "monte carlo mclist6 mp {}".format(self.mclist6[-5:])
        print " mp {}".format(mp_list[current_price_index-10:current_price_index+1])
...
##
        ...
        def calc_recentdown_pro(datac,dataj,datav,n):
            mp = [dataj[i]/datav[i] for i in range(len(datav))]
            c=0
            c1=0
            ...
            if c!=0:
                ...
                print "recent down pro{}".format(0)
                return 0
        rdde=0
        p=20
        datac = self.close_price[current_price_index-p:current_price_index+1]
        ...
        d1=calc_recentdown_pro(datac,dataj,datav,1)
        d2=calc_recentdown_pro(datac,dataj,datav,2)
        d3=calc_recentdown_pro(datac,dataj,datav,3)
        d4=calc_recentdown_pro(datac,dataj,datav,4)
        d5=calc_recentdown_pro(datac,dataj,datav,5)
        print "down pro {} {} {} {} {}".format(d1,d2,d3,d4,d5)
        if d1<0.5 or d3<0.5 :#or  d5>0.5:#or calc_recentdown_pro(datac,dataj,datav,4)<0.5 or calc_recentdown_pro(datac,dataj,datav,5)<0.5:
            rdde=1

        #rdde=0
        #if ml_predict.predict([d1,d2,d3,d4,d5]):
        #    rdde=1
        print "buy day list {}".format(self.buy_day_list)
        print "buy day list {}".format(sum(self.buy_day_list))

###
        syl = [self.close_price[i]/self.close_price[i-1] for i in range(1,len(self.close_price))]
        syl.insert(0,1)
        rsyl = syl[current_price_index-9:current_price_index+1]
        print "rsyl mean {}".format(np.mean(rsyl))
        print "rsyl stable test {}".format(ts.adfuller(rsyl,1))
        stab=ts.adfuller(rsyl,1)
        kk=stab[0]
        kkk=stab[4]['5%']
        rsylstabde=0
        sel=2
        if kk<kkk or (kk>kkk and np.mean(rsyl)>1):
            sel=1
        rsyl = syl[current_price_index-105:current_price_index+1]
        ref = np.array(rsyl[-100:-10])
        sample = np.array(rsyl[-10:])
        ks_d,ks_p_value = stats.ks_2samp(ref,sample)
        print "rsyl ks {}".format(ks_p_value)
###
        rmp=mp_list[current_price_index-10:current_price_index+1]
        rc=self.close_price[current_price_index-10:current_price_index+1]
        v_mp = np.diff(rmp)
        v_c = np.diff(rc)
        jede=0
        #sel = 1
        if sel==1:
            if  diffmp[-1]<0 and  mean_de==1 and mde==1 and self.mclist2[-1]>0:#and (( self.fail_flag==0) or (self.fail_flag==1 and speed_de==1)):#speed_de==1:#and len(self.mclist2)>0 and self.mclist5[-1]<0 :#and self.mclist5[-1]<0 :#and self.mclist3[-1]>self.close_price[current_price_index]:#and (( self.fail_flag==0) or (self.fail_flag==1 and rdde==1)):
                jede=1
            if (diffmp[-1]>0 and  mean_de==2 ) :#or (mp_list[current_price_index]<mp_list[current_price_index-1]-np.std(mp_list[current_price_index-10:current_price_index+1])):
                jede=2
        elif 0:#sel==2:
            ...
            if self.close_price[current_price_index]>mp :#and mean_de==1 and self.vol[current_price_index]>np.mean(self.vol[current_price_index-4:current_price_index+1]):#>np.mean(self.vol[current_price_index-9:current_price_index+1]):#and len(self.mclist4)>2 and (self.mclist4[-2]-mp)>(self.mclist4[-3]-mp1):#and self.:#len(self.mclist2)>0 and self.mclist2[-1]>0 :#and  self.mclist5[-1]>0:#self.mclist5[-1]>0:#and self.close_price[current_price_index-1]<mp1:
            ...
            if self.close_price[current_price_index]<mp :#and  mean_de==2:
                jede=2
     #   elif sel==2:
...
###
        ...
        if  self.buy_price!=0 and  (jede==2):#(self.close_price[current_price_index]>self.buy_price or self.buy_day>=3):#((jede==2 )):# or (self.mclist4[-2]>mp)):#and mean_de==2:#and imf_flag==2):#and sha_de==2:#and self.close_price[current_price_index-1]>self.close_price[current_price_index]:#emd_boll_de==2:#ma_flag==2:#bayes_de==2:#emd_data[-1]<emd_data[-2]:#(power_de_ma_list1==2) and imf_flag==2:# or boll_de==2):#and self.close_price[current_price_index-1]>self.close_price[current_price_index]:#  self.close_price[current_price_index-1]>self.close_price[current_price_index] :#power_de_ma_list==2:#emd_data2[-1]>emd_data2[-2] and  emd_data[-1]<emd_data[-2]:#me[-1]<me[-2] and me[-2]>me[-3]:#boll_de==2 and self.boll_de[-2]!=2:#  me[-1]<me[-2] and me[-2]>me[-3]:#power_de_ma_list==2:#((judge_extrem(emd_data)<=0 and judge_extrem(emd_data[:-1])>0) or (judge_extrem(emd_data)<0)):#self.de2==1 and emd_data[-1]<emd_data[-2]:#((judge_extrem(emd_data[:-1])>0 and judge_extrem(emd_data)<=0) ):#imf_flag==2:# (  (len(self.boll_de)>1 and self.boll_de[-2]==2 and self.boll_de[-1]!=2) or (len(self.boll_de)>1 and self.boll_de[-2]!=1 and self.boll_de[-1]==1)):#( self.pre_cha_de[-1]==1 or imf_flag==2) :#and (boll_de==2 or self.buy_day>10):#(boll_de==2 or (deltama5<0 and deltama5_before<0)):#emd_data[-1]<emd_data[-2]<emd_data[-3]  and emd_data[-3]>emd_data[-4] :# (r (  emd_data[-3]<emd_data[-2] and emd_data[-1]<emd_data[-2] and emd_data[-1]>0)):#imf_flag==2:#boll_de==2:# self.close_flag[-2]==2 and imf_flag!=2:# and self.boll_de[-1]!=2 and len(self.boll_de)>1 and self.boll_de[-2]==2:#boll_de==2:#((boll_de!=2 and self.boll_de[-2]==2) or ((imf_flag==2) and deltama3_before<0 and deltama3<0))  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
            ...
            if 1:#self.fail_flag==0:
                ...
                self.buy_day_list.append(self.buy_day)
                self.each_day=0
                self.buy_price = 0
                self.buy_mean_price = 0
                self.one_time_day=0
            ...
            if 0:#self.fail_flag!=0:
                ...
                self.down_pro.append([d1,d2,d3,d4,d5])
...
#            self.last_as=self.total_asset
        ...
        print "down pro %s"%self.down_pro
...
#        data11 = self.je
        ...
        plt.plot([i for i in range(len(data2))],data2,'o',[i for i in range(len(data2))],data2,'b')#,big0index2,big0value2,'ro',small0index2,small0value2,'go')#,self.buy_x_index,data2_buy_value,'yo',self.sell_x_index,data2_sell_value,'ko')             
        plt.subplot(514).axis([start,len(data3),min(data3[start:]),max(data3[start:])])
        #plt.subplot(514)#.axis([0,len(ma3),8000,12000])
        plt.plot([i for i in range(len(data3))],data3,'o',[i for i in range(len(data3))],data3,'b')#,[i for i in range(len(data2))],data2,'r')#,data3_x_index,data3_y_index,'yo',data3_x_index2,data3_y_index2,'ko')
