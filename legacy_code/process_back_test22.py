...
from leastsqt import leastsqt_predict_2
from leastsqt import leastsqt_predict_3
from leastsqt import leastsqt_predict_4
from svm_uti import *
...
class process_util():
    ...
    def run_predict1(self,imf_list,current_price_index,date,datafile,residual,imf_open,imf_macd,sel_flag,emd_data,emd_data2):
        ...
        r_c=self.close_price[current_price_index-499:current_price_index+1]
        max_index = list(argrelextrema(np.array(r_c),np.greater)[0])
        min_index = list(argrelextrema(np.array(r_c),np.less)[0])
        tmp=max_index+min_index
        tmp.sort()
        trust0=tmp[-1]
        print "rc%s"%r_c[-10:]
        print "trust0%s"%trust0
...
####
        ...
        #imf0_min_index=filter(lambda n:n<499,imf0_min_index)
        #imf0_max_index=filter(lambda n:n<499,imf0_max_index)
        imf0_or_flag=0
        if imf0_min_index[-1]>imf0_max_index[-1] and imf0_min_index[-1]>498 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf0_or_flag=1
        if imf0_min_index[-1]<imf0_max_index[-1] and imf0_max_index[-1]>498 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf0_or_flag=2 
        ...
        imf1_or_flag=0
        if imf1_min_index[-1]>imf1_max_index[-1] and imf1_min_index[-1]>497 and imf1_max_index[-1]<495:#-imf2_max_index[-1]>3:
            imf1_or_flag=1
        if imf1_min_index[-1]<imf1_max_index[-1] and imf1_max_index[-1]>496 :#and imf1_min_index[-1]<495:#-imf2_max_index[-1]>3:
            imf1_or_flag=2
...
###
        ...
        imf2_or_flag=0
        if imf2_min_index[-1]>imf2_max_index[-1] and imf2_min_index[-1]>497 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf2_or_flag=1
...
####
        ...
        #        imf_max_index.pop()
        if imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>(datalength-4)  :#and imf_min_index[-1]-imf_max_index[-1]>10:#and ex_flag==1:#and imf_process[-1]<0 :
            imf2_flag=1
        if imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>=(datalength-4):
            imf2_flag=2
...
#            i=1
        ...
        imf1_flag=0
        if (imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>(datalength-4)) :#and imf1_or_flag!=2) or (imf1_or_flag==1) :#and ex_flag==1:#and imf_process[-1]<0 :
            imf1_flag=1
        if (imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>(datalength-4) ) :#or (imf1_or_flag==2):
            imf1_flag=2
...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if  imf2_or_flag==1 :#and imf2_or_flag!=2:#imf1_or_flag!=2 and imf2_or_flag!=2 :#and imf2_flag!=2 and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
#       #     if (imf2_flag==1 and self.open_flag==1) or (imf2_flag==1 and imf2_macd_flag==1) or (imf2_macd_flag==1 and self.close_flag==1) or (imf2_macd_flag==1 and imf2_flag==1):#and imf2_macd_flag==1:#and power_de==1:#and imf3_flag==1:
        ...
        #    #if (imf2_flag==2 or imf2_macd_flag==2) and  imf2_or_flag==2 :#and imf2_open_flag!=2:
            if imf2_or_flag==2 :#or imf1_flag==2:#and imf2_open_flag==2 :
...
#
        ...
        print " min index %s"%min_index
        com_de=0
        if max_index[-1]>min_index[-1] and max_index[-1]>496:
            com_de=2
#######
...
####
        ...
        for i in range(3,len(ma_list)):
            ma5_list.append(np.mean(close_price[i-2:i+1]))
        dddd=ma5_list[current_price_index-50:current_price_index+1]
        normal_max_index = list(argrelextrema(np.array(dddd),np.greater)[0])
        normal_min_index = list(argrelextrema(np.array(dddd),np.less)[0])
...
#####
        ...
        #boll_de1=0
        #maa=ma_list[current_price_index]+ma_list[current_price_index-1]+ma_list[current_price_index-2]+ma_list[current_price_index-3]+ma_list[current_price_index-4]
        #maa=emd_data[-1]+emd_data[-2]+emd_data[-3]+emd_data[-4]+emd_data[-5]
        #maa=maa/5.0
        #if emd_data[-1]<maa:
        #    boll_de1=1
        #if emd_data[-1]>maa:
...
######
        ...
        ma_decision=0
        if deltama10>0 :##and deltama30>0:
            ma_decision=1
...
######
        ...
        if emd_data[-1]<emd_data[-2]:
            self.mama.append(2)
        elif emd_data[-1]>emd_data[-2]:
            self.mama.append(1)
        else:
            self.mama.append(0)
        print "selfmama%s"%self.mama[-10:]
        ...
        if  self.buy_price!=0 and ( boll_de==2):#len(self.mama)>1 and self.mama[-1]==2 and self.mama[-2]==1:# emd_data[-1]<emd_data[-2] and emd_data[-2]>emd_data[-3]:#(imf_flag==2 or (len(self.boll_de)>1 and self.boll_de[-2]==2 and boll_de!=2)):# or (deltama5<0 and deltama5_before<0)):#>emd_data[-3] :#imf_flag==2 :#and (len(self.boll_de)>1 and self.boll_de[-2]==2 and boll_de!=2) :#and (boll_de==2 or self.buy_day>10):#(boll_de==2 or (deltama5<0 and deltama5_before<0)):#emd_data[-1]<emd_data[-2]<emd_data[-3]  and emd_data[-3]>emd_data[-4] :# (r (  emd_data[-3]<emd_data[-2] and emd_data[-1]<emd_data[-2] and emd_data[-1]>0)):#imf_flag==2:#boll_de==2:# self.close_flag[-2]==2 and imf_flag!=2:# and self.boll_de[-1]!=2 and len(self.boll_de)>1 and self.boll_de[-2]==2:#boll_de==2:#((boll_de!=2 and self.boll_de[-2]==2) or ((imf_flag==2) and deltama3_before<0 and deltama3<0))  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
            ...
            if self.fail_flag==0:
                ...
                print "sellsell 1"
                self.buy_price=0
        elif 0:#self.buy_price!=0 and ((((self.buy_price-current_price)/self.buy_price)>0.03 ) )  :#and self.share > 0 :
            print "sell sell 4"
            if 1:#self.fail_flag==0:
                if self.buy_price>=trade_price:
                    self.fail_flag+=flag_fail
        ...
        elif    self.buy_price==0 and  imf_flag==1 :#and deltama3_before<deltama3_before2 and boll_de==1:#and self.macd[current_price_index]>self.macd[current_price_index-1]:#len(self.mama)>1 and self.mama[-1]==1 and self.mama[-2]==2 and boll_de==0:#emd_data[-1]>emd_data[-2] and emd_data[-2]<emd_data[-3]:#imf_flag==1 and boll_de==1:#and deltama3<deltama3_before :#and (len(self.boll_de)>1 and self.boll_de[-2]==2 and boll_de!=2):#and emd_data[-1]>emd_data[-2]:#boll_de==1:#and deltama3>deltama3_before :#and deltama3>deltama3_before:#and pvpower_de==1 :#emd_data[-1]>emd_data[-2] and emd_data[-2]<emd_data[-3]:#self.close_price[current_price_index]>self.close_price[current_price_index-1]:#and (deltama3_before<deltama3 and deltama3_before<0 and deltama3>0) :#and ma_ex_de==1:#and distance_decision==1 and deltama3_before<deltama3 and deltama3_before<deltama3_before2 :#and deltama3_before2<deltama3_before3 :#and ma_ex_de==1:# and distance_decision==1 :# and self.close_price[current_price_index]>self.close_price[current_price_index-1]:#deltama3_before<deltama3:#self.macd[current_price_index-1]<self.macd[current_price_index] :#and (self.macd[current_price_index-2]<0 or self.macd[current_price_index-1]<0 or self.macd[current_price_index]<0):#and deltama5>=0 :#and deltama3_before<deltama3 ) or (deltama3>0 and deltama3_before<0)):# and  mama3>mama3_b and mama3_b<mama3_b2 and distance_decision==1:#and deltama5<0:#and self.close_price[current_price_index]>self.close_price[current_price_index-1] and deltama5_before<deltama5<0: #and deltama3_before<0 and deltama3>0 and deltama5_before<deltama5<0:#and ma_decision==1 :#and distance_decision==1:#deltama3_before<deltama3:# (deltama3_before>deltama3 or deltama3<0):
...
#
        ...
        ma_list=[ma10_list[i]-ma20_list[i] for i in range(len(close_price))]
...
def mamama(data,n):
    ...
    for ii in range(period):
        ma3.append(0)
    for ii in range(period,len(data)):
        ma3.append(np.mean(ma2[ii-(period-1):ii+1]))
...
#####
        ...
        if 0:#ma5<ma10<ma20<ma30:
           ...
           # emd_data.append(open_price[i])
            ...
            emd_data=(ma_list[i-500:i])
            emd_data=(ma2[i-500:i])
            emd_data=(ma2[i-500:i])
            #emd_data=(pm[i-500:i])


            pp11=leastsqt_predict(emd_data[-10:],1)[0]
            pp12=leastsqt_predict_2(emd_data[-10:],1)[0]
            pp13=leastsqt_predict_3(emd_data[-10:],1)[0]
            pp1=(pp11+pp12+pp13)/3.0

            train1=close_price[i-10:i]
            train2=close_price[i-10:i]
            train3=close_price[i-10:i]
            train1.append(open_price[i])
            train2.append(open_price[i])
            train3.append(open_price[i])
            pp21=leastsqt_predict(train1,2)[0]
            pp22=leastsqt_predict_2(train2,2)[0]
            pp23=leastsqt_predict_3(train3,2)[0]


            p11=leastsqt_predict(ma2[i-10:i],3)[0]
            p12=leastsqt_predict_2(ma2[i-10:i],3)[0]
            p13=leastsqt_predict_3(ma2[i-10:i],3)[0]
            p14=leastsqt_predict_4(ma2[i-10:i],3)[0]

            p21=leastsqt_predict(ma2[i-10:i],3)[1]
            p22=leastsqt_predict_2(ma2[i-10:i],3)[1]
            p23=leastsqt_predict_3(ma2[i-10:i],3)[1]
            p24=leastsqt_predict_4(ma2[i-10:i],3)[1]

            p31=leastsqt_predict(ma2[i-10:i],3)[2]
            p32=leastsqt_predict_2(ma2[i-10:i],3)[2]
            p33=leastsqt_predict_3(ma2[i-10:i],3)[2]
            p34=leastsqt_predict_4(ma2[i-10:i],3)[2]

            pre1 = (p11+p12+p13+p14)/4.0
            pre2 = (p21+p22+p23+p24)/4.0
            pre3 = (p31+p32+p33+p34)/4.0
            ...
            pre1 = (p11+p12+p13+p14)/4.0
            pre2 = (p21+p22+p23+p24)/4.0
            p_c=close_price[i-500:i]

            p_c.append(pre1)
            p_c.append(pre2)
            mamadata=mama(p_c,2)

            kk1=(k1+mamadata[-2])/2.0
            kk2=(k2+mamadata[-1])/2.0
            emd_data.append(kk1)
           # emd_data.append(kk2)
            ...
            zhi10=ma2[i-1]+ma2[i-2]+ma2[i-3]+ma2[i-4]+ma2[i-5]+ma2[i-6]+ma2[i-7]+ma2[i-8]+ma2[i-9]+pre1
            zhi5=ma2[i-1]+ma2[i-2]+ma2[i-3]+ma2[i-4]+pre1
            zhi3=ma2[i-1]+ma2[i-2]+pre1
            zhi2=ma2[i-1]+pre1
            pppp1=zhi3/3.0-zhi10/10.0
#            pppp1=zhi2/2.0-zhi5/5.0
           # emd_data.append(pppp1)

            zhi10=ma2[i-1]+ma2[i-2]+ma2[i-3]+ma2[i-4]+ma2[i-5]+ma2[i-6]+ma2[i-7]+ma2[i-8]+pre1+pre2
            zhi5=ma2[i-1]+ma2[i-2]+ma2[i-3]+pre1+pre2
            zhi3=ma2[i-1]+pre1+pre2
            zhi2=pre1+pre2
            pppp1=zhi3/3.0-zhi10/10.0
 #           pppp1=zhi2/2.0-zhi5/5.0
            #emd_data.append(pppp1)
           ...
           # emd_data1=[]
           # for i in range(3):
           #     emd_data1.append(emd_data[i])
           # for i in range(3,len(emd_data)):
           #     emd_data1.append(np.mean(emd_data[i-2:i+1]))
           # emd_data=emd_data1

            zhi10=ma2[i-1]+ma2[i-2]+ma2[i-3]+ma2[i-4]+ma2[i-5]+ma2[i-6]+ma2[i-7]+pre1+pre2+pre3
            zhi3=pre1+pre2+pre3
            pppp1=zhi3/3.0-zhi10/10.0
           # emd_data.append(pppp1)

            pp2=(pp21+pp22+pp23)/3.0
           # pp3=(pp211+pp222+pp233)/3.0
           # pp2=(pp21+pp22)/2.0
            print "pp%s"%pp1
            print "pp%s"%pp2
            print "real%s"%close_price[i]
            ...
            zhi3=close_price[i-1]+close_price[i-2]+pp2
            zhi=zhi3/3.0-zhi5/5.0
           # zhi=zhi3/3.0-zhi5/5.0
