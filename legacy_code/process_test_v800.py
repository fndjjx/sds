...
################
                    ...
                    if np.corrcoef(price_max_min_list,imf_max_min_list)[0][1]>0.1 and np.corrcoef(price_max_min_list2,price_min_max_list2)[0][1]<-0.2 and 499-imf_max_true_index[-1]>mean_max_min-1 and 499-imf_max_true_index[-1]<mean_max_min+std_max_min:
...
#####################################################################################################################
        ...
        if self.buy_price!=0 and imf_extrem_decision9==1:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
