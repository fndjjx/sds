...
##############################################################
     ...
     #       small_imf_max_true_index = small_imf_max_index
        ...
        if small_imf_min_true_index[-1]>small_imf_max_true_index[-1]:
            ...
            self.imf1_last = small_imf_min_true_index[-1]
        else:
            self.imf1_last_last = "max"
            self.imf1_last = small_imf_max_true_index[-1]
...
###########################
        ...
        price_flag=0
        if price_min_index[-1]>price_max_index[-1] or price_min_index[-1]>496:
            price_flag=1
...
###################
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            #if imf_max_true_index[-1]>imf_min_true_index[-1]:
            #    if imf_max_true_index[-1]-imf_min_true_index[-1]<3:
            #        imf_max_true_index.pop()
            #elif imf_max_true_index[-1]<imf_min_true_index[-1] :
            #    if imf_min_true_index[-1]-imf_max_true_index[-1]<3:
            #        imf_min_true_index.pop()
            value_imf_max_true=[]
            value_imf_min_true=[]
...
##############
                    ...
                    last_min_index = current_price_index-499+imf_min_true_index[-1]
                    last_last_min_index = current_price_index-499+imf_min_true_index[-2]
                    last_max_index = current_price_index-499+imf_max_true_index[-1]
                    price_min_max = self.close_price[last_last_min_index:last_max_index+1]
                    print price_min_max
                    line_1 = splinerestruct(price_min_max,1)
                    print line_1
                    line_1_arc = ((line_1[-1]-line_1[0])/(len(line_1)-1))
                    price_max_min = self.close_price[last_max_index:last_min_index+1]
                    print price_max_min
                    line_2 = splinerestruct(price_max_min,1)
                    print line_2
                    line_2_arc = ((line_2[-1]-line_2[0])/(len(line_2)-1))

                    price_last = self.close_price[last_min_index:current_price_index+1]
                    line_3 = splinerestruct(price_last,1)
                    print line_3
                    line_3_arc = ((line_3[-1]-line_3[0])/(len(line_3)-1))
                    print "line 1 arc%s"%line_1_arc
                    print "line 2 arc%s"%line_2_arc
                    print "line 3 arc%s"%line_3_arc
                    ...
                    if (imf_min_true_index[-1]>=495 and (self.last_last=="max")) :#or  (self.last_last=="max" and imf1_flag==1) :#and line_1_arc>line_2_arc:#and abs(((self.core+self.core2)/2.0)-current_core)>3 :#and abs(self.core-self.core3)<=2 and abs(self.core-self.core2)<=2:#est_per%2==0 and (yu<0.3 or yu>0.7):
                        down_test_decision1=-1
...
#                    if line_arc>line_arc_before>line_arc_before2 and line_arc_before2<line_arc_before3:
			...
			b2data =  self.close_price[current_core:current_price_index-1]
                    ...
                    last_max_index = current_price_index-499+imf_max_true_index[-1]
                    last_min_index = current_price_index-499+imf_min_true_index[-1]
                    price_min_max = self.close_price[last_min_index:last_max_index+1]
                    print price_min_max
                    line_1 = splinerestruct(price_min_max,1)
                    print line_1
                    line_1_arc = ((line_1[-1]-line_1[0])/(len(line_1)-1))
                    price_max_min = self.close_price[last_max_index:current_price_index+1]
                    print price_max_min
                    line_2 = splinerestruct(price_max_min,1)
                    print line_2
                    line_2_arc = ((line_2[-1]-line_2[0])/(len(line_2)-1))
...
#######
        ...
        print "small max min%s %s"%(small_imf_max_true_index[-1],small_imf_min_true_index[-1])
        print "last small max min%s"%self.imf1_last_last
        print "last small max min%s"%self.imf1_last
        ...
        cri_sample = []
        for i in range(5):
            cri_sample.append(i+1)
        for i in range(5):
            ma5_line.append(self.ma(self.close_price, 5, -i, current_price_index))
        #ma5_line = splinerestruct(ma5_line,1)
        ma5_line.reverse()
        print "ma5_line%s"%ma5_line
        ma5_arc=((ma5_line[-1]-ma5_line[0])/(len(ma5_line)-1))
        print "ma5 arc%s"%ma5_arc

        for i in range(5):
            macd_line.append(self.macd[current_price_index-i])
        #macd_line = splinerestruct(macd_line,1)
        macd_line.reverse()
        print "macd_line%s"%macd_line
        macd_arc=((macd_line[-1]-macd_line[0])/(len(macd_line)-1))
        print "macd arc%s"%macd_arc


        #qiangshi = (self.close_price[current_price_index]-self.close_price[current_price_index-1])*self.vol[current_price_index]
        #self.qiangshi_index.append(qiangshi)


        print "ma macd %s"%(np.corrcoef(ma5_line,macd_line)[0][1])
        print "ma up %s"%(np.corrcoef(ma5_line,cri_sample)[0][1])
        print "macd up %s"%(np.corrcoef(macd_line,cri_sample)[0][1])

        corre = np.correlate(self.close_price[current_price_index-9:current_price_index+1],imf_process[-10:],"same")
        print "corre %s"%(corre)
        #corre = list(splinerestruct(corre,1))
        print "corre %s"%(corre)
        cri_sample=[]
        for i in range(10):
            cri_sample.append(i)
        print cri_sample
        corr_decision1=1
        print "corr core%s "%(np.corrcoef(corre,cri_sample)[0][1])
        if (np.corrcoef(corre,cri_sample)[0][1])>0.5:
            corr_decision1=0
        print "\n"
        print "corre %s"%list(corre[-10:])
        macd_ma_decision=1
        if (np.corrcoef(ma5_line,macd_line)[0][1])<0:
            macd_ma_decision=0
        ...
        print "macd up %s"%(np.corrcoef(macd_line,cri_sample)[0][1])
        print "ma up %s"%(np.corrcoef(ma5_line,cri_sample)[0][1])
        print self.ma_count1
        print self.ma_count
 
        if  (self.ma_count<=self.ma_count2<0 and self.ma_count<=-5) :#or (self.ma_count1<=self.ma_count and self.ma_count1<0 and (np.corrcoef(macd_line,cri_sample)[0][1])<-0.8) :#or ((np.corrcoef(ma5_line,cri_sample)[0][1])<-0.9 and (np.corrcoef(macd_line,cri_sample)[0][1])<-0.9):
            ma_decision3 = 0
        print "ma_decision3 %s"%ma_decision3
        ma_decision4=0
        if self.ma_count1>0 or deltama5>0 :
            ma_decision4=1
        print "ma_decision4 %s"%ma_decision4

        up_decision1 = 1
        if (np.corrcoef(macd_line,cri_sample)[0][1])<-0.8 and up_potion>1.6:
            up_decision1 = 0
 ...
 #       if distance_decision2 ==1 and self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0.01:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
        if     self.buy_price!=0 and imf1_flag==2 and ((current_price-self.buy_price)/self.buy_price) >0.01:#(imf_extrem_decision6==1 ) :#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            print "open%s close%s"%(self.open_price[current_price_index],current_price)
...
#            self.ma_record=[]
  ...
  #      elif delta_macd1>delta_macd2 and macd_ma_decision==1 and  ma_decision3==1 and  self.buy_price==0 and  up_potion_decision2==1 and    down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
        elif   ma_decision4==1 and ma_decision3==1 and self.fail_flag==0 and self.buy_price==0 and  imf0_flag==1 and imf1_flag==1 and delta_buy_decision1==1  :#imf_extrem_decision6==-1  :
...
#        (imf, residual) = my_emd.emd(0.03,0.03)
        ...
        (imf0,imf1,imf2,imf3)= my_eemd.eemd_process(emd_data,10)
        imf=[]
        imf.append(imf0)
        imf.append(imf1)
        imf.append(imf2)
        ...
        imf_first = imf
        if 0:
            imf_data = np.array(imf[2])
            imf_max_index = list(argrelextrema(imf_data,np.greater)[0])
            imf_min_index = list(argrelextrema(imf_data,np.less)[0])
            tmp = imf_max_index+imf_min_index
            tmp.sort()
            if tmp[-1]<490:
...
#####
        ...
        tmp=[(self.qiangshi_index[i]+self.qiangshi_index[i+1]+self.qiangshi_index[i+2]+self.qiangshi_index[i+3]+self.qiangshi_index[i+4])/5 for i in range(len(self.qiangshi_index)-5)]
        self.qiangshi_index=tmp
        
        plt.subplot(413)
        plt.plot([i for i in range(len(self.qiangshi_index))],self.qiangshi_index,'b')             
