...
########
        ...
        if self.imfflag!=0 and self.imfday<5:
            self.imfday+=1
        elif self.imfday>=5:
            self.imfday=0
            self.imfflag=0
        ...
        if imf_close_flag==1:
            self.imfflag=1
            self.imfday=1
        elif imf_close_flag==2:
            self.imfflag=0
            self.imfday=0

        if self.bollflag!=0 and self.bollday<3:
            self.bollday+=1
        elif self.bollday>=3:
            self.bollday=0
            self.bollflag=0
        ...
        print "imfdayflag %s %s "%(self.imfday,self.imfflag)
        print "bolldayflag %s %s "%(self.bollday,self.bollflag)
        ma_de=1
        if ma3<ma5<ma10<ma30:
            ma_de=0
        ma_de=0
        if deltama30>0:#deltama10_before:
            ma_de=1
        ...
        if  self.buy_price!=0  and (boll_de==2):# or (deltama5<0 and deltama5_before>0 ) ) :#(imf_flag==2) and deltama3<deltama3_before  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
            ...
            if self.fail_flag!=0:
                ...
                self.one_time_day=1
                self.last_buy_result=0
                self.buy_std.append(np.mean(self.emdstd[-5:]))
...
#####
        ...
        print "std emd%s"%(np.std(close_price[i-30:i])/np.mean(close_price[i-30:i]))
        print "std emd%s"%(np.mean(close_price[i-30:i])/np.std(close_price[i-30:i]))
        c=(np.mean(close_price[i-30:i])/np.std(close_price[i-30:i]))
        c1=(np.mean(close_price[i-31:i-1])/np.std(close_price[i-31:i-1]))
        print "c1 c%s %s"%(c1,c)
        #my_emd = one_dimension_emd(pre_emd_data)
        #(imf, residual) = my_emd.emd(0.01,0.01)
        #presnr.append(calc_SNR(pre_emd_data,imf)[0])
        deltama3 = ma_func(close_price, 3, 0, i-1)-ma_func(close_price, 3, -1, i-1)
        deltama10 = ma_func(close_price, 10, 0, i-1)-ma_func(close_price, 10, -1, i-1)
        deltama3_before = ma_func(close_price, 3, -1, i-1)-ma_func(close_price, 3, -2, i-1)
        deltama10_before = ma_func(close_price, 10, -1, i-1)-ma_func(close_price, 10, -2, i-1)
        print "deltama3%s %s"%(deltama3_before,deltama3)
        ...
        #if (np.mean(close_price[i-30:i])/np.std(close_price[i-30:i]))<25:
        #if c>c1 or (deltama3_before>deltama3 and deltama3<0):
        if deltama10_before>deltama10:
            emd_data = ma[i-500:i]
            print "ma2"
        else:
