...
####
        ...
        imf1_or_flag=0
        if imf1_min_index[-1]>imf1_max_index[-1] and imf1_min_index[-1]>=197 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf1_or_flag=1
...
###
        ...
        imf2_or_flag=0
        if ((imf2_min_index[-1]<imf2_max_index[-1] and imf2_max_index[-1]<=195)):# or (imf2_min_index[-1]>imf2_max_index[-1] and imf2_min_index[-1]>=797)) and abs(imf_list[2][-1]/imf_list[2][imf2_max_index[-1]])>0.9: 
...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if     imf1_or_flag!=2 :#and imf2_or_flag==1:#and residual_flag==1:#and imf1_flag!=2 :#and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
###
    ...
    #        ma_list=self.buy_ma
      ...
      #      bayes_de = 2
        ...
        max_index = argrelextrema(np.array(datac[-10:]),np.greater)[0]
        min_index = argrelextrema(np.array(datac[-10:]),np.less)[0]
        ...
        if self.buy_price==0:
            pro=calc_pro(min_diff_index,min_index)
            pro1=calc_pro(min_diff_index,max_index)
            print "pro {}".format(pro)
            print "pro1 {}".format(pro1)
            if pro>0.5:
                sel=1
            else:
                sel=2
      ...
      #  print c2/float(c1)
       ...
       # self.radio1.append(c2/float(c1))
        ...
        for i in range(len(datac)):
            print "b_flag {}".format(b_flag)
            print "cp {}".format(datac[i])
            print "mp {}".format(mp[i])
            if mp[i]<datac[i] and b_flag==0:
                c11 += 1
                b_flag=datac[i]
                d2=0
        ...
        jede=0
        sel=1
        if 1:#sel==1:#self.sel%2==1:#s1<s2:#np.std(rp1)/np.mean(rp1)>np.std(rp)/np.mean(rp):
...
#        data11 = self.je
        ...
        plt.plot([i for i in range(len(data1))],data1,'o',[i for i in range(len(data1))],data1,self.buy_x_index,data1_buy,'ro',self.sell_x_index,data1_sell,'go')#,self.buy_x_index,ma_buy_value,'r*',self.sell_x_index,ma_sell_value,'g*')             
