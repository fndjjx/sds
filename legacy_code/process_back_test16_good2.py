...
#####
        ...
        #        imf_max_index.pop()
        #if imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>=997  :#and imf_min_index[-1]-imf_max_index[-1]>10:#and ex_flag==1:#and imf_process[-1]<0 :
        #    imf2_flag=1
        #if imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>=997:
        #    imf2_flag=2
...
########
        ...
        boll_low2 = 3*(-np.std(self.close_price[current_price_index-period1:current_price_index+1]))+np.mean(self.close_price[current_price_index-period1:current_price_index+1])
        ...
        current_price=self.close_price[current_price_index]
        #boll_de=0
        #if self.close_price[current_price_index-2]<boll_low1_b2 and self.close_price[current_price_index-1]>boll_low1_b and current_price>boll_low1:
        #    boll_de=1
        ...
        flag_fail=0
        if  self.buy_price!=0  and (boll_de==2 ) and distance_decision2==1:#and ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
        ...
        elif   self.buy_price!=0 and ma3<boll_low1:#((((self.buy_mean_price-current_mean_price)/self.buy_mean_price)>0.01 ) )  and self.share > 0 and np.mean(self.snr[-3:])<np.mean(self.snr[-4:-1]): #and (self.buy_price-current_price)<0 and self.share > 0:
            print "sell sell 2"
            if self.fail_flag==0:
                if self.buy_price>trade_price:
                    self.fail_flag=flag_fail
...
###
    ...
    for ii in range(period,len(close_price)):
        ma.append(np.mean(macd[ii-(period-1):ii+1]))
