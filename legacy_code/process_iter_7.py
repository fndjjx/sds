...
class process_util():
  ...
  #          decision = 1
   ...
   #         fp.writelines("\n")
    ...
    def run_predict_price_period(self,imf,current_price_index,residual,imf_next):
...
#            ma_decision3 = 0
        ...
        if self.ma(self.close_price, 5, 0, current_price_index)-self.ma(self.close_price, 5, -1, current_price_index) > self.ma(self.close_price, 5, -1, current_price_index)-self.ma(self.close_price, 5, -2, current_price_index) and self.ma(self.close_price, 5, 0, current_price_index)-self.ma(self.close_price, 5, -1, current_price_index) > 0:
            ...
            ma_decision61 = 0
        if self.ma(self.close_price, 20, 0, current_price_index)-self.ma(self.close_price, 20, -1, current_price_index) > self.ma(self.close_price, 20, -1, current_price_index)-self.ma(self.close_price, 20, -2, current_price_index):
            ma_decision9 = 1
        else:
            ma_decision9 = 0
...
#        else:
#            ex_decision5 = 0
        if self.before_last_min-current_last_min>=2 or self.before_last_max-current_last_max<=-2 :
            ex_decision5 = 1
        else:
            ex_decision5 = 0
...
#            self.sell_y_index.append(current_price)
        if imf_max_index[-1]>imf_min_index[-1]  and imf_max_index[-1] - imf_max_index[-2]>20 and  self.share>0  :
            print "sell 1"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
...
#            self.last_buy_flag = 0
#            self.sell_y_index.append(current_price)
#        elif  float(self.buy_price)!=0 and (float(self.buy_price)-float(current_price))/float(self.buy_price)>0.02 and self.share>0:
#            print "sell 4"
#            self.money = self.sell(self.share,trade_price)
#            self.share = 0
#            self.buy_price = 0
...
#            self.last_buy_flag = 1
        ...
        elif imf_min_index[-1]-imf_max_index[-1]>50 and self.money>0 and now_position_last_min_delta < 10:#and (self.ma(self.close_price, 20, 0, current_price_index) - self.ma(self.close_price, 20, -1, current_price_index) > 0.2) and imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]-imf_min_index[-2]>20 : #and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 5, -1, current_price_index)) :#and imf_power_decision == -1 and power_decision == -1 and self.money>0 :
            print "buy 1"
            self.share = self.buy(self.money,trade_price)
            self.money = 0
            self.buy_price = trade_price
...
#            self.last_buy_flag = 1
  ...
  #      power_next = result - imf_ma3_next
   ...
   #         imf.pop()
        ...
        fp = open("imf%s"%(len(imf)-2))
        lines = fp.readlines()
        fp.close()
        imf_raw_last = []
        for eachline in lines:
        #    print eachline.split(" ")[0]
            imf_raw_last.append(float(eachline.split("\n")[0]))
        imf = imf_without3
        ...
        self.run_predict_price_period(imf,current_index,residual,imf_raw1)
