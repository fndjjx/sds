...
class process_util():
    ...
    def __init__(self,open_price,close_price,macd,date,vol):
        ...
        self.fail_flag = 0
        self.up_dict = {}.fromkeys(('delta_ma_decision1','delta_price_decision1','std_decision1','vol_decision1','macd_decision8','extrem_decision10','power_decision2'),0)
        self.down_dict = {}.fromkeys(('delta_ma_decision1','delta_price_decision1','std_decision1','vol_decision1','macd_decision8','extrem_decision10','power_decision2'),0)
        self.delta_ma_decision1 = 0
        self.delta_price_decision1 = 0
        self.std_decision1 = 0
        self.vol_decision1 = 0
        self.macd_decision8 = 0
        self.power_decision2 = 0
        self.up_count = 0
        self.down_count = 0


    def stat(self,buy_price,trade_price,delta_ma_decision1,delta_price_decision1,std_decision1,vol_decision1,macd_decision8,extrem_decision10):
        print "delta_ma_decision1 %s"%self.delta_ma_decision1
        print " delta_price_decision1  %s"%self.delta_price_decision1
        print "std_decision1 %s"%self.std_decision1
        print "vol_decision1 %s"%self.vol_decision1
        if buy_price<trade_price:
            self.up_count += 1
            for key in self.up_dict:
                print "key %s"%key
                if eval("self."+key) == 1:
                    self.up_dict[key] += 1
        if buy_price>trade_price:
...
#####################
        ...
        imf_ma10_before = self.ma(imf, 10 , -1, current_index)
        imf_ma5_current = (imf[current_index-4]+imf[current_index-3]+imf[current_index-2]+imf[current_index-1]+imf[current_index])/5.0
        imf_ma10_current = self.ma(imf, 10 , 0, current_index)
        imf_power_before = imf[current_index-1] - imf_ma5_before
        imf_power_before10 = imf[current_index-1] - imf_ma10_before
        imf_power_current = imf[current_index] - imf_ma5_current
        imf_power_current10 = imf[current_index] - imf_ma10_current
        print "imf power before %s"%imf_power_before
        print "imf power before10 %s"%imf_power_before10
        print "imf power current %s"%imf_power_current
        print "imf power current10 %s"%imf_power_current10

        if (imf_power_before10>0 and imf_power_current10<0): #or float(imf_power_before)/imf_power_current > 2.5:
            imf_power_decision10 = 1
        if (imf_power_before10<0 and imf_power_current10>0):# or float(imf_power_current)/imf_power_before > 2.5:
            imf_power_decision10 = -1
...
#            ma_decision3 = 0
        ...
        vol_ma10_before = self.ma(self.vol,10,-1,current_price_index)
        vol_ma20 = self.ma(self.vol,20,0,current_price_index)
        vol_ma20_before = self.ma(self.vol,20,-1,current_price_index)
        print "vol 5 %s"%vol_ma5
        print "vol 10 %s"%vol_ma10
        print "vol 10 before %s"%vol_ma10_before
        print "vol 20 %s"%vol_ma20
        print "vol 20 before %s"%vol_ma20_before
        ...
        print "delta ma 60 %s"%(self.ma(self.close_price, 60, 0, current_price_index)-self.ma(self.close_price, 60, -1, current_price_index))
...
#            macd_decision1 = 1
        ...
        if self.macd[current_price_index-1]>self.macd[current_price_index-2] and self.macd[current_price_index]>self.macd[current_price_index-1]:
            macd_decision22 = 0
        else:
            macd_decision22 = 1
...
#        elif self.macd[current_price_index] > 0 and ((self.macd[current_price_index-1]-self.macd[current_price_index-2])>(self.macd[current_price_index]-self.macd[current_price_index-1])):
        ...
        if macd_5<0 and macd_5_before<0  and macd_5_before2<0 and macd_5_before3>0: 
            ...
            macd_decision7 = 1
        if (self.macd[current_price_index]<0 and self.macd[current_price_index]>self.macd[current_price_index-1] and self.macd[current_price_index-2]<self.macd[current_price_index-1]) or (self.macd[current_price_index]>0 and self.macd[current_price_index]>self.macd[current_price_index-1]):
            macd_decision8 = 1
        else:
            macd_decision8 = 0
        ...
        tomorrow_will = 0
        print "last imf%s "%self.last_imf



        delta_ma_3 = self.ma(self.close_price, 3, 0, current_price_index)-self.ma(self.close_price, 3, -1, current_price_index)
        delta_ma_5 = self.ma(self.close_price, 5, 0, current_price_index)-self.ma(self.close_price, 5, -1, current_price_index)
        delta_delta_ma_5 = self.ma(self.close_price, 5, 0, current_price_index)-2*self.ma(self.close_price, 5, -1, current_price_index)+self.ma(self.close_price, 5, -2, current_price_index)
        delta_ma_7 = self.ma(self.close_price, 7, 0, current_price_index)-self.ma(self.close_price, 7, -1, current_price_index)
        delta_ma_10 = self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index)
        delta_delta_ma_10 = self.ma(self.close_price, 10, 0, current_price_index)-2*self.ma(self.close_price, 10, -1, current_price_index)+self.ma(self.close_price, 10, -2, current_price_index)
        delta_ma_20 = self.ma(self.close_price, 20, 0, current_price_index)-self.ma(self.close_price, 20, -1, current_price_index)
        delta_delta_ma_20 = self.ma(self.close_price, 20, 0, current_price_index)-2*self.ma(self.close_price, 20, -1, current_price_index)+self.ma(self.close_price, 20, -2, current_price_index)
        delta_ma_60 = self.ma(self.close_price, 60, 0, current_price_index)-self.ma(self.close_price, 60, -1, current_price_index)

        delta_ma_decision1 = 0
        delta_list = [delta_ma_3,delta_ma_5,delta_ma_10,delta_ma_20,delta_ma_60]
        num_po = len([i for i in delta_list if i > 0])
        num_ne = len([i for i in delta_list if i < 0])
        print "num po %s"%num_po
        print "num ne %s"%num_ne
        if num_ne<num_po:
            delta_ma_decision1 = 1
        else:
            delta_ma_decision1 = 0

        delta_delta_ma_decision1 = 0
        delta_delta_list = [delta_delta_ma_5,delta_delta_ma_10,delta_delta_ma_20]
        deltadelta_num_po = len([i for i in delta_delta_list if i > 0])
        deltadelta_num_ne = len([i for i in delta_delta_list if i < 0])
        print "deltadelta num po %s"%deltadelta_num_po
        print "deltadelta num ne %s"%deltadelta_num_ne
        if deltadelta_num_ne<deltadelta_num_po:
            delta_delta_ma_decision1 = 1
        else:
            delta_delta_ma_decision1 = 0

        if delta_ma_5 > 0 and deltadelta_num_po ==3:
            delta_delta_ma_decision2 = 1
        else:
            delta_delta_ma_decision2 = 0
        
        print "hold time %s"%self.holdtime


        if self.holdtime !=0:
            self.holdtime += 1

        if self.close_price[current_price_index]> self.close_price[current_price_index-1]> self.close_price[current_price_index-2]>self.close_price[current_price_index-3]>self.close_price[current_price_index-4]:
            delta_price_decision1 = 0
        else:
            delta_price_decision1 = 1
        print "delta price decision %s"%delta_price_decision1


        if current_price>max(self.close_price[current_price_index-100:current_price_index]):
            delta_price_decision2 = 0
        else:
            delta_price_decision2 = 1


        #if ((self.vol[current_price_index-1]-self.vol[current_price_index])/self.vol[current_price_index-1])<0.5 :
        if self.vol[current_price_index]>vol_ma5 :
            vol_decision1 = 1
        else:
            vol_decision1 = 0

 
        if now_position_last_min_delta == 1 or now_position_last_min_delta == 2:
            extrem_decision10 = 1
        else:
            extrem_decision10 = 0
...
#        if self.buy_price!=0 and ((power_before-power_current)/power_before)<0.5 and self.share > 0 :
#            print "sell 1"
#            self.money = self.sell(self.share,trade_price)
#            self.share = 0
#            self.buy_price = 0
...
#            self.buy_macd = 0
        once_flag = 0
        if self.buy_price!=0 and ((macd_decision2 == 1 and (imf_power_decision == 1 ) ) or (((trade_price-self.buy_price)/self.buy_price)>0.03 and macd_decision22 == 1) )and self.share > 0 :
            print "sell 1"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            ...
            self.holdtime = 0
            once_flag = 1
            self.stat(self.buy_price,trade_price,self.delta_ma_decision1,self.delta_price_decision1,self.std_decision1,self.vol_decision1,self.macd_decision8,self.extrem_decision10)
            self.buy_price = 0
            self.delta_ma_decision1 = 0
            self.delta_price_decision1 = 0                     
            self.std_decision1 = 0
            ...
            self.macd_decision8 = 0
            self.power_decision2 = 0
            self.extrem_decision10 = 0
        if self.buy_price!=0 and (((self.hold_time > 2 ) and ((trade_price-self.buy_price)/self.buy_price)>0.015) or (self.hold_time == 5  ) )and self.share > 0 :
            print "sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            if self.buy_price>trade_price:
...
#            tomorrow_will = -1
        elif once_flag == 0 and self.money>0 and (imf_power_decision == -1 or imf_power_decision == -1 ):# and macd_decision8 == 1 and power_decision2 == 1:#and imf[-1]<self.ma(imf, 10, 0, current_index):#and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 10, 0, current_price_index)) :#imf[-1]>imf[-2] and current_price<self.ma(self.close_price, 5, 0, current_price_index) and self.vol[current_price_index]>self.vol[current_price_index-1] and ((trade_price-current_price)/current_price)<0.05 and ( now_position_last_min_delta == 1 or now_position_last_min_delta == 1) and  self.money>0 and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0  and self.money>0:
#        elif self.macd[current_price_index]>self.macd[current_price_index-1] and std_decision1 == 1 and (delta_delta_ma_decision2 == 1 or delta_delta_ma_decision1 == 1) and delta_price_decision1 == 1 and delta_ma_decision1 == 1 and self.money>0 and (imf_power_decision == -1 or imf_power_decision == -1 ) and  self.money>0 :#and residual[-2]<residual[-1] :#and imf[-1]<self.ma(imf, 10, 0, current_index):#and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 10, 0, current_price_index)) :#imf[-1]>imf[-2] and current_price<self.ma(self.close_price, 5, 0, current_price_index) and self.vol[current_price_index]>self.vol[current_price_index-1] and ((trade_price-current_price)/current_price)<0.05 and ( now_position_last_min_delta == 1 or now_position_last_min_delta == 1) and  self.money>0 and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0  and self.money>0:
            if self.fail_flag == 0:
                print "buy buy 1"
                self.share = self.buy(self.money,trade_price)
                self.money = 0
                ...
                tomorrow_will = -1
                self.holdtime = 1
                self.delta_ma_decision1 = delta_ma_decision1 
                self.delta_price_decision1 = delta_price_decision1
                self.std_decision1 = std_decision1
                self.vol_decision1 = vol_decision1
                self.macd_decision8 = macd_decision8
                self.extrem_decision10 = extrem_decision10
                self.power_decision2 = power_decision2
            else:
                print "fail flag %s"%self.fail_flag
                self.fail_flag -= 1
#        elif  imf_power_decision == -1 and imf_power_current>0 and imf[-1]<current_price and self.last_imf_power < 0 and  self.money>0 and power_decision == -1 and self.money>0 and (self.ma(self.close_price, 5, 0, current_price_index)-self.ma(self.close_price, 5, -1, current_price_index))>(self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index)):
#            print "buy buy 2"
#            self.share = self.buy(self.money,trade_price)
#            self.money = 0
#            self.buy_price = trade_price
...
#            tomorrow_will = -1
        ...
        self.last_imf_power_decision = imf_power_decision
        self.last_imf = imf[-1]
...
#                imf_raw2.append(float(eachline.split("\n")[0]))
            ...
            self.run_predict_price_period(imf,current_index,residual,imf_without2,date,datafile)
            endtime = datetime.datetime.now()
            print "run time"
            print (endtime - starttime).seconds
        print self.up_dict
        print self.down_dict
        print "up count %s"%self.up_count
        print "down count %s"%self.down_count
...
if __name__ == "__main__":
    ...
    print "begin %s"%begin
    process.generate_emd_data_fix(close_price,begin,70)
