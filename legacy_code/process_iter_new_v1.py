...
##############################################################
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            #    imf_extrem_decision = 1
            if abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                if imf_max_true_index[-1]<imf_min_true_index[-1] and imf_min_true_index[-1]<490:
                ...
                #    print "last min max %s"%last_min_max
                    ...
                    if  abs(current_last_max-last_min_max)/float(last_min_max)<=0.3:
                        imf_extrem_decision = -1
                elif imf_max_true_index[-1]>imf_min_true_index[-1] and imf_max_true_index[-1]>490:
                    ...
                    if  abs(last_min_max-current_last_max)/float(last_min_max)<=0.3:
                        imf_extrem_decision = -1
            if abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:
                if imf_min_true_index[-1]<imf_max_true_index[-1] and imf_max_true_index[-1]<490:
                    ...
                    if  abs(current_last_min-last_max_min)/float(last_max_min)<=0.3:
                        imf_extrem_decision = 1
                    ...
                    print "current_last_min %s"%current_last_min
                    print "potion %s"%(abs(last_max_min-current_last_min)/float(last_max_min))
                    if  abs(last_max_min-current_last_min)/float(last_max_min)<=0.3:
                        imf_extrem_decision = 1
                elif imf_min_true_index[-1]>imf_max_true_index[-1] and imf_min_true_index[-1]<490:
                    last_max_min = imf_min_true_index[-1] - imf_max_true_index[-1]
                    print "last max min %s"%last_max_min
...
#            self.holdtime = 0
#            once_flag = 1
#        elif self.buy_price!=0 and ((trade_price-self.buy_price)/self.buy_price) <-0.03 and self.share > 0 :
#            print "sell 3"
#            self.money = self.sell(self.share,trade_price)
#            self.share = 0
#            self.buy_price = 0
