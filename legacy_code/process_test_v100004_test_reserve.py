...
##############################################################
     ...
     #       small_imf_max_true_index = small_imf_max_index
        ...
        imf1_flag=0
        if small_imf_min_true_index[-1]>=496 and small_imf_min_true_index[-1]>small_imf_max_true_index[-1] :#and self.imf1_last_last=="max":
            imf1_flag=1
        if small_imf_max_true_index[-1]<=495 and small_imf_max_true_index[-1]>small_imf_max_true_index[-1] :#and self.imf1_last_last=="max":
            imf1_flag=1
        if small_imf_max_true_index[-1]>=496 and small_imf_max_true_index[-1]>small_imf_min_true_index[-1] :#and self.imf1_last_last=="min":
            imf1_flag=2
        if small_imf_min_true_index[-1]<=495 and small_imf_min_true_index[-1]>small_imf_min_true_index[-1] :#and self.imf1_last_last=="min":
            imf1_flag=2
...
##############
                    ...
                    last_max_index = current_price_index-499+imf_max_true_index[-1]
                    price_max_min = self.close_price[last_max_index:last_min_index+1]
                    price_min_max = self.close_price[last_min_index:current_price_index+1]
                    line_1_1 = splinerestruct(price_max_min,1)
                    line_1_1_arc = ((line_1_1[-1]-line_1_1[0])/(len(line_1_1)-1))
                    line_2_2 = splinerestruct(price_min_max,1)
                    line_2_2_arc = ((line_2_2[-1]-line_2_2[0])/(len(line_2_2)-1))
                    print "line 1 sell arc%s"%line_1_1_arc
                    print "line 2 sell arc%s"%line_2_2_arc

#####
                    if imf1_flag==1 and  (imf_min_true_index[-1]>496 or (self.last_last=="max" and imf1_flag==1)) and line_1_arc>line_2_arc :#and imf1_flag==1:#and abs(((self.core+self.core2)/2.0)-current_core)>3 :#and abs(self.core-self.core3)<=2 and abs(self.core-self.core2)<=2:#est_per%2==0 and (yu<0.3 or yu>0.7):
                    ...
                    if imf1_flag==2 and imf_min_true_index[-1]<496 and line_2_2_arc>line_1_1_arc and (current_core-self.buy_core)>2:
                        down_test_decision1=1   
...
#####
                    ...
                    last_min_index = current_price_index-499+imf_min_true_index[-1]
                    last_max_index = current_price_index-499+imf_max_true_index[-1]
                    last_last_max_index = current_price_index-499+imf_max_true_index[-2]
                    price_max_min = self.close_price[last_last_max_index:last_min_index+1]
                    price_min_max = self.close_price[last_min_index:last_max_index+1]
                    line_1_1 = splinerestruct(price_max_min,1)
                    line_1_1_arc = ((line_1_1[-1]-line_1_1[0])/(len(line_1_1)-1))
                    line_2_2 = splinerestruct(price_min_max,1)
...
#                    if line_arc>line_arc_before>line_arc_before2 and line_arc_before2<line_arc_before3:
			...
			b2data =  self.close_price[current_core:current_price_index-1]
                    ...
                    if  imf_max_true_index[-1]<=496 and imf1_flag==1 and line_1_arc>line_2_arc:
                        down_test_decision1=-1
                    ...
                    if imf_max_true_index[-1]>=495 and  self.buy_price!=0 and line_2_2_arc>line_1_1_arc and (current_core-self.buy_core)>2 :#and self.last_last=="min":
                        down_test_decision1=1
                    self.core3 = self.core2
                    self.core2 = self.core
                    self.core = current_core
...
###############
        ...
        up_decision1 = 1
        if self.ma_count<0 and up_potion>1.5:
            up_decision1 = 0
...
#######
        ...
        corre = np.correlate(self.close_price[current_price_index-4:current_price_index+1],imf_process[-5:],"same")
        print "corre %s"%(corre)
        ...
        print "corr core%s "%(np.corrcoef(self.close_price[current_price_index-4:current_price_index+1],imf_process[-5:])[0][1])
        print self.close_price[current_price_index-4:current_price_index+1]
        print imf_process[-5:]
        #if (np.corrcoef(corre,cri_sample)[0][1])>0.5:
        if (np.corrcoef(self.close_price[current_price_index-4:current_price_index+1],imf_process[-5:])[0][1])>0.5:
            corr_decision1=1
        print "\n"
        ...
        if self.fail_flag>0 and self.fail_flag<15:
            self.fail_flag+=1
        else:
            self.fail_flag=0

        up_decision3 = 1
        if  ((np.corrcoef(macd_line,cri_sample)[0][1])<0  and (up_potion>1.5 )) or ((np.corrcoef(macd_line,cri_sample)[0][1])<-0.85):
            up_decision3 = 0
 ...
 #       if distance_decision2 ==1 and self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0.01:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
        if  distance_decision2 ==1 and  self.buy_price!=0 and  down_test_decision1==1 and ((current_price-self.buy_price)/self.buy_price) >0:#(imf_extrem_decision6==1 ) :#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            print "open%s close%s"%(self.open_price[current_price_index],current_price)
            ...
            self.buy_core=0
            self.fail_flag=1
        elif self.buy_price!=0 and self.buy_day>10 and   (down_test_decision1==1 ) and self.share > 0 :
            print "sell sell 4"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.total_asset = self.money - self.sell_fee(self.money)
...
#            self.ma_record=[]
  ...
  #      elif delta_macd1>delta_macd2 and macd_ma_decision==1 and  ma_decision3==1 and  self.buy_price==0 and  up_potion_decision2==1 and    down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
        elif  up_decision3==1 and  delta_macd1>delta_macd2 and self.fail_flag==0 and self.buy_price==0 and  down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
