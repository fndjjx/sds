...
##
        ...
        sel=0
        if self.buy_price==0:
            if stab_de==1 :#and ks_p_value>0.5:#not cdcd>cdcd2:#calc_down(self.close_price[current_price_index-10:current_price_index+1])<3:#0.7<p_c/n_c<1.3 :#and ks_p_value>0.5:#or (np.mean(diffmp[-5:])>0 and stab_de!=1):#self.pimf1[-1][1]>0.4:#self.cu[-1]>0.6 or (len(self.cu)>1 and self.cu[-1]>self.cu[-2]):
                sel=1
            #elif p_c/n_c>1.3:
            else:
                sel=2
        ...
        datac = self.close_price[current_price_index-20:current_price_index+1]
        datav = self.vol[current_price_index-20:current_price_index+1]
        dataj = self.je[current_price_index-20:current_price_index+1]
        mp=[dataj[i]/datav[i] for i in range(len(datav))]
...
##
        ...
        vp,vm,vs=montecarlo_simulate(self.vol[current_price_index-20:current_price_index+1],1000)
        jp,jm,js=montecarlo_simulate(self.je[current_price_index-20:current_price_index+1],1000)
...
##
        ...
        if sel==1:#self.sel%2==1:#s1<s2:#np.std(rp1)/np.mean(rp1)>np.std(rp)/np.mean(rp):
            if self.close_price[current_price_index]<mp and mean_de==1 and self.mclist4[-1]>mp:#len(self.mclist2)>0 and self.mclist2[-1]>0 :#and  self.mclist5[-1]>0:#self.mclist5[-1]>0:#and self.close_price[current_price_index-1]<mp1:
                jede=1
...
#        data11 = self.je
        ...
        plt.plot([i for i in range(len(ma3))],ma3,'y',ma3,'yo')
