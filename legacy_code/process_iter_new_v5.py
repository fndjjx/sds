...
##############################################################
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            if self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                print "prepare buy"
                if imf_max_true_index[-1]<imf_min_true_index[-1] and imf_min_true_index[-1]<485 and len(imf_max_remove_index)!=0:
                ...
                #    if  abs(current_last_max-last_min_max)/float(last_min_max)<=0.25:
                    ...
                    if (buyhigh/buyhigh2)>0.8 and (buyhigh/buyhigh2)<1.2: 
                        imf_extrem_decision = -1
                elif imf_max_true_index[-1]>imf_min_true_index[-1] and imf_max_true_index[-1]<485 and len(imf_min_remove_index)!=0:
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif  sta_decision == 1 and self.buy_price == 0 and distance_decision == 1 and distance_decision3 == 1 and price_delta_decision == 1 and imf_extrem_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
