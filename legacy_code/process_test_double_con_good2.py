...
class process_util():
    ...
    def __init__(self,open_price,close_price,macd,date,vol,high_price,low_price):
        ...
        self.last_power_ma5_decision=0
...
##############################################################
        ...
        small_small_data = np.array(imf_process)
        ...
        imf0_flag=0
        print small_small_imf_min_index[-1]
        print small_small_imf_max_index[-1]
        if small_small_imf_min_index[-1]>small_small_imf_max_index[-1] and small_small_imf_min_index[-1]>=46 :#and small_small_imf_min_index[-1]-small_small_imf_max_index[-1]>3:#and self.imf1_last_last=="max":
            imf0_flag=1
        if small_small_imf_min_index[-1]<small_small_imf_max_index[-1] and small_small_imf_max_index[-1]>=46:#and self.imf1_last_last=="max":
            imf0_flag=2


        print "imf0 flag%s"%imf0_flag

        big_data = np.array(big_imf[:90])
        ...
        print "\n"
        residual_flag=0
        #if len(imf3_min_index)>0 and len(imf3_max_index)>0 and imf3_max_index[-1]>imf3_min_index[-1] and imf3_max_index[-1]>=70 and imf3_max_index[-1]<90:
        #    residual_flag=1
        #if len(imf3_min_index)>0 and len(imf3_max_index)>0 and imf3_max_index[-1]<imf3_min_index[-1] and imf3_min_index[-1]>90:
        tmp = []
        tmp = imf3_max_index+imf3_min_index
        tmp.sort()
        if len(imf3_min_index)>0 and len(imf3_max_index)>0 and (tmp[-1]-tmp[-2]<5 or tmp[-1]>=45):
            if imf3_min_index[-1]>imf3_max_index[-1]:
                imf3_min_index.pop()
            elif imf3_max_index[-1]>imf3_min_index[-1]:
                imf3_max_index.pop()
             
        if len(imf3_min_index)>0 and len(imf3_max_index)>0 and imf3_max_index[-1]>imf3_min_index[-1] and imf3_max_index[-1]<45:
            residual_flag=1
        if len(imf3_min_index)>0 and len(imf3_max_index)>0 and imf3_max_index[-1]<imf3_min_index[-1] and imf3_min_index[-1]<45:
            residual_flag=2
        if (len(imf3_min_index)==0 and len(imf3_max_index)==0) or (len(imf3_min_index)==0 and len(imf3_max_index)==1):
            residual_flag=3
        print "residual%s"%residual_flag

        imf_flag=0
        if (residual_flag==1 and imf0_flag==1) or (residual_flag==3 and imf0_flag==1):
            imf_flag=1
        if residual_flag==2 and imf0_flag==2:
            imf_flag=2
        print "imf flag%s"%imf_flag

        small_data = np.array(imf_list[-1])
        ...
        if imf2_max_index[-1]>imf2_min_index[-1]:
            current_imf2_core = current_price_index-(99-imf2_max_index[-1])
            current_imf2_sec_core = -(current_price_index-(99-imf2_min_index[-1]))
        else:
            current_imf2_core = -(current_price_index-(99-imf2_min_index[-1]))
            current_imf2_sec_core = current_price_index-(99-imf2_max_index[-1])
        print "\n\n\n"
        ...
        imf1_flag=0
        if small_imf_min_index[-1]>small_imf_max_index[-1] and (((96<small_imf_min_index[-1] and small_imf_max_index[-1]<95)) or (96==small_imf_min_index[-1] and small_imf_max_index[-1]<99))   :#and self.imf1_last_last=="max":
            imf1_flag=1
        if small_imf_min_index[-1]<small_imf_max_index[-1] and small_imf_max_index[-1]>=96:#and self.imf1_last_last=="max":
            imf1_flag=2
        ...
        if 0:#imf_last_max_index!=[] and imf_last_min_index!=[]:
            tmp = []
...
#            imf1_flag=1
        ...
        if imf2_max_index[-1]>imf2_min_index[-1] and 85<imf2_max_index[-1]<95 :#and result_flag==1 :# and result_flag==1 :#and result_flag==1 :#and self.imf1_last_last=="max":
            imf2_flag=1
        if imf2_max_index[-1]<imf2_min_index[-1] and imf2_min_index[-1]>=95 :#and result_flag==1 :# and result_flag==1 :#and result_flag==1 :#and self.imf1_last_last=="max":
            imf2_flag=1
        ...
        if imf2_max_index[-1]<imf2_min_index[-1] and imf2_min_index[-1]<95 :#and  self.close_price[current_price_index]<self.ma(self.close_price, 5, 0, current_price_index):#and self.imf1_last_last=="max":
            imf2_flag=2
        if imf2_max_index[-1]>imf2_min_index[-1] and imf2_max_index[-1]>95 :#and self.result_list[-1]!=1:#and  self.close_price[current_price_index]<self.ma(self.close_price, 5, 0, current_price_index):#and self.imf1_last_last=="max":
            imf2_flag=2
...
#            imf1_flag=2
        ...
        self.imfflaglist.append(imf_flag)
        
        return (imf0_flag,residual_flag)
##########################
...
####################
        ...
        p3 = self.ma(self.close_price, 3, 0, current_price_index)
        p3_before = self.ma(self.close_price, 3, -1, current_price_index)
        p3_before2 = self.ma(self.close_price, 3, -2, current_price_index)
        v3 = self.ma(self.vol, 3, 0, current_price_index)
        v3_before = self.ma(self.vol, 3, -1, current_price_index)
        v3_before2 = self.ma(self.vol, 3, -2, current_price_index)
        pv3 = p3*v3
        pv3_before = p3_before*v3_before
        pv3_before2 = p3_before2*v3_before2

        p5 = self.ma(self.close_price, 5, 0, current_price_index)
        p5_before = self.ma(self.close_price, 5, -1, current_price_index)
        p5_before2 = self.ma(self.close_price, 5, -2, current_price_index)
        v5 = self.ma(self.vol, 5, 0, current_price_index)
        v5_before = self.ma(self.vol, 5, -1, current_price_index)
        v5_before2 = self.ma(self.vol, 5, -2, current_price_index)
        pv5 = p5*v5
        pv5_before = p5_before*v5_before
        pv5_before2 = p5_before2*v5_before2

        print "pv ma3 %s %s "%(pv3_before,pv3)
        print "pv deltama3 %s %s "%(pv3_before-pv3_before2,pv3-pv3_before)
        print "pv ma5 %s %s "%(pv5_before,pv5)
        print "pv deltama5 %s %s "%(pv5_before-pv5_before2,pv5-pv5_before)
        pv3_decision=0
        if pv3-pv3_before>0 and pv3_before-pv3_before2>0:
            pv3_decision=1

        pv = [self.vol[i]*self.close_price[i] for i in range(len(self.vol))]
        pv3 = self.ma(pv, 3, 0, current_price_index)
        pv3_before = self.ma(pv, 3, -1, current_price_index)
        pv3_before2 = self.ma(pv, 3, -2, current_price_index)
        pv5 = self.ma(pv, 5, 0, current_price_index)
        pv5_before = self.ma(pv, 5, -1, current_price_index)
        pv5_before2 = self.ma(pv, 5, -2, current_price_index)
        print "pv ma3 %s %s "%(pv3_before,pv3)
        print "pv deltama3 %s %s "%(pv3_before-pv3_before2,pv3-pv3_before)
        print "pv ma5 %s %s "%(pv5_before,pv5)
        print "pv deltama5 %s %s "%(pv5_before-pv5_before2,pv5-pv5_before)
        ...
        pv5_decision=0
        if pv5-pv5_before>0  and pv5_before-pv5_before2>0:
            pv5_decision=1
...
##########
        ...
        self.last_power_ma5_decision=power_ma5_decision
...
#########
        ...
        rsi_decision=1
        if self.rsi(self.close_price,3,current_price_index)>70 or self.rsi(self.close_price,5,current_price_index)>70:
            rsi_decision=0
...
#######
 ...
 #       if distance_decision2 ==1 and self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0.01:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
        if  (imf2_flag==2 or deltama5_before>deltama5 ) and  self.buy_price!=0  and ((current_price-self.buy_price)/self.buy_price) >0:#(imf_extrem_decision6==1 ) :#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
        ...
        elif   self.buy_price!=0 and ((((self.buy_mean_price-current_mean_price)/self.buy_mean_price)>0.03 ) )  and self.share > 0:
            print "sell sell 2"
            if self.buy_price>trade_price:
                self.fail_flag=1
            self.money = self.sell(self.share,trade_price)
            ...
            self.buy_core=0
            self.one_time_day=0
        elif   self.buy_price!=0 and self.one_time_day>10 and  imf2_flag==2 and self.share > 0:
            print "sell sell 3"
            if self.buy_price>trade_price:
                self.fail_flag=1
            self.money = self.sell(self.share,trade_price)
...
#            self.ma_record=[]
  ...
  #      elif delta_macd1>delta_macd2 and macd_ma_decision==1 and  ma_decision3==1 and  self.buy_price==0 and  up_potion_decision2==1 and    down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
        elif    imf2_flag==1 and     self.buy_price==0 and  delta_buy_decision1==1 and deltama3_before>deltama3 :#((0>deltama3_before and deltama3>0) or (0>deltama3_before2 and deltama3_before>0)):#and power_ma5_decision==-1 :#((deltama5_before>deltama5) or (deltama3_before<deltama3)):#and (self.last_power_ma5_decision==-1 or power_ma5_decision==-1) :#(deltama5>deltama5_before>deltama5_before2 and deltama3_before<deltama3):#( (((deltama3_before<0 and deltama3>0) or ( 0<deltama3_before<deltama3 and deltama5_before<0 and deltama5>0))) )  and delta_buy_decision1==1  :#imf_extrem_decision6==-1  :
...
########
     ...
     #   imf_third.append(imf3)
        ...
        print "50 std %s"%np.std(last_50_price)
        print "std/mean %s"%(np.std(last_50_price)/np.mean(last_50_price))
        print "rsi 2 %s"%self.rsi(self.close_price,2,current_index)
        print "rsi 3 %s"%self.rsi(self.close_price,3,current_index)
        print "rsi 5 %s"%self.rsi(self.close_price,5,current_index)
    ...
    def draw_fig(self,datafile,start,save=0):
        ...
        data1 = self.vol
        #data = [((self.high_price[i]+self.low_price[i])/2)*data1[i] for i in range(len(data))]
        #data = [self.close_price[i]*data1[i] for i in range(len(data))]
...
#
        ...
        #obv = [((2*self.close_price[i]-self.high_price[i]-self.low_price[i])/(self.high_price[i]-self.low_price[i]))*self.vol[i] for i in range(len(self.vol))]
        #ma5 = []
        #for i in range(79):
        #    ma5.append(0)

        #for i in range(80,len(obv)+1):
        #    mean_5 = np.mean(obv[i-79:i+1])
        #    ma5.append(mean_5)
...
if __name__ == "__main__":
    ...
    for eachline in lines:
        ...
        vol.append(float(eachline.split("\t")[6]))
...
##############
    ...
    #generate_emd_func.generate_mavol5_emd_data_fix(10)
    #generate_emd_func.generate_mavol3_emd_data_fix(10)
    #generate_emd_func.generate_mavol2_emd_data_fix(80)
