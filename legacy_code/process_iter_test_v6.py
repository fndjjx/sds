...
##############################################################
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            index_list.sort() 
            last_extrem_position = index_list[-10] 
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1] and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                    ...
                    print [residual[i] for i in list(argrelextrema(np.array(residual),np.greater)[0])]
                    print argrelextrema(np.array(residual),np.less)[0]
                    print [residual[i] for i in list(argrelextrema(np.array(residual),np.less)[0])]
                    ...
                    delta_residual = [residual[i+1]-residual[i] for i in range(0,len(residual)-1)]
                    delta2_residual = [delta_residual[i+1]-delta_residual[i] for i in range(0,len(delta_residual)-1)]
                    delta3_residual = [delta2_residual[i+1]-delta2_residual[i] for i in range(0,len(delta2_residual)-1)]

                    delta_residual_max = list(argrelextrema(np.array(delta_residual[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(delta_residual[:]),np.less)[0])

                    print "1 chafen delta_residual_max %s "%delta_residual_max
                    print "1 chafen delta_residual_min %s "%delta_residual_min

                    data6 = list(np.array(splinerestruct(delta_residual,5)))
                    delta_residual_max = list(argrelextrema(np.array(data6[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(data6[:]),np.less)[0])

                    print "delta_residual_max %s "%delta_residual_max
                    print "delta_residual_min %s "%delta_residual_min

                    delta_residual_max = list(argrelextrema(np.array(delta2_residual[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(delta2_residual[:]),np.less)[0])

                    print "2 chafen delta_residual_max %s "%delta_residual_max
                    print "2 chafen delta_residual_min %s "%delta_residual_min

                    data6 = list(np.array(splinerestruct(delta2_residual,5)))
                    delta_residual_max = list(argrelextrema(np.array(data6[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(data6[:]),np.less)[0])
                    ...
                    delta_residual_max = list(argrelextrema(np.array(delta3_residual[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(delta3_residual[:]),np.less)[0])

                    print "before spline delta_residual_max %s "%delta_residual_max
                    print "before spline delta_residual_min %s "%delta_residual_min

                    data6 = list(np.array(splinerestruct(delta3_residual,5)))
                    delta_residual_max = list(argrelextrema(np.array(data6[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(data6[:]),np.less)[0])
                    ...
                    if max(len(delta_residual_min),len(delta_residual_max))>5:
                        jieshu = 5
                    else:
                        jieshu = 5 #max(len(delta_residual_min),len(delta_residual_max))
                    print "jieshu %s"%jieshu

                    data6 = list(np.array(splinerestruct(delta3_residual,jieshu)))
                    delta2_residual = data6
                    delta_residual_max = list(argrelextrema(np.array(delta2_residual[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(delta2_residual[:]),np.less)[0])
                    ...
                    if len(delta_residual_max)>0 and len(delta_residual_min)>0:
                      
                      #  for i in delta_residual_min:
                      #      if i<8 or i>len(residual)-5:
                      #          delta_residual_min.remove(i)

                      #  for i in delta_residual_max:
                      #      if i<8 or i>len(residual)-5:
                      #          delta_residual_max.remove(i)
                        if len(delta_residual_max)>0 and len(delta_residual_min)>0:
                            tmp = delta_residual_min+delta_residual_max
                            tmp.sort()
                            tmp.pop()
                            for i in tmp:
                                if i<8 or i>len(residual)-3:
                                    tmp.remove(i)
                            print "all index %s"%tmp
                            last_trust_min = imf_min_true_index[-1]
                            if len(tmp)>0:
                                if tmp[0] in delta_residual_max:
                                    for i in range(len(tmp)):
                                        if i%2 == 0:
                                            imf_max_true_index.append(tmp[i]+last_trust_min)
                                        else:
                                            imf_min_true_index.append(tmp[i]+last_trust_min)
                                if tmp[0] in delta_residual_min:
                                    tmp.pop(0)
                                    for i in range(len(tmp)):
                                        if i%2 == 0:
                                            imf_max_true_index.append(tmp[i]+last_trust_min)
                    ...
                    print "imf_max_true_index %s"%imf_max_true_index
                    print "imf_min_true_index %s"%imf_min_true_index
                    print "len imf_max_true_index %s"%len(imf_max_true_index)
                    print "len imf_min_true_index %s"%len(imf_min_true_index)


                    imf_max = imf_max_true_index
                    imf_min = imf_min_true_index
                    if imf_max[0]>imf_min[0] and imf_min[-1]<imf_max[-1]:
                        min_max = [imf_max[i]-imf_min[i] for i in range(len(imf_max))]
                        max_min = [imf_min[i+1]-imf_max[i] for i in range(len(imf_max)-1)]
                    elif imf_max[0]>imf_min[0] and imf_min[-1]>imf_max[-1]:
                    ...
                    min_max.remove(max(min_max))
                    min_max.remove(min(min_max))
                    mean_max_min = np.mean(max_min[-1:])
                    std_max_min = np.std(max_min[-1:])
                    mean_min_max = np.mean(min_max[-1:])
                    std_min_max = np.std(min_max[-1:])
                    print "mean min max %s"%mean_min_max
                    print "std min max %s"%std_min_max
                    print "mean max min %s"%mean_max_min
                    print "std max min %s"%std_max_min

                    if  ((last_max>last_min and last_max!= 0 and 499-last_max>(mean_max_min-2) and 499-last_max<(mean_max_min+2)) or (last_min>last_max and last_min!=0 and 499-last_min<3)):
                        imf_extrem_decision5 = -1
                    print "decision 5 %s"%imf_extrem_decision5
                   ...
                   # residual_spline_min_index = tmp2
                    ...
                    if last_max == 0:
                        ...
                        imf_max_true_index.append(last_max)
                    if last_min == 0:
                        last_min = imf_min_true_index[-1] 
                    else:
                        imf_min_true_index.append(last_min)

                    print "last max %s"%last_max
                    print "last min %s"%last_min
                    
                    if abs(imf_min_true_index[-1]-imf_min_true_index[-2])>15 and abs(imf_max_true_index[-1]-imf_max_true_index[-2])>15 and abs(last_max-last_min)<20 and abs(last_max-last_min)>8 and ((last_max>last_min and last_max!= 0 and 499-last_max>(mean_max_min) and 499-last_max<(mean_max_min+std_max_min/2)) or (last_min>last_max and last_min!=0 and 499-last_min<3)):
                    #if (last_min>last_max and last_min!=0 and 499-last_min<3):
                        imf_extrem_decision2 = -1
                    print "decision %s"%imf_extrem_decision2

#                    if imf_min_true_index[-1]<475 and len(imf_min_remove_index)>0:# and len(imf_max_remove_index)==0:
#                        if len(imf_min_remove_index)>0 and imf_min_remove_index[-1]>495:
#                            imf_extrem_decision2 = -1
#                        else:
#                            imf_extrem_decision2 = 0
#                    print "decision %s"%imf_extrem_decision2
                   # if last_max>last_min and last_max<475:    
                   #     imf_extrem_decision2=-1
                    print "decision %s"%imf_extrem_decision2
                        ...
                        if len(delta_residual_max)>0 and len(delta_residual_min)>0:
                            ...
                            for i in tmp:
                                if i<8 or i>len(residual)-3:
                                    tmp.remove(i)
                            print "all index %s"%tmp
                            last_trust_max = imf_max_true_index[-1]
                            if len(tmp)>0:
                                if tmp[0] in delta_residual_min:
                                    for i in range(len(tmp)):
                                        if i%2 == 0:
                                            imf_min_true_index.append(tmp[i]+last_trust_max)
                                        else:
                                            imf_max_true_index.append(tmp[i]+last_trust_max)
                                if tmp[0] in delta_residual_max:
                                    tmp.pop(0)
                                    for i in range(len(tmp)):
                                        if i%2 == 0:
     ...
     #                           imf_extrem_decision2 = -1
                ...
                #    residual_spline_min_index = tmp2
                    ...
                    if abs(imf_min_true_index[-1]-imf_min_true_index[-2])>15 and abs(imf_max_true_index[-1]-imf_max_true_index[-2])>15 and abs(last_max-last_min)>8 and abs(last_max-last_min)<20 and ((last_max>last_min and last_max!= 0 and 499-last_max>(mean_max_min-std_max_min/2) and 499-last_max<(mean_max_min+std_max_min/2)) or (last_min>last_max and last_min!=0 and 499-last_min<3)):
                    #if (last_min>last_max and last_min!=0 and 499-last_min<3):
                        imf_extrem_decision2 = -1

                    print "decision %s"%imf_extrem_decision2
                 #   if imf_max_true_index[-1]<475 and len(imf_min_remove_index)>0 :#and len(imf_max_remove_index)==0:
                 #       if len(imf_min_remove_index)>0 and imf_min_remove_index[-1]>495:
                 #           imf_extrem_decision2 = -1
                 #       else:
                 #           imf_extrem_decision2 = 0
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif ( imf_extrem_decision5 == -1):# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
