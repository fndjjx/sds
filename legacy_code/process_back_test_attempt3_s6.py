...
##
        ...
        #    if calc_down(mp_list[current_price_index-20:current_price_index+1])<1 and calc_up(mp_list[current_price_index-20:current_price_index+1])>0.5:#0.7<p_c/n_c<1.3 :#and ks_p_value>0.5:#or (np.mean(diffmp[-5:])>0 and stab_de!=1):#self.pimf1[-1][1]>0.4:#self.cu[-1]>0.6 or (len(self.cu)>1 and self.cu[-1]>self.cu[-2]):
            if  sharp(s21)>sharp(s22) :#and not (sharp(s21)<0 and sharp(s22)<0):#len(ma3s1)>1 and ma3s1[-1]>ma3s1[-2]:#np.mean(s21)/np.std(s21)>np.mean(s22)/np.std(s22) and np.mean(s21)/np.std(s21)>0 and np.mean(s21)/np.std(s21)>np.mean(s11)/np.std(s11):#sum(s1)>sum(s2) and sum(s1)>0:#deltama10<0 :#and  s2[-1]>0:#self.mpdown[-1]<mampdown[-1] :#and lp2!=1:
                sel=1
            #elif p_c/n_c>1.3:
...
##
        ...
        if len(self.mclist4)>0:
            self.mclist5.append(self.mclist4[-1]-mp)
        #self.mclist4.append(jm/vm)
        self.mclist4.append(mpm)
...
##
        jede=0
        if sel==1:
            if  diffmp[-1]<0 and mean_de==1 and   len(self.mclist2)>0 and self.mclist2[-1]>0:
                jede=1
...
#        data11 = self.je
        ...
        data2 = self.rihb2
        ma3=ma_func2(self.close_price,3)
        data2 = [ma3[i]-mp[i] for i in range(len(self.close_price))]
        data1 = data2
