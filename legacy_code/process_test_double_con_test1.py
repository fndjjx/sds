...
##############################################################
        ...
        big_data = np.array(big_imf[:trust_position+1])
...
#            imf1_flag=1
        ...
        for i in power_list:
            if i<0:
                count+=1
        result_flag=0
        if len(self.result_list)>2 and self.result_list[-1]!=2 and self.result_list[-2]!=2 and self.result_list[-3]!=2 and self.result_list[-4]!=2 and self.result_list[-5]!=2:
            result_flag=1
        ...
        if imf2_max_index[-1]>imf2_min_index[-1] and 480<=imf2_max_index[-1]<495  and power_0>0 and result_flag==1 :#and self.imf1_last_last=="max":
            imf1_flag=1
        if imf2_max_index[-1]<imf2_min_index[-1] and 485<=imf2_min_index[-1]<495 and  self.close_price[current_price_index]<self.ma(self.close_price, 3, 0, current_price_index):#and self.imf1_last_last=="max":
            imf1_flag=2
...
#            imf1_flag=2
        ...
        self.result_list.append(imf1_flag)

        
        return (imf0_flag,imf1_flag)
##########################
...
#######
        ...
        if    (self.ma_count1<=self.ma_count<0 and self.ma_count1<=-5) or (self.ma_count<=self.ma_count2<0 and self.ma_count<=-10):#and (np.corrcoef(macd_line,cri_sample)[0][1])<-0.8) :#or ((np.corrcoef(ma5_line,cri_sample)[0][1])<-0.9 and (np.corrcoef(macd_line,cri_sample)[0][1])<-0.9):
            ma_decision3 = 0
 ...
 #       if distance_decision2 ==1 and self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0.01:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
        if   self.buy_price!=0 and imf1_flag==2 and ((current_price-self.buy_price)/self.buy_price) >0:#(imf_extrem_decision6==1 ) :#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
...
#            self.ma_record=[]
  ...
  #      elif delta_macd1>delta_macd2 and macd_ma_decision==1 and  ma_decision3==1 and  self.buy_price==0 and  up_potion_decision2==1 and    down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
        elif     imf0_flag!=2 and   self.buy_price==0 and    imf1_flag==1 and delta_buy_decision1==1  :#imf_extrem_decision6==-1  :
