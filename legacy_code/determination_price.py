...
def train_nn(data_file,net_file):
    ...
    ann.create_standard_array((1,15,1))
...
def test_each(net_file,price):#,ma5,ma10,vol5,vol10,macd,vol):
    ...
    return ann.run([price])[0]#,ma5,ma10,vol5,vol10,macd,vol])[0]
...
#        print "total asset is %s"%(float(money)+float(share)*float(price))
    ...
    generate_train_data("data_after_format",10,"train_data",1,"data_price_hxdq","data_ma5_hxdq")
    loops = 1
    threads = []
    for i in range(loops):
        t = threading.Thread(target=train,args=("train_data","net_%s.net"%i,"data_price_hxdq",10,"data_ma5_hxdq"))
        threads.append(t)
	print threads
    for i in range(loops):
        threads[i].start()
