...
class process_util():
    ...
    def generate_imf_extrem(self,residual,expect_next,first_distance):
        ...
        while(not tmp and count<=2 ):#and tmp != -1):
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1] and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                  ...
                  #  residual = price_max_data
                    ...
                    data6 = list(np.array(splinerestruct(delta_residual,3)))
                    ...
                    data6 = list(np.array(splinerestruct(delta2_residual,3)))
                    delta_residual_max = list(argrelextrema(np.array(data6[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(data6[:]),np.less)[0])
                    ...
                    data6 = list(np.array(splinerestruct(delta3_residual,3)))
                    delta_residual_max = list(argrelextrema(np.array(data6[:]),np.greater)[0])
                    delta_residual_min = list(argrelextrema(np.array(data6[:]),np.less)[0])
                    ...
                    tmp_result = self.generate_imf_extrem(residual,"max",max_min[-1])
                    last_trust_min = imf_min_true_index[-1]

                    if (tmp_result and tmp_result!=-1):
                        for i in range(len(tmp_result)):
                            if i%2 == 0:
                                imf_max_true_index.append(tmp_result[i]+last_trust_min)
                            else:
                                imf_min_true_index.append(tmp_result[i]+last_trust_min)         
...
#                        too_far = 1
                 ...
                 #   if imf_max_remove_index != []:
                 #       if last_max>imf_max_remove_index[-1]:
                 #           last_max = imf_max_remove_index[-1]
                 #   if imf_min_remove_index != []:
                 #       if last_min>imf_min_remove_index[-1]:
                 #           last_min = imf_min_remove_index[-1]
                    ...
                    if  (( last_max>last_min or (last_min>last_max and last_min>=496) ) and((499-last_max>(mean_max_min-3) and 499-last_max<(mean_max_min+2*std_max_min)) )) :#or (last_min>last_max and last_min!=0 and 499-last_min<=3): #or (last_min>last_max and last_min!=0 and 499-last_min<=mean_max_min+mean_min_max+2 and 499-last_min>=mean_min_max+mean_max_min-2):
          ...
          #                  estimate_max = price_last_max_index[(argrelextrema(restruct_price_max,np.greater)[0])[0]]
            ...
            #            imf_extrem_decision = -1
             ...
             #       residual = price_min_data
                    ...
                    tmp_result = self.generate_imf_extrem(residual,"min",min_max[-1])
                    last_trust_max = imf_max_true_index[-1]

                    if (tmp_result and tmp_result!=-1):
                        for i in range(len(tmp_result)):
                            if i%2 == 0:
                                imf_min_true_index.append(tmp_result[i]+last_trust_max)
                            else:
                                imf_max_true_index.append(tmp_result[i]+last_trust_max)
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif imf_std_decision == 1 and too_far == 0 and  ( imf_extrem_decision5 == -1) and distance_decision == 1 and self.buy_decision == -1 :#and self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
        ...
        if too_far == 0 and  ( imf_extrem_decision5 == -1) and imf_std_decision == 1:
            self.buy_decision = -1
        else:
            self.buy_decision = 0
