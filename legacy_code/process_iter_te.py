...
#            macd_decision1 = 1
        ...
        if self.macd[current_price_index] < 0 and self.macd[current_price_index-1]>self.macd[current_price_index-2] and self.macd[current_price_index]>self.macd[current_price_index-1]:
            macd_decision2 = 0
        #elif self.macd[current_price_index] > 0 and self.macd[current_price_index]>self.macd[current_price_index-1]:
...
#        elif self.macd[current_price_index] > 0 and ((self.macd[current_price_index-1]-self.macd[current_price_index-2])>(self.macd[current_price_index]-self.macd[current_price_index-1])):
        ...
        delta_list = [delta_ma_5,delta_ma_10,delta_ma_20]
        ...
        num_po = len([i for i in delta_delta_list if i > 0])
        num_ne = len([i for i in delta_delta_list if i < 0])
        print "deltadelta num po %s"%num_po
        print "deltadelta num ne %s"%num_ne
        if num_ne<3:
...
#            self.buy_macd = 0
        once_flag = 0
        if self.buy_price!=0 and ((macd_decision2 == 1 and imf_power_decision == 1  ) or ((trade_price-self.buy_price)/self.buy_price)>0.03 )and self.share > 0 :
            print "sell 1"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            ...
            self.holdtime = 0
            once_flag = 1
        if self.buy_price!=0 and (((self.hold_time > 2 ) and ((trade_price-self.buy_price)/self.buy_price)>0.015) or self.hold_time == 10  )and self.share > 0 :
            print "sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            if self.buy_price>trade_price:
...
#            tomorrow_will = -1
        elif delta_delta_ma_decision1 == 1 and (self.last_power_decision!=1 or self.last_imf_power_decision != 1 )and std_decision1 == 1 and once_flag == 0 and ((self.vol[current_price_index-1]-self.vol[current_price_index])/self.vol[current_price_index-1])<0.5 and delta_price_decision1 == 1 and delta_ma_decision1 == 1 and (imf_power_decision == -1 or power_decision == -1 )and  self.money>0  :#and imf[-1]<self.ma(imf, 10, 0, current_index):#and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 10, 0, current_price_index)) :#imf[-1]>imf[-2] and current_price<self.ma(self.close_price, 5, 0, current_price_index) and self.vol[current_price_index]>self.vol[current_price_index-1] and ((trade_price-current_price)/current_price)<0.05 and ( now_position_last_min_delta == 1 or now_position_last_min_delta == 1) and  self.money>0 and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0  and self.money>0:
            if self.fail_flag == 0:
                print "buy buy 1"
                self.share = self.buy(self.money,trade_price)
                self.money = 0
...
#            tomorrow_will = -1
        ...
        self.last_power_decision = power_decision
        self.last_imf = imf[-1]
...
if __name__ == "__main__":
    ...
    print "begin %s"%begin
    process.generate_emd_data_fix(close_price,begin,60)
