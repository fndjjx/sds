...
#            (imf_process,extenflag1,ex_num) = matchlist3(imf_process)
        ...
        if imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>len(imf_process)-5 and ex_flag==1:#and imf_process[-1]<0 :
            imf2_flag=1
...
###########
        ...
        if i==1 :#or tmp[-2]<990:
            (imf_process,extenflag1,ex_num,cha) = matchlist1(imf_list[1])
        elif i==2:
            (imf_process,extenflag1,ex_num,cha) = matchlist2(imf_list[1])
        else:
            (imf_process,extenflag1,ex_num,cha) = matchlist3(imf_list[1])
...
##########
        decision=0
        if (imf2_flag==1 and imf1_flag==1 and imf2_or_flag!=2) or (imf2_or_flag==1) :#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
#        if (imf2_flag==1 and self.open_flag==1) or (imf2_flag==1 and imf2_macd_flag==1) or (imf2_macd_flag==1 and self.close_flag==1) or (imf2_macd_flag==1 and imf2_flag==1):#and imf2_macd_flag==1:#and power_de==1:#and imf3_flag==1:
            decision=1 
        elif (imf2_flag==1  and imf2_or_flag!=2) or (imf2_or_flag==1) :#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            decision=3 
...
#######
        ...
        elif  ( (imf_flag==1 ) or (imf_flag==3 and deltama5>deltama5_before)) and  self.buy_price==0 and distance_decision==1:#and deltama3_before<deltama3:# and distance_decision==1 :# and self.close_price[current_price_index]>self.close_price[current_price_index-1]:#deltama3_before<deltama3:#self.macd[current_price_index-1]<self.macd[current_price_index] :#and (self.macd[current_price_index-2]<0 or self.macd[current_price_index-1]<0 or self.macd[current_price_index]<0):#and deltama5>=0 :#and deltama3_before<deltama3 ) or (deltama3>0 and deltama3_before<0)):# and  mama3>mama3_b and mama3_b<mama3_b2 and distance_decision==1:#and deltama5<0:#and self.close_price[current_price_index]>self.close_price[current_price_index-1] and deltama5_before<deltama5<0: #and deltama3_before<0 and deltama3>0 and deltama5_before<deltama5<0:#and ma_decision==1 :#and distance_decision==1:#deltama3_before<deltama3:# (deltama3_before>deltama3 or deltama3<0):
