...
##############################################################
        ...
        if imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>195 and imf_min_index[-1]-imf_max_index[-1]>1:
            imf_flag=1
        if imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>=90:
            imf_flag=2
...
###########################
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,residual,emd_data):
        ...
        self.snr.append(calc_SNR(emd_data,imf_list)[0])
...
####################
        signal_index=calc_SNR(emd_data,imf_list)[1]
        if signal_index>len(imf_list)-1:
            tmp=residual
        else:
            tmp=imf_list[signal_index]
            for i in range(signal_index+1,len(imf_list)):
                tmp = [tmp[j]+imf_list[i][j] for j in range(len(tmp))]

            tmp = [tmp[j]+residual[j] for j in range(len(tmp))]


        tmp_max_index = list(argrelextrema(np.array(tmp),np.greater)[0])
        tmp_min_index = list(argrelextrema(np.array(tmp),np.less)[0])
        print "tmp max index%s"%tmp_max_index
        print "tmp min index%s"%tmp_min_index
...
#######
        ...
        snr_decision=0
        if np.mean(self.snr[-5:])>np.mean(self.snr[-10:]) :#and np.mean(self.snr[-3:])>0:
        ...
        print "snr %s"%np.mean(self.snr[-1])
        ...
        #if self.buy_price!=0  and deltama5_before>deltama5 and deltama3_before>deltama3 and ((current_price-self.buy_price)/self.buy_price) >0 and self.buy_price!=0 :
        if self.buy_price!=0 and (deltama3_before>deltama3) and distance_decision2==1 :#and current_price-self.buy_price>0:#emd_data[-1]<emd_data[-2] and emd_data[-2]>emd_data[-3] :#mama3<mama3_b and mama3_b>mama3_b2 and current_price-self.buy_price>0 and self.buy_price!=0:
            print "sell sell 1"
...
##################
        svm_train(traindata,10)

####
        
        pp=[]
        testdata1 = traindata[-10:]
        print "testdata1"
        print testdata1
        pp.append(float(svm_predict(testdata1)))
        testdata2 = traindata[-9:]
...
if __name__ == "__main__":
    ...
    generate_emd_func = generateEMDdata(datafile,begin,200,emd_dir)
