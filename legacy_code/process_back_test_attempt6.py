...
##
        ...
        mamp=ma_func2(mp_list,3)

        dd = ma3[current_price_index-99:current_price_index+1]
        print dd
        dd2 = mamp[current_price_index-99:current_price_index+1]
        ...
        if len(self.feature)==len(self.lable) and len(self.feature)>0:
            clf.fit(self.feature,self.lable) 
            print clf.predict([self.x[-1]])[0]
        print self.feature
        print self.lable
##
        sel=1
        if self.buy_price==0 :#and len(self.feature)==len(self.lable) and len(self.feature)>0:#and len(self.mpup)>5 and self.mpup[-1]>self.mpup[-5]:
        #    if calc_down(mp_list[current_price_index-20:current_price_index+1])<1 and calc_up(mp_list[current_price_index-20:current_price_index+1])>0.5:#0.7<p_c/n_c<1.3 :#and ks_p_value>0.5:#or (np.mean(diffmp[-5:])>0 and stab_de!=1):#self.pimf1[-1][1]>0.4:#self.cu[-1]>0.6 or (len(self.cu)>1 and self.cu[-1]>self.cu[-2]):
            if  self.x[-1]>0.5:#-np.std(self.x):#(np.std(speed_c)-np.std(speed_m))/np.std(speed_c)>0.1  :#self.sel==2 and self.fail_flag==3:#cpstd-mpstd/cpstd>0.00 :#and not (sharp(s21)<0 and sharp(s22)<0):#len(ma3s1)>1 and ma3s1[-1]>ma3s1[-2]:#np.mean(s21)/np.std(s21)>np.mean(s22)/np.std(s22) and np.mean(s21)/np.std(s21)>0 and np.mean(s21)/np.std(s21)>np.mean(s11)/np.std(s11):#sum(s1)>sum(s2) and sum(s1)>0:#deltama10<0 :#and  s2[-1]>0:#self.mpdown[-1]<mampdown[-1] :#and lp2!=1:
                sel=1
             #   self.fail_flag=0
            #elif p_c/n_c>1.3:
...
##3
        ...
        max_index=list(argrelextrema(np.array(diffmp),np.greater)[0])
        min_index=list(argrelextrema(np.array(diffmp),np.less)[0])
        datac = self.close_price[current_price_index-100:current_price_index+1]
        maxde=0
        if np.mean(self.je[current_price_index-2:current_price_index+1])>np.mean(self.je[current_price_index-4:current_price_index+1]):#datac[min_index[-1]]>datac[min_index[-2]]: 
            maxde=1
        jede=0
        sel=1
        if sel==1:
            if  diffmp[-1]<0 and  mean_de==1 :#and gmmde==1 and   len(self.mclist2)>0 and self.mclist2[-1]>0 :#and maxde==1:#and (( self.fail_flag==0) or (self.fail_flag==1 and gmmde2==1)):
                jede=1
            if diffmp[-1]>0 and mean_de==2:#self.close_price[current_price_index]<self.close_price[current_price_index-1]:# mean_de==2 :#and deltama10>0) or (deltama10<0 and diffmp[-1]>0):
                jede=2
        elif 0:#sel==2:
...
##
        def calc_shift(bl,sl):
            r={}
            r[0] = sum([abs(bl[i]-sl[i]) for i in range(len(bl))]) 
            print bl
            print sl
            for i in range(1,5):
                bt = bl[:-i]
                ...
                r[i] = sum([abs(bt[j]-st[j]) for j in range(len(bt))])

            n=sorted(r.items(),key=lambda d:d[1])
            print n
            return n[0][0]
                
                

        #madatac=ma_func2(self.close_price,2)
        madatac=self.close_price
        #mamp=ma_func2(mp_list,2)
        mamp=mp_list
        datac_r = madatac[current_price_index-19:current_price_index+1]
        mp_r = mamp[current_price_index-19:current_price_index+1]
        print calc_shift(datac_r,mp_r)
        min_index=argrelextrema(np.array(datac_r),np.less)[0]
        max_index=argrelextrema(np.array(datac_r),np.greater)[0]
        min_index2=argrelextrema(np.array(mp_r),np.less)[0]
        max_index2=argrelextrema(np.array(mp_r),np.greater)[0]
        ...
        try:
            for i in range(1,len(min_index)-1):
                d1 = abs(min_index[i]-min_index2[i])
                d2 = abs(min_index[i]-min_index2[i-1])
                d3 = abs(min_index[i]-min_index2[i+1])
                if min(d1,d2,d3) == d1:
                    self.x2.append(min_index2[i]-min_index[i])
                elif min(d1,d2,d3) == d2:
                    self.x2.append(min_index2[i-1]-min_index[i])
                else:
                    self.x2.append(min_index2[i+1]-min_index[i])
        except:
            self.x2.append(0)

        print "selfx2 {}".format(self.x2[-10:])
        print np.mean(self.x2[-10:])

        try:
            for i in range(1,len(max_index)-1):
                d1 = abs(max_index[i]-min_index2[i])
                d2 = abs(max_index[i]-min_index2[i-1])
                d3 = abs(max_index[i]-min_index2[i+1])
                if min(d1,d2,d3) == d1:
                    self.x2.append(min_index2[i]-max_index[i])
                elif min(d1,d2,d3) == d2:
                    self.x2.append(min_index2[i-1]-max_index[i])
                else:
                    self.x2.append(min_index2[i+1]-max_index[i])
        except:
            self.x3.append(0)

        print "selfx3 {}".format(self.x3[-10:])
        print np.mean(self.x3[-30:])
###
