...
#        elif self.macd[current_price_index] > 0 and ((self.macd[current_price_index-1]-self.macd[current_price_index-2])>(self.macd[current_price_index]-self.macd[current_price_index-1])):
        ...
        if (2*np.std(std_list_20)/self.ma(self.close_price, 20, 0, current_price_index))>0.04:
...
#            self.down_macd_counter = 0
#            self.buy_macd = 0
        if self.buy_price!=0 and (imf_power_decision == 1  or ((trade_price-self.buy_price)/self.buy_price)>0.03 )and self.share > 0 :
            print "sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
...
#            self.buy_macd = 0
#            tomorrow_will = 1
        elif  macd_decision2 == 1 and float(self.buy_price)!=0 and (float(self.buy_price)-float(current_price))/float(self.buy_price)>0.01 and self.share>0:
            print "sell 4"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
...
#            tomorrow_will = -1
        elif delta_price_decision1 == 1 and delta_ma_decision1 == 1 and imf_power_decision == -1 and  self.money>0 and residual[-2]<residual[-1] :#and imf[-1]<self.ma(imf, 10, 0, current_index):#and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 10, 0, current_price_index)) :#imf[-1]>imf[-2] and current_price<self.ma(self.close_price, 5, 0, current_price_index) and self.vol[current_price_index]>self.vol[current_price_index-1] and ((trade_price-current_price)/current_price)<0.05 and ( now_position_last_min_delta == 1 or now_position_last_min_delta == 1) and  self.money>0 and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0  and self.money>0:
            if self.fail_flag == 0:
                print "buy buy 1"
                self.share = self.buy(self.money,trade_price)
                self.money = 0
