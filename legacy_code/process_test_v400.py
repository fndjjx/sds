...
#                count = 10
   ...
   #         return -1
    ...
    def double_filter(self,data):
        ...
        a=data
        c = splinerestruct2(a,a,np.linspace(0,len(a)-1,len(a)),3,100)
        b=c
        #c = splinerestruct2(b,b,np.linspace(0,len(b)-1,len(b)),1,1000)
        #print "coef %s"%np.corrcoef(b,c)[0][1]
        ##if np.corrcoef(b,c)[0][1]<0.85:
        #d = [b[i]-c[i] for i in range(len(c))]
        #b=d
        #    print "coef %s"%np.corrcoef(b,c)[0][1]
        #print "data coef %s"%np.corrcoef(b,data)[0][1]
...
##############################################################
        ...
        print "coef %s"%np.corrcoef(imf_process,price_list)[0][1]
...
#            min_diff = abs(after_re[-1]-after_re[vol_min_index[-1]])
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                    ...
                    last_dis = abs(imf_max_true_index[-1]-imf_min_true_index[-1])
                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    if max_index != []:
                        ...
                        print len(data)-max_index[-1]
                        print tmp[0]
                        print tmp[-1]
                        if (tmp[0] in max_index) and (tmp[-1] in max_index) and len(data)-max_index[-1]>(mean_max_min)-std_max_min:#std_max_min and len(data)-max_index[-1]<dis+std_max_min:#+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                    ...
                    if min_index != []:
                        ...
                        if (tmp[0] in max_index) and (tmp[-1] in min_index) and len(data)-min_index[-1]<5 and self.last_min_index == [] and self.last_max_index==[] and min_index!=[]:
                            imf_extrem_decision6 = -1
                        if (tmp[0] in max_index) and (tmp[-1] in min_index) and len(data)-min_index[-1]<4 :#and self.last_min_index == [] and self.last_max_index==[] and min_index!=[]:
                            imf_extrem_decision6 = -1
                    print "imf deicison 6 %s"%imf_extrem_decision6
                    if len(tmp)>3 or too_far==1:
                        imf_extrem_decision6 = 0
                    if ((data[0]-data[-1])/data[0])>0.15:
                        imf_extrem_decision6 = 0
                    if tmp!=[]:
                        if tmp[-1] in min_index:
                            imf_extrem_decision8 = 0
                    ...
                    self.last_max_index.append(max_index)
                    self.last_min_index.append(min_index)
            ...
            #            imf_extrem_decision = -1
             ...
             #           residual = price_min_data
                    ...
                    if max_index != []:
                        ...
                        if (tmp[0] in min_index) and (tmp[-1] in max_index) and len(data)-max_index[-1]>(mean_max_min)-std_max_min:# and len(data)-max_index[-1]<dis+std_max_min :#+2*std_max_min :#and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = -1
                    if min_index != []:
                        #if (tmp[0] in min_index) and  tmp[0]>4 and (tmp[-1] in min_index) and len(data)-min_index[-1]<6:
                        print len(data)-min_index[-1]
                        if (tmp[0] in min_index) and (tmp[-1] in min_index) and len(data)-min_index[-1]<5 and self.last_min_index == [] and self.last_max_index==[] and min_index!=[]:
                            imf_extrem_decision6 = -1
                        if (tmp[0] in min_index) and (tmp[-1] in min_index) and len(data)-min_index[-1]<4 :#and self.last_min_index == [] and self.last_max_index==[] and min_index!=[]:
                            imf_extrem_decision6 = -1
                    print "imf deicison 6 %s"%imf_extrem_decision6
                    print "last min %s"%self.last_min_index
                    print " min %s"%min_index 
        ...
        change_decision1 = 0
        print "min index%s"%self.last_min_index
        print "max index%s"%self.last_max_index
        if len(self.last_max_index)>8 and len(self.last_min_index)>8:
            if self.last_min_index[-7]==[]  and self.last_min_index[-6]==[] and self.last_min_index[-5]==[] and self.last_min_index[-4]==[] and self.last_min_index[-3]==[] and self.last_min_index[-2]==[] and self.last_min_index[-1]!=[]:
                change_decision1 = -1
            elif self.last_max_index[-4]==[] and self.last_max_index[-3]==[] and self.last_max_index[-2]==[] and self.last_max_index[-1]!=[]:
                change_decision1 = 1
...
#####################################################################################################################
        ...
        ma_decision2 = 0
        delta_ma = [deltama5,deltama10,deltama20,deltama30]
        count=0
        for i in delta_ma:
            if i>0:
                count+=1
        ...
        macd_decision1 = 0
        if self.macd[current_price_index]<0 and self.macd[current_price_index-1]>self.macd[current_price_index]:# and self.macd[current_price_index-2]>self.macd[current_price_index-1] and self.macd[current_price_index-1]>self.macd[current_price_index]:
            macd_decision1 = 1
        ...
        if change_decision1==-1 :#up_decision1 == 1  and imf_extrem_decision6 == -1:#  (current_price-ma5) > (self.close_price[current_price_index-1]-self.ma(self.close_price, 5, -1, current_price_index)) and ( imf_extrem_decision6 == -1):
            self.buy_decision = -1
...
#            buy_decision2 = -1
        ...
        if self.buy_price!=0 and change_decision1==1 :#distance_decision2 == 1 and (imf_extrem_decision == 1 ) and  ((trade_price-self.buy_price)/self.buy_price) >0 and self.share > 0 :
            print "sell sell 1"
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif  self.buy_price==0 and ma_decision2==1 and change_decision1 == -1 :#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
