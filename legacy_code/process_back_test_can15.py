...
#        last_max=1
        ...
        if (imf_close_flag==1) :#or (imf_high_flag==1 and imf_close_flag==1 and deltama5<deltama5_before):#or (len(self.high_flag)>0 and (self.high_flag[-1]==1))):# and (self.high_flag[-2]!=2) ) and imf_high_flag!=2:#or ( len(self.close_flag)>1 and self.close_flag[-1]!=2 and self.close_flag[-2]==1) or ( len(self.close_flag)>2 and self.close_flag[-3]==1 and self.close_flag[-1]!=2 and self.close_flag[-2]!=2) or (len(self.close_flag)>3 and (self.close_flag[-4]==1) and self.close_flag[-3]!=2 and self.close_flag[-2]!=2 and self.close_flag[-1]!=2)) :#and ((len(self.high_flag)>0 and self.high_flag[-1]==1) or imf_high_flag==1):
...
#
        ...
        com_de=1
        if ma_max_index[-1]>ma_min_index[-1] and ma_max_index[-1]>56:
            com_de=1
#######
...
######
        ...
        if self.buy_price!=0:
            self.each_day+=1
        if  self.buy_price!=0  and (boll_de==2 or ((imf_flag==2) and deltama3_before<0 and deltama3<0))  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
...
#
        ...
        tmp=ma_max_index+ma_min_index
        tmp.sort()
#        tmp.insert(0,0)
        for kkk in range(1,len(tmp)):
            print (tmp[-kkk])
            if i-tmp[-kkk]>200:
                break
        flag=tmp[-kkk]
        ...
        diff_tmp=np.diff(tmp)
        
        sel=1
        #if ma_max_index[-1]>ma_min_index[-1] and ma_max_index[-1]>496:#and len(ma5)-ma_min_index[-1]<=5 :#and ma_min_index[-1]-ma_max_index[-1]>=2:#and abs(ma_max_index[-1]-last_max)<2:
        #    ma_ex_de=2
#        if small_num>0 and ((large_num/float(small_num))>3):# or ((large_num/float(small_num))<2)):
        print "diff tmp%s %s"%(np.mean(diff_tmp),1.5*np.std(diff_tmp))
        print "diff tmp%s %s"%(np.mean(diff_tmp),np.std(diff_tmp))
