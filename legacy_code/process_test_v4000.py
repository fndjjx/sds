...
##############################################################
        ...
        price_recent1 = self.close_price[current_price_index-4:current_price_index+1]
        for i in range(len(imf_list)):
            coef_list1.append(np.corrcoef(price_recent1,imf_list[i][-5:])[0][1])
            print "imf%s data coef %s"%(i,np.corrcoef(price_recent1,imf_list[i][-5:])[0][1])

        print "imf1 imf2 coef %s"%np.corrcoef(imf_list[1][-10:],imf_list[2][-10:])[0][1]
        price_recent2 = self.close_price[current_price_index-9:current_price_index+1]
        for i in range(len(imf_list)):
            coef_list2.append(np.corrcoef(price_recent2,imf_list[i][-10:])[0][1])
            print "imf%s data coef %s"%(i,np.corrcoef(price_recent2,imf_list[i][-10:])[0][1])

        print "imf1 imf2 coef %s"%np.corrcoef(imf_list[1][-10:],imf_list[2][-10:])[0][1]


        coef_decision2 = 1
        if np.corrcoef(imf_list[1][-10:],imf_list[2][-10:])[0][1]<-0.2:
            coef_decision2 = 0



        price_recent3 = self.close_price[current_price_index-19:current_price_index+1]
        for i in range(len(imf_list)):
            coef_list3.append(np.corrcoef(price_recent3,imf_list[i][-20:])[0][1])
            print "imf%s data coef %s"%(i,np.corrcoef(price_recent3,imf_list[i][-20:])[0][1])


        coef_decision1 = 0
        if coef_list1[1]>0.65 and coef_list1[2]>0 and coef_list1[1]>coef_list1[2]:#and coef_list1[2]<coef_list1[1]:
            coef_decision1 = 1
        elif coef_list1[2]>0.65 :#and coef_list1[2]>coef_list1[1] :
            coef_decision1 = 2
...
##########################
        ...
        print "small small imf max index %s"%small_small_imf_max_index
        small_small_imf_min_index = list(argrelextrema(small_small_data,np.less)[0])
        print "small small imf min index %s"%small_small_imf_min_index



###########################

        four_data = np.array(four_imf)
        four_imf_max_index = list(argrelextrema(four_data,np.greater)[0])
        print "\n\n"
        print "four imf max index %s"%four_imf_max_index
        four_imf_min_index = list(argrelextrema(four_data,np.less)[0])
        print "four imf min index %s"%four_imf_min_index
...
#######################
        ...
        residual_data = np.array(residual)
        residual_max_index = list(argrelextrema(residual,np.greater)[0])
...
#            min_diff = abs(after_re[-1]-after_re[vol_min_index[-1]])
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    small_index_max = list(filter(lambda n:n>imf_min_true_index[-1]+2, small_imf_max_index))
                    ...
                    print "haha"
                    max_pair = [(i,price_list[i]) for i in small_index_max]
                    print max_pair
                    max_value = [i[1] for i in max_pair]
                    print max_value
                    max_index=[]
                    min_index=[]
                    ...
                    if len(small_index_max)>0:
                        if (max(max_value)-min(max_value))/min(max_value)>=0.08:
                            for i in max_pair:
                                if max(max_value)==i[1]:
                                    max_index.append(i[0])
                        print max_index
                        if len(small_index_max)>=2 and (max(max_value)-min(max_value))/min(max_value)<0.08:
                            max_index.append(sum(small_index_max[:2])/len(small_index_max[:2]))
                        elif len(small_index_max)<2 and (max(max_value)-min(max_value))/min(max_value)<0.08:
                            print "small index max%s"%small_index_max
                            max_index.append((sum(small_index_max)+499)/((len(small_index_max))+1))
                        small_index_min = list(filter(lambda n:n>max_index[-1]+2, small_imf_min_index))
                        if len(small_index_min)>0 and small_index_min[-1]<498:
                            min_index.append(sum(small_index_min)/len(small_index_min))

                    #if len(max_index)>0 and max_index[0]-imf_min_true_index[-1]<4:
                    #    min_index = []
                    #    max_index = []
                    if  1:#coef_decision1==2 and self.buy_price!=0:
                        print "enter coef decision2"
                        max_index2=imf_max_remove_index
                        min_index2=imf_min_remove_index
                        max_index2 = list(filter(lambda n:n<496, max_index2))
                        min_index2 = list(filter(lambda n:n<496, min_index2))

                    if max_index!=[] and max_index2!=[]:
                        tmp = (max_index[0]+max_index2[0])/2
                        max_index=[]
                        max_index.append(tmp)
                    ...
                    if tmp!=[] and max_index != []:
                        if (tmp[0] in max_index) and tmp[0]>4 and (tmp[-1] in max_index) and 499-max_index[-1]>mean_max_min-1 and 499-max_index[-1]<mean_max_min+std_max_min:#+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
            ...
            #            imf_extrem_decision = -1
             ...
             #           residual = price_min_data
                    ...
                    small_index_min = filter(lambda n:n>imf_max_true_index[-1]+2, small_imf_min_index)
                    ...
                    print "haha"
                    max_pair = [(i,price_list[i]) for i in small_index_max]
                    min_pair = [(i,price_list[i]) for i in small_index_min]
                    print max_pair
                    max_value = [i[1] for i in max_pair]
                    min_value = [i[1] for i in min_pair]
                    print max_value
                    print min_value
                    max_index=[]
                    min_index=[]
                    ...
                    if len(small_index_min)>0:
                        if (max(min_value)-min(min_value))/min(min_value)>=0.08:
                            for i in min_pair:
                                if min(min_value)==i[1]:
                                    min_index.append(i[0])
                        print min_index
                        if len(small_index_min)>2 and (max(min_value)-min(min_value))/min(min_value)<0.08:
                            min_index.append(sum(small_index_min[:2])/len(small_index_min[:2]))
                        elif len(small_index_min)<=2 and (max(min_value)-min(min_value))/min(min_value)<0.08:
                            min_index.append(sum(small_index_min)/len(small_index_min))
                        small_index_max = list(filter(lambda n:n>min_index[-1]+2, small_imf_max_index))
                        if len(small_index_max)>0 and small_index_max[-1]<498:
                            max_index.append(sum(small_index_max)/len(small_index_max))
                    ...
                    print min_index2
                    if min_index!=[] and min_index2!=[]:
                        tmp=[]
                        tmp.append((min_index[0]+min_index2[0])/2)
                        min_index = tmp
                    ...
                    if max_index != []:
                        ...
                        if (tmp[-1] in max_index) and 499-max_index[-1]>(mean_max_min)-1.5 and 499-max_index[-1]<mean_max_min+std_max_min :#+2*std_max_min :#and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = -1
                    if min_index != []:
                        #if (tmp[0] in min_index) and  tmp[0]>4 and (tmp[-1] in min_index) and len(data)-min_index[-1]<6:
                        print len(data)-min_index[-1]
                        if (tmp[-1] in min_index) and 499-min_index[-1]<4 and self.last_min_index == [] and self.last_max_index==[] and min_index!=[]:
                            imf_extrem_decision6 = -1
                    if min_index != []:
                        #if (tmp[0] in max_index) and tmp[0]>4 and (tmp[-1] in min_index)  and len(data)-min_index[-1]<6:
                        print len(data)-min_index[-1]
...
##################
        fall_decision1 = 1
        last_four_price = self.close_price[current_price_index-3:current_price_index+1]
        chazhi_last_four = [(last_four_price[i+1]-last_four_price[i])/last_four_price[i+1] for i in range(len(last_four_price)-1)]
        neg_chazhi = [i for i in chazhi_last_four if i<0]
        print "chazhi for four %s"%chazhi_last_four
        if min(chazhi_last_four)<-0.03 or len(neg_chazhi)==3:
            fall_decision1 = 0
...
#####################################################################################################################
        ...
        macd_decision1 = 1
        if (macd_list[-1]<0 and macd_list[-2]>0) or (macd_list[-2]<0 and macd_list[-3]>0) or (macd_list[-3]<0 and macd_list[-4]>0):
            macd_decision1 = 0
        ...
        macd_decision2 = 1
        if self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]<self.macd[current_price_index-3]:
            macd_decision2 =0 
        if self.ma_count<0 and self.ma_count2<0 and self.ma_count<self.ma_count2:
            not_do = 1
        if self.ma_count>0 and self.ma_count2<0 and abs(self.ma_count2)<10:
            not_do = 1

        if self.ma_count<0 and self.ma_count2>0 and self.ma_count2>15:
            do = 0
        if current_price>ma5 and self.ma_count>0 and self.ma_count2<0 :#and abs(self.ma_count2)>15:
            do = 1
        if self.ma_count>0 and self.ma_count2>0 and self.ma_count-self.ma_count2>=4:
            do = 1
        ...
        if self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((current_price-self.buy_price)/self.buy_price) >0.02:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            ...
            self.holdtime = 0
            once_flag = 1
        elif  self.buy_price!=0 and (((self.buy_price-trade_price)/self.buy_price)>0.02 )  and imf_extrem_decision6!=-1 and self.share > 0 :
            print "sell sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
            ...
            self.holdtime = 0
            once_flag = 1
        elif  self.buy_price!=0 and (((current_price-self.buy_price)/self.buy_price)>0.03 )   and self.share > 0 :
            print "sell sell 3"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif do==1 and coef_decision1>0  and  self.buy_price==0 and   imf_extrem_decision6==-1  and  up_decision1 == 1 :#and ma10>ma30:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
