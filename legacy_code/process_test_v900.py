...
class process_util():
    ...
    def __init__(self,open_price,close_price,macd,date,vol):
        ...
        self.last_extrem_flag=0
        self.last_small_extrem = 0
...
#                count = 10
   ...
   #         return -1
    ...
    def spline_filter(self,data,up_potion):

        b=splinerestruct2(data,data,np.linspace(0,len(data)-1,len(data)),2,100)
        if abs(up_potion)<1.5 and abs(up_potion)>0.6:
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,train_data_dir,train_number,hidden_number,result_number,residual):
        ...
        for i in range(len(imf_list)):
            coef_list.append(np.corrcoef(price_recent,imf_list[i][-20:])[0][1])
        ...
        coef_decision1 = 0
        if coef_list[2]>coef_list[1] :#and coef_list[2]>coef_list[0]:
            coef_decision1 = 1
        elif coef_list[2]<coef_list[1]:
            coef_decision1 = 2
...
##############################################################
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            imf_small_index = small_imf_max_index+small_imf_min_index
            imf_small_index.sort()

            imf_small_decision1=0
            if imf_small_index[-1] in small_imf_max_index:
                if self.last_small_extrem == "min":
                    imf_small_decision1=1
                self.last_small_extrem="max"
            else:
                self.last_small_extrem="min"

            last_50_price = self.close_price[current_price_index-50:current_price_index+1]
            ...
            imf_max_true_index = list(filter(lambda n:n<=490,imf_max_index))
            imf_min_true_index = list(filter(lambda n:n<=490,imf_min_index))
            ...
            imf_true_index = imf_max_true_index+imf_min_true_index
            imf_true_index.sort()
            cha_decision=1
            chazhi = [imf_true_index[i+1]-imf_true_index[i] for i in range(len(imf_true_index)-1)]
            print "chazhi %s"%np.mean(chazhi[-3:])
            if np.mean(chazhi[-3:])<=5:
                cha_decision=0
...
#            min_diff = abs(after_re[-1]-after_re[vol_min_index[-1]])
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                    ...
                    if 499-imf_min_true_index[-1]>mean_min_max-1 and 499-imf_min_true_index[-1]<mean_min_max+1:
                        imf_extrem_decision9 = 1
                    print "imf_extrem_decision9 %s"%imf_extrem_decision9


                    self.last_extrem_flag = "min"
...
#############################
                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    #(max_index,min_index)=self.spline_filter(data,up_potion)
                    ...
                    print tmp
                    if max_index != []:
                        print (tmp[0] in max_index)
                        if min_index!=[]:
                            dis = tmp[-1]-tmp[-2]
                        else:
                            dis = tmp[-1]
                        #if (tmp[0] in max_index) and tmp[0]>4 and (tmp[-1] in max_index) and len(data)-max_index[-1]>mean_max_min+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                        if (tmp[-1] in max_index) and len(data)-max_index[-1]>(mean_max_min)-std_max_min:#std_max_min and len(data)-max_index[-1]<dis+std_max_min:#+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = 0
                    if min_index != []:
                        #if (tmp[0] in max_index) and tmp[0]>4 and (tmp[-1] in min_index)  and len(data)-min_index[-1]<6:
                        print len(data)-min_index[-1]
                        if (tmp[-1] in min_index) and len(data)-min_index[-1]<4 and self.last_min_index == [] and self.last_max_index==[] and min_index!=[]:
                            imf_extrem_decision6 = 0
                    ...
                    if imf_extrem_decision6==-1:
                        estimate_max = [i+imf_min_true_index[-1] for i in max_index]
                        estimate_min = [i+imf_min_true_index[-1] for i in min_index]
                        new_imf_max_true_index = imf_max_true_index+estimate_max
                        new_imf_min_true_index = imf_min_true_index+estimate_min

                    print "new max %s"%new_imf_max_true_index
                    print "new min %s"%new_imf_min_true_index
...
################
                    ...
                    price_max_min_list = self.close_price[current_price_index-499+imf_min_true_index[-1]:current_price_index-499+imf_max_true_index[-1]+1]
                    price_max_min_list2 = self.close_price[current_price_index-499+imf_min_true_index[-1]:current_price_index-499+imf_max_true_index[-1]+1]
                    price_min_max_list = self.close_price[current_price_index-499+imf_max_true_index[-1]:current_price_index+1]
                    price_min_max_list2 = self.close_price[current_price_index-499+imf_max_true_index[-1]:current_price_index+1]
                    imf_max_min_list = imf_process[imf_min_true_index[-1]:imf_max_true_index[-1]+1]
                    


                    length = min(len(price_max_min_list2),len(price_min_max_list2))
                    if len(price_max_min_list2)>len(price_min_max_list2):
                        for i in range(len(price_max_min_list2)-length):
                            price_min_max_list2.append(price_min_max_list2[-1])
                    else:
                        for i in range(len(price_min_max_list2)-length):
                            price_max_min_list2.append(price_max_min_list2[-1])

                    
                    print price_min_max_list2
                    print price_max_min_list2
                    print imf_max_min_list
                    print 499-imf_max_true_index[-1]
             #       if np.corrcoef(price_max_min_list,imf_max_min_list)[0][1]>0.6 and np.corrcoef(price_max_min_list2,price_min_max_list2)[0][1]<-0.1 and 499-imf_max_true_index[-1]>mean_max_min-1 and 499-imf_max_true_index[-1]<mean_max_min+2:
                    if np.corrcoef(price_max_min_list,imf_max_min_list)[0][1]>0.6  and 499-imf_max_true_index[-1]>mean_max_min-1 and 499-imf_max_true_index[-1]<mean_max_min+2:
                    #if 499-imf_max_true_index[-1]>mean_max_min-1 and 499-imf_max_true_index[-1]<mean_max_min+2*std_max_min:
                        imf_extrem_decision9 = -1
                    print price_max_min_list
                    print price_min_max_list
                    print imf_max_min_list
                    print "price imf coef %s"%np.corrcoef(price_max_min_list,imf_max_min_list)[0][1]
                    print "price price coef %s"%np.corrcoef(price_max_min_list2,price_min_max_list2)[0][1]
                    print "imf_extrem_decision9 %s"%imf_extrem_decision9
                    

                    if self.buy_price!=0 and self.last_extrem_flag=="min":
                        imf_extrem_decision9 = 1
...
#######################
                    ...
                    if max_index != []:
                        if min_index!=[]:
                            ...
                            dis = imf_max_true_index[-1]-imf_min_true_index[-1]
                        else:
                            dis = tmp[-1]
                        ...
                        if (tmp[-1] in max_index) and len(data)-max_index[-1]>(mean_max_min)-std_max_min:# and len(data)-max_index[-1]<dis+std_max_min :#+2*std_max_min :#and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = -1
                    if min_index != []:
...
##########
        ...
        price_ma10_before = self.ma(self.close_price, 10, -1, current_price_index)
        price_ma10_current = self.ma(self.close_price, 10, 0, current_price_index)
        ...
        power_ma5ma10_before = price_ma5_before-price_ma10_before
        power_ma5ma10_current = price_ma5_current-price_ma10_current
        ...
        if (power_before<0 and power_current>0) or power_current>power_before>0:
            power_decision = -1
        elif power_before>0 and power_current<0:
            power_decision = 1

        power_ma5_decision = 0
        if (power_ma5_before<0 and power_ma5_current>0):# or power_ma5_current>power_ma5_before>0:
            power_ma5_decision = -1
        elif power_ma5_before>0 and power_ma5_current<0:
            power_ma5_decision = 1

        power_ma5ma10_decision = 0
        if power_ma5ma10_before<0 and power_ma5ma10_current>0:
            power_ma5ma10_decision = -1
        elif power_ma5ma10_before>0 and power_ma5ma10_current<0:
            power_ma5ma10_decision = 1
...
#####################################################################################################################
        ...
        if imf_extrem_decision9==-1 and deltama5<0 and  deltama10<0 and deltama20<0 and deltama30<0:
            imf_extrem_decision10 = -2
            self.wait_flag=1
        elif imf_extrem_decision9==-1:
            imf_extrem_decision10 = -1

        print "10 decision %s"%imf_extrem_decision10
        ...
        ma_decision = 0
        deltama_list = [deltama5,deltama10,deltama20,deltama30]
        tmp_delta_list = [i for i in deltama_list if i<0]
        if len(tmp_delta_list)>=3:
            ma_decision = 1
        ...
        print "macd %s"%self.macd[current_price_index-5:current_price_index+1]
        macd_decision1 = 0
        if self.macd[current_price_index]<0 :#and self.macd[current_price_index-3]>self.macd[current_price_index-2] and self.macd[current_price_index-2]>self.macd[current_price_index-1] and self.macd[current_price_index-1]>self.macd[current_price_index]:
            macd_decision1 = 1
        ...
        macd_decision1 = 0
        if self.macd[current_price_index-1]<0 and self.macd[current_price_index]>0:
            macd_decision1 = 1

        this_buy = 0
        if  imf_extrem_decision10 == -2:#  (current_price-ma5) > (self.close_price[current_price_index-1]-self.ma(self.close_price, 5, -1, current_price_index)) and ( imf_extrem_decision6 == -1):
            this_buy = -1
        if this_buy == -1:
            self.buy_decision=-1
        ...
        if  self.sell_decision!=-1 and self.buy_price!=0 and ((current_price-self.buy_price)/self.buy_price) >0.02:
            self.sell_decision = -1
        if coef_decision1==2 and imf_small_decision1==1 and self.buy_price!=0 :#cha_decision==1 and self.buy_price!=0 and imf_extrem_decision9==1:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            ...
            self.holdtime = 0
            once_flag = 1
        elif coef_decision1==1 and  self.buy_price!=0 and imf_extrem_decision9==1:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
            ...
            self.holdtime = 0
            once_flag = 1
#        elif cha_decision==0 and self.buy_price!=0 and  self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
#            print "sell sell 11"
#            self.money = self.sell(self.share,trade_price)
#            self.share = 0
#            self.buy_price = 0
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif self.wait_flag==1 and   self.buy_price==0 and  power_ma5_decision==-1 :#and power_ma5_decision==-1:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            if coef_decision1==1:
                print "buy buy 1"
                ...
                self.wait_flag=0
        elif self.wait_flag==0 and coef_decision1==1 and   self.buy_price==0 and   imf_extrem_decision10==-1 :#and power_ma5_decision==-1:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 2"
            self.share = self.buy(self.money,trade_price)
            self.money = 0
            self.buy_price = trade_price
...
#            self.buy_decision = 0
        ...
        if this_buy == -1 and self.buy_count==0:
            self.buy_decision = -1
            self.buy_count = 1
        elif this_buy!=-1 and self.buy_count>=1:
            self.buy_count += 1
        if this_buy!=-1 and self.buy_count>1:
            self.buy_decision = 0
            self.buy_count = 0
   ...
   #         imf.pop()
        ...
        print "coef data imf %s"%np.corrcoef(data,imf)
