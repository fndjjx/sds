...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if  imf2_flag==1:# and imf1_flag!=2 :#and imf2_or_flag!=2 :#and imf2_flag!=2 and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
####
        ...
        prechabig0=0
        #if len(self.pre_cha)>2 and self.pre_cha[-2]>0:# and self.pre_cha[-2]<0:
        #if len(self.pre_cha)>3 and  data2_ma3_list[-3]>0 and  data2_ma3_list[-2]<0:
        #    prechabig0=1
        if len(self.pre_cha)>2 and self.pre_cha[-2]<0 and self.pre_cha[-3]>0:
            prechabig0=1
        prechaopenbig0=0
        if len(self.pre_cha_open)>2 and self.pre_cha_open[-1]<0 and self.pre_cha_open[-3]>0:
            prechaopenbig0=1
        #if len(self.pre_cha)>1 and self.pre_cha[-2]>0 and data2_ma3_list[-2]<data2_ma3_list[-3]:#and self.pre_cha[-1]<0:
        #    prechabig0=1
        if self.buy_price!=0 and len(self.pre_cha)>2 and self.pre_cha[-2]>0 and self.pre_cha[-3]<0:
            prechabig0=2

        if len(self.pre_cha)>20:
            print "mean pre cha max min%s"%mean_max_min_distance(self.pre_cha[-20:])

        print "extreme num %s"%calc_extreme_num(emd_data[-20:])
        print "prechade%s"%self.pre_cha_de[-10:]
        print "open pre cha%s"%self.pre_cha_open[-10:]
        ...
        elif  prechaopenbig0==1 and (boll_de==1) and prechabig0==1 and self.buy_price==0  :#and distance_decision==1:#and emd_data[-1]>emd_data[-2]:#boll_de==1:#and deltama3>deltama3_before :#and deltama3>deltama3_before:#and pvpower_de==1 :#emd_data[-1]>emd_data[-2] and emd_data[-2]<emd_data[-3]:#self.close_price[current_price_index]>self.close_price[current_price_index-1]:#and (deltama3_before<deltama3 and deltama3_before<0 and deltama3>0) :#and ma_ex_de==1:#and distance_decision==1 and deltama3_before<deltama3 and deltama3_before<deltama3_before2 :#and deltama3_before2<deltama3_before3 :#and ma_ex_de==1:# and distance_decision==1 :# and self.close_price[current_price_index]>self.close_price[current_price_index-1]:#deltama3_before<deltama3:#self.macd[current_price_index-1]<self.macd[current_price_index] :#and (self.macd[current_price_index-2]<0 or self.macd[current_price_index-1]<0 or self.macd[current_price_index]<0):#and deltama5>=0 :#and deltama3_before<deltama3 ) or (deltama3>0 and deltama3_before<0)):# and  mama3>mama3_b and mama3_b<mama3_b2 and distance_decision==1:#and deltama5<0:#and self.close_price[current_price_index]>self.close_price[current_price_index-1] and deltama5_before<deltama5<0: #and deltama3_before<0 and deltama3>0 and deltama5_before<deltama5<0:#and ma_decision==1 :#and distance_decision==1:#deltama3_before<deltama3:# (deltama3_before>deltama3 or deltama3<0):
...
#            self.last_as=self.total_asset
    ...
    def run(self,emd_data, datafile,current_index,date,emd_file,emd_data2,emd_data3,preflag,chazhi,prechade,chazhi_open):
        self.pre_cha=chazhi
        self.pre_cha_open=chazhi_open
        ...
        (imf, residual) = my_emd.emd(0.5,0.5)
...
#
        ...
        data2_y_index = [data2[i-start] for i in self.buy_x_index]
        data2_x_index = [i-start for i in self.buy_x_index]
...
def mean_max_min_distance(data):
...
##
    open_ma2_list=[]
    for i in range(2):
        open_ma2_list.append(0)
    for i in range(2,len(open_price)):
        open_ma2_list.append(np.mean(open_price[i-1:i+1]))

    open_ma3_list=[]
    for i in range(3):
        open_ma3_list.append(0)
    for i in range(3,len(open_price)):
        open_ma3_list.append(np.mean(open_price[i-2:i+1]))

    open_ma5_list=[]
    for i in range(5):
        open_ma5_list.append(0)
    for i in range(5,len(open_price)):
        open_ma5_list.append(np.mean(open_price[i-4:i+1]))

    open_ma_list=[open_ma2_list[i]-open_ma5_list[i] for i in range(len(open_price))]
##
    chazhi = []
    chazhi_open = []
...
#####
        ...
        if 0:#ma5<ma10<ma20<ma30:
           ...
           # emd_data.append(open_price[i])
            ...
            pp2=leastsqt_predict(close_price[i-3:i],1)[0]
            pp2=leastsqt_predict(ma3_list[i-3:i],1)[0]
            #pp2=leastsqt_predict(ma2[i-3:i],1)[0]
            pp22=leastsqt_predict_2(ma3_list[i-5:i],1)[0]
            print "pp%s"%pp1
            print "pp%s"%pp2
            ...
            chazhi.append((pp2)-ma3_list[i])
            #chazhi.append((pp2)-ma2[i])
            
##
            pp2=leastsqt_predict(open_ma3_list[i-3:i],1)[0]
            print "emd data pre cha %s"%chazhi[-10:]
            chazhi_open.append(pp2-open_ma3_list[i])
#
            #emd_data.append(pp1)
...
####
        process.run(emd_data,datafile,i-1,date[-1],emd_file,emd_data2,emd_data3,pre_flag,chazhi,prechade,chazhi_open)
