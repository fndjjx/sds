...
####
        ...
        print "ma3list %s"%ma3_list[current_price_index-10:current_price_index+2]
        if len(self.kalv)>1 and self.kalv[-2][0]>ma5_list[current_price_index] and kalval[0]<ma5_list[current_price_index+1]:
        #if len(self.pre_cha)>1 and self.pre_cha[-2]>0 and (pz-rz)<0:
        ...
        #if len(self.pre_cha)>1 and self.pre_cha[-2]<0 and (pz-rz)>0:
        #if len(self.kalv)>1 and self.kalv[-2]<price_b and kalval>trade_price:
        if len(self.kalv)>1 and  (self.kalv[-2][0]-ma3_list[current_price_index])<0 and (kalval[0]-ma3_list[current_price_index+1])>0:
        #    prechabig0=2
...
#        p=float(getanytime(mindata,cdate,"10:30:00"))
    ...
    k_v = list(kalman_simple(ma3_list[:-1],ma3_list[1:]))
    #k_v=k_v1+k_v2
    k_v.insert(0,0)
    k_v1 = list(kalman_simple(close_price[:-1],close_price[1:]))
    k_v1.insert(0,0)
