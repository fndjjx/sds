...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if  imf2_flag==1 and imf1_flag==1:#imf1_or_flag!=2 and imf2_or_flag!=2 :#and imf2_flag!=2 and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
###########################
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,residual,emd_data,imf_open,imf_macd,emd_std,preflag,emd_data2,prechade,prechade2):
...
####
        ...
        ana_precha=self.pre_cha[:-1]
        ana=[]
        for i in range(1,len(ana_precha)):
            if ana_precha[i]<0 and ana_precha[i-1]>0:
                ana.append(ana_precha[i-1])
        high_line=np.mean(ana)+np.std(ana)
        print "highline%s"%high_line
        ...
        pdata2=2
        for i in range(pdata2):
            data2_ma2_list.append(0)
        for i in range(pdata2,len(data2)):
            data2_ma2_list.append(np.mean(data2[i-(pdata2-1):i+1]))

        prechabig0=0
        if len(self.pre_cha)>1 and  self.pre_cha[-2]>0:# and np.mean(self.pre_cha[-5:-1])<np.mean(self.pre_cha[-6:-2]):
            prechabig0=1

        print "pre cha de %s"%self.pre_cha_de[-10:]

        print "prechade real %s pre %s"%(self.pre_cha[-1],prechade2)

        bollboll=0
        if ((len(self.boll_de)>1 and self.boll_de[-2]==1 and boll_de==0)):
            bollboll=1
        ...
        elif  self.buy_price==0 and prechabig0==1 and self.buy_price==0 and  imf_flag==1 and distance_decision==1:#((self.close_price[current_price_index]-self.close_price[current_price_index-1])/self.close_price[current_price_index-1])<0.02:#emd_data[-1]>emd_data[-2]:#boll_de==1:#and deltama3>deltama3_before :#and deltama3>deltama3_before:#and pvpower_de==1 :#emd_data[-1]>emd_data[-2] and emd_data[-2]<emd_data[-3]:#self.close_price[current_price_index]>self.close_price[current_price_index-1]:#and (deltama3_before<deltama3 and deltama3_before<0 and deltama3>0) :#and ma_ex_de==1:#and distance_decision==1 and deltama3_before<deltama3 and deltama3_before<deltama3_before2 :#and deltama3_before2<deltama3_before3 :#and ma_ex_de==1:# and distance_decision==1 :# and self.close_price[current_price_index]>self.close_price[current_price_index-1]:#deltama3_before<deltama3:#self.macd[current_price_index-1]<self.macd[current_price_index] :#and (self.macd[current_price_index-2]<0 or self.macd[current_price_index-1]<0 or self.macd[current_price_index]<0):#and deltama5>=0 :#and deltama3_before<deltama3 ) or (deltama3>0 and deltama3_before<0)):# and  mama3>mama3_b and mama3_b<mama3_b2 and distance_decision==1:#and deltama5<0:#and self.close_price[current_price_index]>self.close_price[current_price_index-1] and deltama5_before<deltama5<0: #and deltama3_before<0 and deltama3>0 and deltama5_before<deltama5<0:#and ma_decision==1 :#and distance_decision==1:#deltama3_before<deltama3:# (deltama3_before>deltama3 or deltama3<0):
...
#            self.last_as=self.total_asset
    ...
    def run(self,emd_data, datafile,current_index,date,emd_file,emd_data2,emd_data3,preflag,chazhi,prechade,prechade2):
        self.pre_cha=chazhi
...
#        (imf_macd, residual) = my_emd3.emd(0.03,0.03)
        ...
        print current_index
        self.run_predict(imf,current_index,date,datafile,residual,emd_data,imf_open,imf_macd,std,preflag,emd_data2,prechade,prechade2)
...
#####
        ...
        if 0:#ma5<ma10<ma20<ma30:
           ...
           # emd_data.append(open_price[i])
            ...
            if len(chazhi)>2:     
                prechade2=(leastsqt_predict(chazhi[-2:],1)[0])
...
####
        process.run(emd_data,datafile,i-1,date[-1],emd_file,emd_data2,emd_data3,pre_flag,chazhi,prechade,prechade2)
