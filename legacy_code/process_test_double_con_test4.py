...
##############################################################
        ...
        if 1:#self.buy_price==0:
            ...
            if tmp[-2]>=495:
                ...
                imf2_min_index = list(argrelextrema(imf2_data,np.less)[0])
                print " imf2 min index %s"%imf2_min_index
            elif tmp[-2]<495 and tmp[-1]-tmp[-2]>=5:
        ...
        imf1_flag=0
        if small_imf_min_index[-1]>small_imf_max_index[-1] and (((496<small_imf_min_index[-1] and small_imf_max_index[-1]<495)) or (496==small_imf_min_index[-1] and small_imf_max_index[-1]<494))   :#and self.imf1_last_last=="max":
            imf1_flag=1
        ...
        last_data = np.array(imf_list[-1][:480])
        ...
        if imf_last_max_index!=[] and imf_last_min_index!=[]:
            ...
            if (tmp[-1] in imf_last_min_index) and tmp[-1]>460:
                residual_flag=1
            if (tmp[-1] in imf_last_max_index) and 0.85<((499-tmp[-1])/float(tmp[-1]-tmp[-2]))<1.2:
                residual_flag=1
...
#            imf1_flag=1
        ...
        if len(self.result_list)>=2 and self.result_list[-1]!=2 and self.result_list[-2]!=2:
            result_flag=1
        ...
        if imf2_max_index[-1]>imf2_min_index[-1] and 485<imf2_max_index[-1]<495 and result_flag==1:# and result_flag==1 :#and result_flag==1 :#and self.imf1_last_last=="max":
            imf2_flag=1
        ...
        if imf2_max_index[-1]<imf2_min_index[-1] and 480<imf2_min_index[-1]<495 :#and  self.close_price[current_price_index]<self.ma(self.close_price, 5, 0, current_price_index):#and self.imf1_last_last=="max":
            imf2_flag=2
        if imf2_max_index[-1]>imf2_min_index[-1] and imf2_max_index[-1]>495 and self.result_list[-1]!=1:#and  self.close_price[current_price_index]<self.ma(self.close_price, 5, 0, current_price_index):#and self.imf1_last_last=="max":
            imf2_flag=2
