...
#                count = 10
   ...
   #         return -1
    ...
    def run_predict1(self,imf_list,current_price_index,date,datafile,train_data_dir,train_number,hidden_number,result_number,residual):
        ...
        process_data = self.close_price[current_price_index-499:current_price_index+1]
...
##############################################################
        ...
        imf0_flag=0
        if small_small_imf_min_index[-1]>small_small_imf_max_index[-1] and small_small_imf_min_index[-1]>=496:#and self.imf1_last_last=="max":
            imf0_flag=1
        if small_small_imf_min_index[-1]<small_small_imf_max_index[-1] and small_small_imf_max_index[-1]>496:#and self.imf1_last_last=="max":
            imf0_flag=2
        ...
        big_data = np.array(big_imf[:490])
        big_data = np.array(small_imf)
        ...
        imf2_data = np.array(imf_process[:495])
        if self.buy_price==0:
        ...
        print "trust position%s"%trust_position

        if imf2_max_index[-1]>imf2_min_index[-1]:
            current_imf2_core = current_price_index-(499-imf2_max_index[-1])
            current_imf2_sec_core = -(current_price_index-(499-imf2_min_index[-1]))
        else:
            current_imf2_core = -(current_price_index-(499-imf2_min_index[-1]))
            current_imf2_sec_core = current_price_index-(499-imf2_max_index[-1])
        print "\n\n\n"
        print "imf2 core last %s sec %s fir %s"%(self.imf2_core,current_imf2_sec_core,current_imf2_core)
        ...
        if 0:#tmp[-1]<485:
            ...
            imf_process =  list(imf_list[1][:trust_position+1])
            imf2_data = np.array(imf_process)
            imf2_max_index = list(argrelextrema(imf2_data,np.greater)[0])
            print " imf2 max index %s"%imf3_max_index
            imf2_min_index = list(argrelextrema(imf2_data,np.less)[0])
            print " imf2 min index %s"%imf3_min_index
            flag=1
        ...
        if small_imf_max_true_index[-1]>small_imf_min_true_index[-1]:
            current_imf1_core = current_price_index-(499-small_imf_max_true_index[-1])
            current_imf1_sec_core = current_price_index-(499-small_imf_min_true_index[-1])
        else:
            current_imf1_core = current_price_index-(499-small_imf_min_true_index[-1])
            current_imf1_sec_core = current_price_index-(499-small_imf_max_true_index[-1])
        print "\n\n\n"
        ...
        imf0_flag=0
        if small_imf_min_index[-1]>small_imf_max_index[-1] and small_imf_min_index[-1]>496:#and self.imf1_last_last=="max":
            imf0_flag=1
        if small_imf_min_index[-1]<small_imf_max_index[-1] and small_imf_max_index[-1]>496:#and self.imf1_last_last=="max":
            imf0_flag=2

        imf1_flag=0
        #if small_imf_min_true_index[-1]>496 and small_imf_min_true_index[-1]>small_imf_max_true_index[-1] and abs(current_imf1_sec_core-current_imf1_core)>2:#and self.imf1_last_last=="max":
        #if abs(abs(small_imf_max_true_index[-1]-small_imf_min_true_index[-1])-(499-small_imf_max_true_index[-1]))<2 and small_imf_min_true_index[-1]<small_imf_max_true_index[-1] :#and abs(current_imf1_sec_core-current_imf1_core)>2:#and self.imf1_last_last=="max":
#        if small_imf_min_true_index[-1]>496 and small_imf_min_true_index[-1]>small_imf_max_true_index[-1] and abs(current_imf1_sec_core-current_imf1_core)>5:#and self.imf1_last_last=="max":
#            imf1_flag=1
        ...
        power_4 = self.close_price[current_price_index-4]-self.ma(self.close_price, 5, -4, current_price_index)
        power_5 = self.close_price[current_price_index-5]-self.ma(self.close_price, 5, -5, current_price_index)
        power_list = [power_1,power_2,power_3,power_4,power_5]
        count=0
        for i in power_list:
            if i<0:
                count+=1
        result_flag=0
        result_flag2=0
        #if len(self.result_list)>=5 and self.result_list[-1]!=2 and self.result_list[-2]!=2 and self.result_list[-3]!=2 and self.result_list[-4]!=2 and self.result_list[-5]!=2:
        if len(self.result_list)>=2 and self.result_list[-1]!=2 :
            result_flag=1
        ...
        imf2_flag=0
        imf3_flag=0
        #if imf2_max_index[-1]>imf2_min_index[-1] and 485<=imf2_max_index[-1]<495 and (self.imf2_last=="max" or abs(current_imf2_sec_core-self.imf2_core)<=3) and ((power_0>0 and count>2 ) or (power_0>0 and power_1>0 and count>2) or (power_0>0 and power_1>0 and power_2>0 and power_3<0 and power_4<0)) and abs(abs(current_imf2_sec_core)-abs(current_imf2_core))>3 and result_flag==1:
        #if flag==0 and imf3_max_index[-1]>imf3_min_index[-1] and 480<=imf3_max_index[-1]<490 and  ((power_0>0 and count>2 ) or (power_0>0 and power_1>0 and count>2) or (power_0>0 and power_1>0 and power_2>0 and power_3<0 and power_4<0))  and result_flag==1:
        if imf2_max_index[-1]>imf2_min_index[-1] and 475<imf2_max_index[-1]<495 :#and result_flag==1:# and result_flag==1 :#and result_flag==1 :#and self.imf1_last_last=="max":
            imf2_flag=1
        #if imf3_max_index[-1]>imf3_min_index[-1] and 475<imf3_max_index[-1]<=490 :#and result_flag2==1:# and result_flag2==1:#and result_flag2==1 :#and self.imf1_last_last=="max":
        if imf3_max_index[-1]<imf3_min_index[-1] and (496<imf3_min_index[-1] or (496==imf3_min_index[-1] and imf3_max_index[-1]<494))  :#and result_flag2==1:# and result_flag2==1:#and result_flag2==1 :#and self.imf1_last_last=="max":
            imf3_flag=1
        if imf2_max_index[-1]<imf2_min_index[-1] and 480<imf2_min_index[-1]<495 :#and  self.close_price[current_price_index]<self.ma(self.close_price, 5, 0, current_price_index):#and self.imf1_last_last=="max":
            imf2_flag=2
        if imf2_max_index[-1]>imf2_min_index[-1] and imf2_max_index[-1]>495 :#and  self.close_price[current_price_index]<self.ma(self.close_price, 5, 0, current_price_index):#and self.imf1_last_last=="max":
            imf2_flag=2
        #if imf3_max_index[-1]<imf3_min_index[-1] and 475<imf3_min_index[-1]<=490:#and self.imf1_last_last=="max":
        if imf3_max_index[-1]>imf3_min_index[-1] and 495<imf3_max_index[-1]:#and self.imf1_last_last=="max":
            imf3_flag=2
        #if imf2_max_index[-1]>imf2_min_index[-1] and imf2_max_index[-1]>=496 and abs(current_imf2_core-self.buy_core)>3 :#and abs(imf2_max_index[-1]-imf2_min_index[-1])>=6:#and self.imf1_last_last=="max":
        #    imf1_flag=2
        #if imf2_max_index[-1]<imf2_min_index[-1] and imf2_min_index[-1]<495 :#and abs(current_imf2_core-self.buy_core)>3 :#and abs(imf2_max_index[-1]-imf2_min_index[-1])>=6:#and self.imf1_last_last=="max":
        #    imf1_flag=2
        #if small_imf_min_true_index[-1]<small_imf_max_true_index[-1] and 0.8<((small_imf_max_true_index[-1]-small_imf_min_true_index[-1])/float(499-small_imf_max_true_index[-1]))<1.2 and abs(499-current_imf1_core)>2:
        #    imf1_flag=1
        if 0:#abs(current_imf1_sec_core-current_imf1_sec_core)<2:
            imf1_flag=0

        #if small_imf_max_true_index[-1]>=496 and small_imf_max_true_index[-1]>small_imf_min_true_index[-1] :#and self.imf1_last_last=="min":
        #    imf1_flag=2
        #if small_imf_max_true_index[-1]<small_imf_min_true_index[-1] and 0.75<((small_imf_min_true_index[-1]-small_imf_max_true_index[-1])/float(499-small_imf_min_true_index[-1]))<1.3 and imf0_flag==2:
         #   imf1_flag=2
#        if small_imf_max_true_index[-1]<small_imf_min_true_index[-1] and small_imf_min_true_index[-1]<495 and imf0_flag==2:
#            imf1_flag=2
        if imf2_min_index[-1]>imf2_max_index[-1]:
            self.imf2_last = "min"
        else:
            self.imf2_last = "max"
...
####################
        ...
        deltama25_before3 = self.ma(self.close_price, 25, -3, current_price_index)-self.ma(self.close_price, 25, -4, current_price_index)
...
#######
        ...
        if    (self.ma_count<=self.ma_count2<0 and self.ma_count<=-5):#and (np.corrcoef(macd_line,cri_sample)[0][1])<-0.8) :#or ((np.corrcoef(ma5_line,cri_sample)[0][1])<-0.9 and (np.corrcoef(macd_line,cri_sample)[0][1])<-0.9):
            ma_decision3 = 0
 ...
 #       if distance_decision2 ==1 and self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0.01:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
        if   deltama3<deltama3_before and self.buy_price!=0 and (imf2_flag==2) and ((current_price-self.buy_price)/self.buy_price) >0.01:#(imf_extrem_decision6==1 ) :#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            ...
            self.ma_record=[]
            self.buy_core=0
        elif   self.buy_price!=0 and ((((self.buy_mean_price-current_mean_price)/self.buy_mean_price)>0.02 ) or (deltama3_before2<0 and deltama3_before<0 and deltama3<0))  and self.share > 0:
            print "sell sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
...
#            self.ma_record=[]
  ...
  #      elif delta_macd1>delta_macd2 and macd_ma_decision==1 and  ma_decision3==1 and  self.buy_price==0 and  up_potion_decision2==1 and    down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
        elif      imf2_flag==1 and  self.buy_price==0 and   ( (((deltama3_before<0 and deltama3>0) or (deltama5_before<0 and deltama5>0))) )  and delta_buy_decision1==1  :#imf_extrem_decision6==-1  :
...
#######
     ...
     #   (imf0,imf1,imf2,imf3)= my_eemd.eemd_process(emd_data,50,4,"multi")
     #   imf=[]
     #   imf.append(imf0)
     #   imf.append(imf1)
     #   imf.append(imf2)
...
if __name__ == "__main__":
    ...
    generate_emd_func.generate_ma_emd_data_fix(25)
