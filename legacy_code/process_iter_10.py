...
#        elif self.macd[current_price_index] > 0 and ((self.macd[current_price_index-1]-self.macd[current_price_index-2])>(self.macd[current_price_index]-self.macd[current_price_index-1])):
        ...
        if extrem_decision == 1 and self.buy_price < self.close_price[current_price_index] and self.share > 0 and ex_decision5 == 1 :
            ...
            self.down_macd_counter = 0
            self.buy_macd = 0
#        if extrem_decision == 1 and imf[imf_max_index[-2]] > imf[imf_max_index[-1]]  and self.share > 0 and ex_decision5 == 1 :
#            print "sell 1"
#            self.money = self.sell(self.share,trade_price)
#            self.share = 0
#            self.buy_price = 0
...
#            tomorrow_will = 1
            ...
            self.buy_macd = 0
            tomorrow_will = 1
        elif macd_decision1 == 1 and ex_decision1 == 1 and self.money>0 and std_decision1 == 1 and self.current_extrem == "min" and  ( now_position_last_min_delta == 1 or now_position_last_min_delta == 2)  and ex_decision1 == 1 and self.money>0 :#and macd_decision1 == 1:#and power_current>power_before and (self.power_ma_decision(current_price_index)[0] == 1 ): #and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 5, -1, current_price_index)) :#and imf_power_decision == -1 and power_decision == -1 and self.money>0 :
            print "buy buy 1"
            self.share = self.buy(self.money,trade_price)
            self.money = 0
            self.buy_price = trade_price
...
#            tomorrow_will = -1
   ...
   #         imf.pop()
        ...
        self.run_predict_price_period(imf,current_index,residual,imf_raw,date,datafile)
        endtime = datetime.datetime.now()
        print "run time"
        print (endtime - starttime).seconds
