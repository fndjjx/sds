...
class process_util():
    ...
    def run_predict1(self,imf_list,current_price_index,date,datafile,residual):
        ...
        imf2_or_flag=0
        if imf2_min_index[-1]>imf2_max_index[-1] and imf2_min_index[-1]>=996 and imf2_min_index[-1]-imf2_max_index[-1]>1:
            imf2_or_flag=1

        imf3_max_index = list(argrelextrema(np.array(imf_list[3]),np.greater)[0])
        imf3_min_index = list(argrelextrema(np.array(imf_list[3]),np.less)[0])
        imf3_or_flag=0
        if imf3_min_index[-1]>imf3_max_index[-1] and imf3_min_index[-1]>=996 and imf3_min_index[-1]-imf3_max_index[-1]>1:
            imf3_or_flag=1
...
##############################################################
        ...
        (tmp,extenflag) = matchlist2(imf_process)
        ...
        #imf_3 = matchlist(imf_list[3],10,30)
        (imf_3,extenflag2) = matchlist2(list(imf_list[4]))
        imf3_max_index = list(argrelextrema(np.array(imf_3),np.greater)[0])
        print " imf3 max index %s"%imf3_max_index
        imf3_min_index = list(argrelextrema(np.array(imf_3),np.less)[0])
        print " imf3 min index %s"%imf3_min_index
        ...
        if imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>998 and imf_min_index[-1]-imf_max_index[-1]>1:
            imf2_flag=1
        if imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>=995:
            imf2_flag=2
        if imf3_min_index[-1]>imf3_max_index[-1] and imf3_min_index[-1]>990:# and imf_min_index[-1]-imf_max_index[-1]>1:
            imf3_flag=1
        ...
        print "imf flag3 %s"%imf3_flag
        print "exten flag%s"%extenflag
        decision=0
        if imf2_flag==1 and  imf2_or_flag==1 and extenflag<1:
            decision=1 
        if imf2_flag==2:
            decision=2
...
#        print "snr %s"%np.mean(self.snr[-1])
        ...
        if self.buy_price!=0  and (imf_flag==2 or (deltama5<0 and deltama5_before>0) or (deltama3<0 and deltama3_before>0)):
