...
#        if self.buy_price!=0 and (((trade_price-self.buy_price)/self.buy_price)>0.02 )and self.share > 0 :
#            print "sell 1"
#            self.money = self.sell(self.share,trade_price)
#            self.share = 0
#            self.buy_price = 0
...
#            self.down_macd_counter = 0
#            self.buy_macd = 0
        if self.buy_price!=0 and (imf_power_decision == 1  or imf_power_current < 0)and self.share > 0 :
            print "sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
...
#            tomorrow_will = -1
        elif  imf_power_decision == -1 and  self.money>0 :#imf[-1]>imf[-2] and current_price<self.ma(self.close_price, 5, 0, current_price_index) and self.vol[current_price_index]>self.vol[current_price_index-1] and ((trade_price-current_price)/current_price)<0.05 and ( now_position_last_min_delta == 1 or now_position_last_min_delta == 1) and  self.money>0 and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0  and self.money>0:
            print "buy buy 1"
            self.share = self.buy(self.money,trade_price)
            self.money = 0
            self.buy_price = trade_price
...
if __name__ == "__main__":
    ...
    print "begin %s"%begin
    process.generate_emd_data_fix(vol,begin,50)
