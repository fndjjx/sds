...
##############################################################
    ...
    #    print "price_min_index %s"%price_min_index_smooth
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            index_list.sort() 
            last_extrem_position = index_list[-8] 
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1] and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                  ...
                  #      print "restruct price max %s"%(argrelextrema(restruct_price_max,np.greater)[0])
                    ...
                    print "price %s"%price_max_data
                    max_index = argrelextrema(np.array(price_max_data),np.greater)[0]
                    print max_index
                    min_index = argrelextrema(np.array(price_max_data),np.less)[0]
                    print min_index
                    max_value = [price_max_data[i] for i in max_index]
                    print max_value
                    min_value = [price_max_data[i] for i in min_index]
                    print min_value
...
###############################
                    ...
                    array_price_max_data = np.array(price_max_data)
                    max_index_price_max_data = argrelextrema(array_price_max_data,np.greater)[0]
                    max_value_price_max_data = [array_price_max_data[i] for i in max_index_price_max_data]
                    min_index_price_max_data = argrelextrema(array_price_max_data,np.less)[0]
                    min_value_price_max_data = [array_price_max_data[i] for i in min_index_price_max_data]
                    data_up_envelop = list(np.array(splinerestruct(max_index_price_max_data,max_value_price_max_data,len(price_max_data),2,0)))
                    data_down_envelop = list(np.array(splinerestruct(min_index_price_max_data,min_value_price_max_data,len(price_max_data),2,0)))
                    data_middle = [(data_up_envelop[i]+data_down_envelop[i])/2 for i in range(len(price_max_data))]
                    data8 = [price_max_data[i]-data_middle[i] for i in range(len(data_middle))]
                    x1 = np.linspace(0,len(price_max_data)-1,len(price_max_data))
                    data9 = list(np.array(splinerestruct(x1,data8,len(data8),3,8)))

                    array_data9 = np.array(data9)
                    array9_max = list(argrelextrema(array_data9,np.greater)[0])
                    array9_min = list(argrelextrema(array_data9,np.less)[0])
                    print "data9 max %s"%array9_max
                    print "data9 min %s"%array9_min
                    index_set = array9_max+array9_min
                    index_set.sort()
                    print "index set %s"%index_set
                    tmp = imf_min_true_index[-1]
                    if len(index_set)>1:
                        if index_set[0] in array9_min:
                            index_set.pop(0)
                        index_set.pop()
                        if len(index_set)>0:
                            for i in range(len(index_set)):
                                if i%2 == 0:
                                    imf_max_true_index.append(index_set[i]+tmp)
                                else:
                                    imf_min_true_index.append(index_set[i]+tmp)
                    ...
                    if abs(last_max-last_min)<20  and ((last_max>last_min and last_max!= 0 and 499-last_max>(mean_max_min) and 499-last_max<(mean_max_min+std_max_min/2)) or (last_min>last_max and last_min!=0 and 499-last_min<4)):
                    #if (last_min>last_max and last_min!=0 and 499-last_min<3):
                        imf_extrem_decision4 = -1
                    print "decision 4 %s"%imf_extrem_decision4
...
###############################
         ...
         #       #    if len(price_last_min_index)>3:
                ...
                #            estimate_min = price_last_min_index[(argrelextrema(restruct_price_min,np.less)[0])[0]]
                    ...
                    print "price %s"%price_min_data
                    max_index = argrelextrema(np.array(price_min_data),np.greater)[0]
                    print max_index
                    min_index = argrelextrema(np.array(price_min_data),np.less)[0]
                    print min_index
                    max_value = [price_min_data[i] for i in max_index]
                    print max_value
                    min_value = [price_min_data[i] for i in min_index]
                    print min_value
...
##################################################
                    ...
                    array_price_min_data = np.array(price_min_data)
                    max_index_price_min_data = argrelextrema(array_price_min_data,np.greater)[0]
                    max_value_price_min_data = [array_price_min_data[i] for i in max_index_price_min_data]
                    min_index_price_min_data = argrelextrema(array_price_min_data,np.less)[0]
                    min_value_price_min_data = [array_price_min_data[i] for i in min_index_price_min_data]
                    data_up_envelop = list(np.array(splinerestruct(max_index_price_min_data,max_value_price_min_data,len(price_min_data),2,0)))
                    data_down_envelop = list(np.array(splinerestruct(min_index_price_min_data,min_value_price_min_data,len(price_min_data),2,0)))
                    data_middle = [(data_up_envelop[i]+data_down_envelop[i])/2 for i in range(len(price_min_data))]
                    data8 = [price_min_data[i]-data_middle[i] for i in range(len(data_middle))]
                    x1 = np.linspace(0,len(price_min_data)-1,len(price_min_data))
                    data9 = list(np.array(splinerestruct(x1,data8,len(data8),3,8)))
                    ...
                    index_set.sort()
                    print "index set %s"%index_set
                    tmp = imf_max_true_index[-1]
                    if len(index_set)>1:
                        if index_set[0] in array9_max:
                            index_set.pop(0)
                        index_set.pop()
                        print index_set
                        if len(index_set)>0:
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif ( imf_extrem_decision4 == -1):# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
