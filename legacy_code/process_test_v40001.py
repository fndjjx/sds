            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    choice=0
                    if len(self.imf_std)>=10:
                        if (np.std(rawdata)/np.mean(rawdata))<((np.mean(self.imf_std[-10:]))-(np.std(self.imf_std[-10:]))):
                            choice=-1 
                        elif (np.std(rawdata)/np.mean(rawdata))>((np.mean(self.imf_std[-10:]))+(np.std(self.imf_std[-10:]))):
                            choice=1 
                  ...
                  #          min_index.append(sum(small_index_min)/len(small_index_min))
                    ...
                    if len(small_index_max)>0 and coef_choice==1:
                        ...
                        if len(small_index_max)>2:
                            ...
                            max_index.append(sum(small_index_max[:])/len(small_index_max[:]))
                        if imf_max_remove_index!=[] and max_index[0]<imf_max_remove_index[0]:
                            max_index=[]
                  ...
                  #          max_index.append(sum(small_index_max)/len(small_index_max))
                    ...
                    if len(small_index_min)>0 and coef_choice==1:
                        ...
                        if len(small_index_min)>2:
                            ...
                            min_index.append(sum(small_index_min[:])/len(small_index_min[:]))
                        if imf_min_remove_index!=[] and min_index[0]<imf_min_remove_index[0]:
                            min_index=[]
                        print min_index
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif delta_buy_decision1==1 and  self.buy_price==0 and   imf_extrem_decision6==-1  :#and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]<self.macd[current_price_index-3]<self.macd[current_price_index-4]<self.macd[current_price_index-5] :#and small_imf_decision1 == -1:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
