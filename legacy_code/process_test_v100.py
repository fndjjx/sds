...
#            min_diff = abs(after_re[-1]-after_re[vol_min_index[-1]])
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                    ...
                    price_max_data = price_list[imf_min_true_index[-1]+1:] 
...
#                    c = b[:2]+b[-2:]
                    ...
                    for i in max_index:
                        ...
                        max_pair.append((i,data[i]))
                    for i in min_index:
                        min_value.append(data[i])
                        min_pair.append((i,data[i]))
                    
                    max_value.sort()
                    min_value.sort()

                    max_pair.sort(key=lambda x: x[1])
                    min_pair.sort(key=lambda x: x[1])
                    print max_pair
                    print min_pair
                    ...
                    if len(data)>12: 
                        data11 = data[:min_pair[0][0]]  
                        data11_max_index = list(argrelextrema(np.array(data11),np.greater)[0])
                        data11_max_value = []
                        for i in data11_max_index:
                            data11_max_value.append(data11[i])
                        data11_max_value.sort()
                        c=(data11_max_value[-1:])
                        c.append(min_value[0])


                        #data1 = data[max_pair[-1][0]:]
                        #data1_min_index = list(argrelextrema(np.array(data1),np.less)[0])
                        #data1_min_value = []
                        #for i in data1_min_index:
                        #    data1_min_value.append(data1[i])
                        #data1_min_value.sort()
                        #if data1_min_value!=[]:
                        #    c.append(data1_min_value[0])
                    else:
                        c = max_value[-1:]#+min_value[:1]
                    ...
                    if not 0 in index:
                        index.insert(0,0)
                    if not len(data)-1 in index:
                        index.insert(len(data)-1,len(data)-1)
                    index.sort()
                    d=[]
                    for i in index:
                        d.append(data[i])
                    ...
                    print d
                    not_good = 0
                    if len(index)<4 and len(index)>2: 
                        b=splinerestruct2(data,d,index,2,0)
                    elif len(index)>=4:
                        b=splinerestruct2(data,d,index,3,0)
                    else:
                        b=splinerestruct2(data,d,index,1,0)
                        not_good = 1
                    ...
                    if max_index != []:
                        ...
                        cha = abs(b[max_index[-1]]-b[0])/abs(b[max_index[-1]]-b[-1])
                        print "cha cha %s"%cha
                        ...
                        if (tmp[0] in max_index) and (tmp[-1] in max_index) and len(data)-max_index[-1]>mean_max_min-std_max_min and cha<1.25 and cha>0.9:#+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                    ...
                    if min_index != []:
                        ...
                        if (tmp[0] in max_index) and (tmp[-1] in min_index)  and len(data)-min_index[-1]<5:
                            if len(tmp)>=2 and (tmp[-1]-tmp[-2]>3):
                                imf_extrem_decision6 = -1
                            elif len(tmp)<2 :
                                imf_extrem_decision6 = -1

                    if not_good == 1 :#or too_far==1:
                        imf_extrem_decision6 = 0
            ...
            #            imf_extrem_decision = -1
                ...
                #            estimate_min = price_last_min_index[(argrelextrema(restruct_price_min,np.less)[0])[0]]
                    price_min_data = price_list[imf_max_true_index[-1]+1:] 
...
#                    c = b[:2]+b[-2:]
                    ...
                    if len(data)>12:
                        data11 = data[:max_pair[-1][0]]
                        data11_min_index = list(argrelextrema(np.array(data11),np.less)[0])
                        data11_min_value = []
                        for i in data11_min_index:
                            data11_min_value.append(data11[i])
                        data11_min_value.sort()
                        c=(data11_min_value[:1])
                        c.append(max_value[-1])
                    #    c = min_value[:1]
                    #    data1 = data[min_pair[0][0]:]
                    #    data1_max_index = list(argrelextrema(np.array(data1),np.greater)[0])
                    #    data1_max_value = []
                    #    for i in data1_max_index:
                    #        data1_max_value.append(data1[i])
                    #    data1_max_value.sort()
                    #    if data1_max_value!=[]:
                    #        c.append(data1_max_value[-1])
                    else:
                        c = min_value[:1]
                    ...
                    min_index = list(argrelextrema(data,np.less)[0])
                    #count = 0
                    #while (len(max_index+min_index)>=2) and count<5:
                    #    b=splinerestruct(residual)
                    #    residual = [round(i,2) for i in b]
                    #    residual = b
                    #    data = np.array(residual)
                    #    max_index = list(argrelextrema(data,np.greater)[0])
                    ...
                    if max_index != []:
                        ...
                        if (tmp[0] in min_index) and  (tmp[-1] in max_index) and len(data)-max_index[-1]>mean_max_min-std_max_min and cha>0.9 and cha<1.25:#+2*std_max_min :#and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = -1
                        if max_index[0]==0:
                            imf_extrem_decision6 = -1
                    if min_index != []:
                        ...
                        print len(data)-min_index[-1]
                        if (tmp[0] in min_index) and (tmp[-1] in min_index) and len(data)-min_index[-1]<5:
                            if len(tmp)>=2 and tmp[-1]-tmp[-2]>3:
                                imf_extrem_decision6 = -1
                            elif len(tmp)<2:
                                imf_extrem_decision6 = -1
...
#####################################################################################################################
        ...
        if up_decision1 == 1 and delta_power_decision==1 and imf_extrem_decision6==-1:#  current_price>ma5  and self.close_price[current_price_index-1]<self.ma(self.close_price, 5, -1, current_price_index) and ( imf_extrem_decision6 == -1):
            self.buy_decision = -1
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif  self.buy_decision == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
