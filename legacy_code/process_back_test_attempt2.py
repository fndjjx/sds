...
####
        ...
        imf1_or_flag=0
        if imf1_min_index[-1]>imf1_max_index[-1] and imf1_min_index[-1]>=198 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf1_or_flag=1
        #if imf1_min_index[-1]>imf1_max_index[-1] and imf1_min_index[-1]==797 and imf1_min_index[-1]-imf1_max_index[-1]>1:
        #    imf1_or_flag=1
        if imf1_min_index[-1]<imf1_max_index[-1] and imf1_max_index[-1]>=197 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf1_or_flag=2
...
###
        ...
        imf2_or_flag=0
        #if ((imf2_min_index[-1]<imf2_max_index[-1] and imf2_max_index[-1]<=795) or (imf2_min_index[-1]>imf2_max_index[-1] and imf2_min_index[-1]>=797)) and abs(imf_list[2][-1]/imf_list[2][imf2_max_index[-1]])>0.9: 
...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if     imf1_or_flag==1 :#and residual_flag==1:#and imf1_flag!=2 :#and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
###########################
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,residual,emd_data,imf_open,imf_macd,emd_std,preflag,emd_data2,prechade,kalval,predict):
        ...
        self.snr.append(calc_SNR(emd_data,imf_list,20))
...
###
    ...
    #        ma_list=self.buy_ma
      ...
      #      bayes_de = 2
        ...
        print sum(map(lambda x:abs(x),calc_recent_rilling(imf_list[1][-20:])))
