...
#            ma_decision3 = 0
        ...
        vol_ma60_before = self.ma(self.vol,60,-1,current_price_index)
        ...
        print "vol 60 before %s"%vol_ma60_before
        ...
        delta_list = [delta_ma_3,delta_ma_5,delta_ma_7,delta_ma_10,delta_ma_20]
...
#            self.down_macd_counter = 0
#            self.buy_macd = 0
        if self.buy_price!=0 and ( (((self.vol[current_price_index-1]-self.vol[current_price_index])/self.vol[current_price_index-1])>0.5) or ((current_price-self.buy_price)/self.buy_price)>0.03 ) and self.share > 0 :
            print "sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
...
#            tomorrow_will = -1
        elif self.macd[current_price_index]>self.macd[current_price_index-1] and delta_price_decision1 == 1 and (imf_power_decision == -1 )and  self.money>0 and residual[-2]<residual[-1] and delta_ma_decision1 == 1:#and imf[-1]<self.ma(imf, 10, 0, current_index):#and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 10, 0, current_price_index)) :#imf[-1]>imf[-2] and current_price<self.ma(self.close_price, 5, 0, current_price_index) and self.vol[current_price_index]>self.vol[current_price_index-1] and ((trade_price-current_price)/current_price)<0.05 and ( now_position_last_min_delta == 1 or now_position_last_min_delta == 1) and  self.money>0 and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0  and self.money>0:
#        elif delta_price_decision1 == 1 and (now_position_last_min_delta == 1 or now_position_last_min_delta == 2 )and  self.money>0 and residual[-2]<residual[-1] and delta_ma_decision1 == 1:#and imf[-1]<self.ma(imf, 10, 0, current_index):#and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 10, 0, current_price_index)) :#imf[-1]>imf[-2] and current_price<self.ma(self.close_price, 5, 0, current_price_index) and self.vol[current_price_index]>self.vol[current_price_index-1] and ((trade_price-current_price)/current_price)<0.05 and ( now_position_last_min_delta == 1 or now_position_last_min_delta == 1) and  self.money>0 and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0  and self.money>0:
            if self.fail_flag == 0:
                print "buy buy 1"
                self.share = self.buy(self.money,trade_price)
                self.money = 0
...
#            tomorrow_will = -1
    ...
    def run(self,emd_data, datafile,current_index,date):
        ...
        if len(imf)>=2:
            ...
            self.run_predict_price_period(imf_pro,current_index,residual,imf_pro,date,datafile)
            endtime = datetime.datetime.now()
            print "run time"
            print (endtime - starttime).seconds
#        if len(imf)==2:
