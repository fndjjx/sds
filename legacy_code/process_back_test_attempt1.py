...
from calc_svm_imf import most_like_imf
...
class process_util():
    ...
    def calc_last_trust(self,current_price_index,imf_list):
        ...
        data = data_until_now[-500:]
        #data = cubicSmooth5(data)
        ma_max_index, ma_min_index , long_period, short_period, madiff, _ = most_like_imf(data, imf_list, 1, 2)
...
###
        ...
        if imf2_min_index[-1]>imf2_max_index[-1] and imf2_min_index[-1]>=497 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf2_or_flag=1
        if imf2_min_index[-1]<imf2_max_index[-1] and imf2_max_index[-1]>=497 :#and imf2_max_index[-1]-imf2_min_index[-1]>3:
            imf2_or_flag=2
...
#######
      ...
      #  if last_trust==1:
      #      (imf_process,extenflag1,ex_num,cha,cha2) = matchlist1(imf_list[i_n],0,emd_data)
      #  elif last_trust==2:
      #      (imf_process,extenflag1,ex_num,cha,cha2) = matchlist2(imf_list[i_n],0,emd_data)
      #  else:
      #      (imf_process,extenflag1,ex_num,cha,cha2) = matchlist3(imf_list[i_n],0,emd_data)
...
#
       # imf2_flag=0
       # if imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>=497  :#and imf_min_index[-1]-imf_max_index[-1]>10:#and ex_flag==1:#and imf_process[-1]<0 :
       #     imf2_flag=1
       # if imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>=497:
       #     imf2_flag=2
...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if    imf2_or_flag==1 :#and residual_flag==1:#and imf1_flag!=2 :#and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
#            self.last_as=self.total_asset
    ...
    def run(self,emd_data, datafile,current_index,date,emd_file,emd_data2,emd_data3,preflag,chazhi,prechade,kalval,predict):
        ...
        (imf, residual) = my_emd.emd(0.00005,0.00005)
...
#
       ...
       # data1 = ma_me
        ...
        plt.plot([i for i in range(len(data1))],data1,'o',[i for i in range(len(data1))],data1,'b',self.buy_x_index,ma_buy_value,'r*',self.sell_x_index,ma_sell_value,'g*')             
