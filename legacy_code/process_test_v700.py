...
##############################################################
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            big_tmp = big_imf_max_true_index+big_imf_min_true_index
            big_tmp.sort()
            big_decision1 = 0
            if big_tmp[-1] in big_imf_max_true_index and big_tmp[-1]>=490:
                big_decision1 = 1
            ...
            imf_max_true_index = list(filter(lambda n:n<=500,imf_max_index))
            imf_min_true_index = list(filter(lambda n:n<=500,imf_min_index))
...
#####################################################################################################################
        ...
        if self.buy_price!=0 and imf_extrem_decision9==1:#self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif  big_decision1==1 and self.buy_price==0 and   self.buy_decision == -1 :#and   power_ma5_decision==-1 :#and power_ma5_decision==-1:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
