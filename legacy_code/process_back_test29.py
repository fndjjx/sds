...
####
        ...
        print "close price i-1 %s"%close_price[current_price_index-1]

        print "this date%s"%self.date[current_price_index]
        date = self.date[current_price_index].replace("/","-")
        date = date.replace(" ","")
        print date
        nearprice=float(self.getDatanearClose(self.mindata,date))
        zhi5=close_price[current_price_index-1]+close_price[current_price_index-2]+close_price[current_price_index-3]+close_price[current_price_index-4]
        zhi5=zhi5+nearprice

        zhi2=close_price[current_price_index-1]+nearprice

        near_ma=zhi2/2.0-zhi5/5.0
        print "nearprice %s"%nearprice
        print "close %s"%self.close_price[current_price_index]

###3
        prechabig0=0
        if len(self.pre_cha)>1 and self.pre_cha[-2]>0 and (predict-near_ma)<0:#and self.pre_cha[-1]<0:
            prechabig0=1
...
if __name__ == "__main__":
    ...
    mindata = getMinDatabyFile("/home/ly/project/stock/my_work2/sds/back_test/test_data/ll/min1_sh600089.csv")
...
#####
        ...
        if 0:#ma5<ma10<ma20<ma30:
           ...
           # emd_data.append(open_price[i])
            ...
            emd_data=(ma_list[i-500:i-1])


            pp1=leastsqt_predict(emd_data[-10:],1)[0]
            pp2=leastsqt_predict(close_price[i-10:i-1],1)[0]
            print "pp%s"%pp1
            print "pp%s"%pp2
            ...
            print "close i-2 %s"%close_price[i-2]
            print "zhi %s"%zhi
