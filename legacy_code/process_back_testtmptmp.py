...
class process_util():
    ...
    def __init__(self,open_price,close_price,macd,date,vol,high_price,low_price,je,kdjk,kdjd,bias,wr):
...
###
        ...
        #    imf2_or_flag=1
        if imf2_max_index[-1]>=495 :#and imf2_max_index[-1]-imf2_min_index[-1]>3:
            imf2_or_flag=2
...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if imf2_flag==1 and imf2_or_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
###
        ...
        bias_de=0
        bias_c=self.bias[current_price_index]
        bias_b1=self.bias[current_price_index-1]
        bias_b2=self.bias[current_price_index-2]
        if bias_b2>bias_b1 and bias_b1<bias_c:
            bias_de=1
        print "bias%s"%self.bias[current_price_index-5:current_price_index+1]
...
####
        ...
        wr_de=0
        #if self.wr[current_price_index]<50:
        if wrma3>60:
            wr_de=1
#####
        ...
        if  self.buy_price!=0  and boll_de==2:#(imf_flag==2) and deltama3<deltama3_before  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
            ...
            if self.fail_flag==0:
                ...
                print "sellsell 1"
                self.buy_price=0
        elif self.buy_price!=0 and ((((self.buy_price-current_price)/self.buy_price)>0.05 ) ) and current_k<before_k :#and self.share > 0 :
            print "sell sell 4"
            if self.fail_flag==0:
                if self.buy_price>=trade_price:
                    self.fail_flag=flag_fail
...
def bias_func(c,n):
    ...
    for i in range(n,len(c)):
        ma=np.mean(c[i-n+1:i+1])
        bias.append(((c[i]-ma)/ma)*100)
...
def kdj_func2(c,h,l,n):
    ...
    for i in range(n-1,len(c)):
        ...
        hn=max(h[i-(n-1):i])
        ln=min(l[i-(n-1):i])
        rsv.append(100*(float(cn-ln)/(hn-ln)))


    k = [ema(rsv,i,3) for i in range(3,len(rsv))]
    d = [ema(k,i,3) for i in range(3,len(k))]
    k.insert(0,0)
    k.insert(0,0)
    k.insert(0,0)
    d.insert(0,0)
...
if __name__ == "__main__":
    ...
    bias=bias_func(close_price,10)
    wr=wr_func(close_price,high_price,low_price,10)
    process = process_util(open_price,close_price,macd,date,vol,high_price,low_price,je,k,d,bias,wr)
