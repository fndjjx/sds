...
########
        imf_open_max_index = list(argrelextrema(np.array(imf_open[2]),np.greater)[0])
        print " imf open max index %s"%imf_open_max_index
        imf_open_min_index = list(argrelextrema(np.array(imf_open[2]),np.less)[0])
...
###
        ...
        imf_open_max_index = list(argrelextrema(np.array(imf_open_process),np.greater)[0])
        print " imf open max index %s"%imf_open_max_index
        imf_open_min_index = list(argrelextrema(np.array(imf_open_process),np.less)[0])
        print " imf open min index %s"%imf_open_min_index

        imf2_open_flag=0
        
        if imf_open_min_index[-1]>imf_open_max_index[-1] and imf_open_min_index[-1]>len(imf_open_process)-5 :#and extenflag2<0.6 :#and imf_open_min_index[-1]-imf_open_max_index[-1]>3:
            imf2_open_flag=1
        if imf_open_min_index[-1]<imf_open_max_index[-1] and imf_open_max_index[-1]>len(imf_open_process)-5:
            imf2_open_flag=2

#####

        imf_macd_max_index = list(argrelextrema(np.array(imf_macd[2]),np.greater)[0])
        print " imf macd max index %s"%imf_macd_max_index
        imf_macd_min_index = list(argrelextrema(np.array(imf_macd[2]),np.less)[0])
...
###
        ...
        imf_macd_max_index = list(argrelextrema(np.array(imf_macd_process),np.greater)[0])
        print " imf macd max index %s"%imf_macd_max_index
        imf_macd_min_index = list(argrelextrema(np.array(imf_macd_process),np.less)[0])
        print " imf macd min index %s"%imf_macd_min_index
...
#####
        ...
        if imf_macd_min_index[-1]>imf_macd_max_index[-1] and imf_macd_min_index[-1]>len(imf_macd_process)-5 :#and imf_process3[-1]<0:#and imf_open_min_index[-1]-imf_open_max_index[-1]>3:
            imf2_macd_flag=1
        if imf_macd_min_index[-1]<imf_macd_max_index[-1] and imf_macd_max_index[-1]>len(imf_macd_process)-5:
            imf2_macd_flag=2

####
        imf_4_max_index = list(argrelextrema(np.array(imf_list[3]),np.greater)[0])
        print " imf 4 max index %s"%imf_4_max_index
        imf_4_min_index = list(argrelextrema(np.array(imf_list[3]),np.less)[0])
...
###
        ...
        imf_4_max_index = list(argrelextrema(np.array(imf_4_process),np.greater)[0])
        print " imf 4 max index %s"%imf_4_max_index
        imf_4_min_index = list(argrelextrema(np.array(imf_4_process),np.less)[0])
        print " imf 4 min index %s"%imf_4_min_index
        ...
        if imf_4_min_index[-1]>imf_4_max_index[-1] and imf_4_min_index[-1]>=990 :#and imf_process3[-1]<0:#and imf_open_min_index[-1]-imf_open_max_index[-1]>3:
            imf4_flag=1
...
###########################
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,residual,emd_data,imf_open,imf_macd):
...
#######
        ...
        elif    imf_flag==1 and  self.buy_price==0 :#and deltama3_before<deltama3:# and distance_decision==1 :# and self.close_price[current_price_index]>self.close_price[current_price_index-1]:#deltama3_before<deltama3:#self.macd[current_price_index-1]<self.macd[current_price_index] :#and (self.macd[current_price_index-2]<0 or self.macd[current_price_index-1]<0 or self.macd[current_price_index]<0):#and deltama5>=0 :#and deltama3_before<deltama3 ) or (deltama3>0 and deltama3_before<0)):# and  mama3>mama3_b and mama3_b<mama3_b2 and distance_decision==1:#and deltama5<0:#and self.close_price[current_price_index]>self.close_price[current_price_index-1] and deltama5_before<deltama5<0: #and deltama3_before<0 and deltama3>0 and deltama5_before<deltama5<0:#and ma_decision==1 :#and distance_decision==1:#deltama3_before<deltama3:# (deltama3_before>deltama3 or deltama3<0):
    ...
    def run(self,emd_data, datafile,current_index,date,emd_file,emd_data2,emd_data3):
        ...
        emd_data = tmp
        print "\n\n\n"
        print "emd data std %s"%(np.std(emd_data[-50:])/(np.mean(emd_data[-50:])))
        ...
        self.run_predict(imf,current_index,date,datafile,residual,emd_data,imf_open,imf_macd)
