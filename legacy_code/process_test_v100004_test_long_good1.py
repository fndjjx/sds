...
#######
        ...
        if  (self.ma_count<=self.ma_count2<0 and self.ma_count<=-5) or (self.ma_count1<=self.ma_count and self.ma_count1<0 and (np.corrcoef(macd_line,cri_sample)[0][1])<-0.8) or (up_potion>1.5 and (np.corrcoef(macd_line,cri_sample)[0][1])<0 and (np.corrcoef(ma5_line,cri_sample)[0][1])>0) or (((np.corrcoef(ma5_line,cri_sample)[0][1])+ (np.corrcoef(macd_line,cri_sample)[0][1]))/2.0<-0.6 and up_potion>1.5):
            ma_decision3 = 0
 ...
 #       if distance_decision2 ==1 and self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0.01:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
        if     self.buy_price!=0 and down_test_decision1==1 and distance_decision2 ==1 and ((current_price-self.buy_price)/self.buy_price) >0.005:#(imf_extrem_decision6==1 ) :#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            print "open%s close%s"%(self.open_price[current_price_index],current_price)
...
#            self.ma_record=[]
  ...
  #      elif delta_macd1>delta_macd2 and macd_ma_decision==1 and  ma_decision3==1 and  self.buy_price==0 and  up_potion_decision2==1 and    down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
        elif  delta_macd1>delta_macd2 and self.fail_flag==0 and self.buy_price==0 and  down_test_decision1==-1 and delta_buy_decision1==1 and ma_decision3==1  :#imf_extrem_decision6==-1  :
