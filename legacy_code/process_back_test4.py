...
#####
        imf_flag=0
        if self.buy_price==0 and imf_max_index[-1]>imf_min_index[-1] and 488<imf_max_index[-1]<496:
...
#
        ...
        if self.buy_price==0 and imf_min_index[-1]>imf_max_index[-1] and imf_max_index[-1]>488 and imf_min_index[-1]>=497:
                imf_flag=1
        

        if self.buy_price!=0 and imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>=496:
            imf_flag=2



        if self.buy_price!=0 and imf_min_index[-1]>imf_max_index[-1] :#and imf_min_index[-1]<496:
            traindata=imf_process[-55:-5]
            print "traindata %s"%traindata[-10:]
            svm_train(traindata)
            testdata1=traindata[-3:]
...
#
        ...
        if imf_flag==1 :#and self.last_min>imf_min_index[-1]:
            decision=1
        if imf_flag==2:
            decision=2
        
        if imf_min_index[-1]>imf_max_index[-1]:
            self.last_min= imf_min_index[-1]
        else:
            self.last_min= 504
        return decision
        #return decision
...
#######
        ...
        if  ((self.sell_flag==2 and imf_flag!=2) or (self.sell_flag==2 and imf_flag==2 and current_price<self.close_price[current_price_index-1])) and  self.buy_price!=0 :
            ...
            self.one_time_day=0
            self.sell_flag=0
        elif    (imf_flag==1 ) and  self.buy_price==0 and  deltama3>0 :
