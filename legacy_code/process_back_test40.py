...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if   imf1_flag==1 :#and imf1_flag!=2 :#and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
##
    ...
    cdiff=list(np.diff(close_price))
    cdiff.insert(0,0)
    ...
    for i in range(3,len(close_price)):
        diff_ma3_list.append(np.mean(cdiff[i-2:i+1]))

    diff2_ma3_list=[]
    for i in range(3):
        diff2_ma3_list.append(0)
    for i in range(3,len(close_price)):
        diff2_ma3_list.append(np.mean(cdiff2[i-2:i+1]))
...
#####
        ...
        if 0:#ma5<ma10<ma20<ma30:
           ...
           # emd_data.append(open_price[i])
            ...
            if  0:#ts.adfuller(emd_data[-20:], 1)[1]<0.1:
                print "diff1"
                emd_data=(diff_ma3_list[i-500:i])
            else:
                print "diff2"
                emd_data=(diff2_ma3_list[i-500:i])
