...
#        test_data.pop()
        ...
        print "raw test data %s"%test_data_float1[-60:-10]
        test_data_float = test_data_float1[-60:-10]
        ...
        sum = 0
        for i in range(20):
            neuro_prediction = neuroprediction("%s/data_385"%train_data_dir,"net_file",train_number,hidden_number,0,result_number)
 ...
 #       test_data_float2 = test_data_float1[-10:]
 #       fin_result2 = [(recent_price_format3[i]+test_data_float2[i+5])/2 for i in range(len(recent_price_format3))]
 #       print "after add recent price %s"%fin_result2
  
  ##    #######################################################################################
        imf_last_10 = list(imf_list[2])[-10:] 
        fin_result2 = [(fin_result1[i]+imf_last_10[i])/2 for i in range(len(fin_result1))] 
        print "imf last 10 %s"%imf_last_10
        print "fin result2 %s"%fin_result2
  ...
  ##    #######################################################################################
        ...
        print "std 10 %s"%np.std(self.close_price[current_price_index-10:current_price_index])
        std = np.std(self.close_price[current_price_index-10:current_price_index])
        ...
        if (imf[-1]<imf[-2] and imf[-2]>imf[-3]) : #or float(imf_power_before)/imf_power_current > 2.5:
        ...
        if (imf[-1]>imf[-2] and imf[-2]<imf[-3]) and std>0.5: #or float(imf_power_before)/imf_power_current > 2.5:
        ...
        if self.buy_price!=0 and (imf_power_decision == 1) and (trade_price-self.buy_price)/self.buy_price > 0.02 and self.share > 0 :
            print "sell 1"
