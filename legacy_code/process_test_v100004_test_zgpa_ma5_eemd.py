...
##############################################################
     ...
     #       small_imf_max_true_index = small_imf_max_index
        ...
        if small_imf_min_true_index[-1]<small_imf_max_true_index[-1] and 0.8<((small_imf_max_true_index[-1]-small_imf_min_true_index[-1])/float(499-small_imf_max_true_index[-1]))<1.2:
            imf1_flag=1
        if 0:#abs(self.imf1_core-current_imf1_sec_core)>=2:
            imf1_flag=0
...
#######
        ...
        if self.fail_flag>0 and self.fail_flag<10:
            self.fail_flag+=1
        else:
            self.fail_flag=0
...
#            self.ma_record=[]
  ...
  #      elif delta_macd1>delta_macd2 and macd_ma_decision==1 and  ma_decision3==1 and  self.buy_price==0 and  up_potion_decision2==1 and    down_test_decision1==-1 and delta_buy_decision1==1 :#and macd_decision3==1  :#imf_extrem_decision6==-1  :
        elif   self.fail_flag==0 and self.buy_price==0 and  imf1_flag==1 and delta_buy_decision1==1  :#imf_extrem_decision6==-1  :
...
#        (imf, residual) = my_emd.emd(0.03,0.03)
        ...
        (imf1,imf2,imf3)= my_eemd.eemd_process(emd_data,10)
        imf=[]
        imf.append(emd_data)
        imf.append(imf1)
        imf.append(imf2)
