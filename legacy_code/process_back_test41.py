...
####
        ...
        imf1_or_flag=0
        if imf1_min_index[-1]>imf1_max_index[-1] and imf1_min_index[-1]>496 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf1_or_flag=1
        if imf1_min_index[-1]<imf1_max_index[-1] and imf1_max_index[-1]>496 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf1_or_flag=2
...
#            i=1
        ...
        if (imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>496) :#and imf1_or_flag!=2) or (imf1_or_flag==1) :#and ex_flag==1:#and imf_process[-1]<0 :
            imf1_flag=1
        if (imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>495 ) :#or (imf1_or_flag==2):
            imf1_flag=2
...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if   imf2_flag==1 :#and imf1_flag!=2 :#and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
###########################
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,residual,emd_data,imf_open,imf_macd,emd_std,preflag,emd_data2,prechade,kalval,predict):
        ...
        self.snr.append(calc_SNR(emd_data2[-20:],imf_est_list))
...
#            imf_flag=1
        if (imf_close_flag==2 ) or (imf_high_flag==2 ) :#or (imf_high_flag==2 and imf_close_flag!=1):#and imf_high_flag==2:
...
##
        ...
        for i in range(pdata2):
            ma1.append(0)
        for i in range(pdata2,len(emd_data)):
...
###
        ...
        elif boll_de2==2:# and boll_de2==2:
            boll_de=2
...
####
        ...
        if self.de==0 and judge_extrem(emd_data)>=0 and judge_extrem(emd_data[:-1])<0:
            self.de=1
        if self.de2==0 and judge_extrem(emd_data)<=0 and judge_extrem(emd_data[:-1])>0:
            self.de2=1
        ...
        if ts.adfuller(ma_list[current_price_index-10:current_price_index+1], 1)[1]>0.5 and deltama5<0:
            stable_de=0
        if self.buy_price==0 and judge_extrem(emd_data2)>=0 and judge_extrem(emd_data2[:-1])<0:
            self.judge_de.append(1)
        elif self.buy_price!=0 and ((judge_extrem(emd_data)<=0 and judge_extrem(emd_data[:-1])>0) or (judge_extrem(emd_data)<0)):
            self.judge_de.append(2)
        else:
            self.judge_de.append(0)

        print "judge %s"%self.judge_de[-10:]

        power_de_ma_list=0
        if emd_data[-1]-np.mean(emd_data[-5:])>0 and emd_data[-2]-np.mean(emd_data[-6:-1])<0:
            power_de_ma_list=1
        if emd_data[-1]-np.mean(emd_data[-5:])<0 and emd_data[-2]-np.mean(emd_data[-6:-1])>0:
            power_de_ma_list=2
        
        print "compare"
        m1= ma_list[current_price_index]-ma_list[current_price_index-1]
        m2= ma_list[current_price_index-1]-ma_list[current_price_index-2]
        m3= ma_list[current_price_index-2]-ma_list[current_price_index-3]
        print (m1+m2+m3)/3.0
        print emd_data[-1]
        me=[]
        for i in range(len(emd_data)):
            me.append(emd_data[i]*emd_data2[i])
        ...
        max_index = list(argrelextrema(np.array(me),np.greater)[0])
        min_index = list(argrelextrema(np.array(me),np.less)[0])
        max_value = [me[i] for i in max_index]
        min_value = [me[i] for i in min_index]

        print "me max value%s"%max_value
        mean_max_value = np.mean(max_value)
        std_max_value = np.std(max_value)
        print mean_max_value
        print std_max_value

        peak=0
        if me[-1]>(mean_max_value+std_max_value):
            peak=1
        ...
        for i in range(pp):
            ma_me.append(0)
        for i in range(pp,len(emd_data)):
            ma_me.append(np.mean(me[i-(pp-1):i+1]))
        ...
        if  self.buy_price!=0 and me[-1]<me[-2] and me[-2]>me[-3]:#boll_de==2 and self.boll_de[-2]!=2:#  me[-1]<me[-2] and me[-2]>me[-3]:#power_de_ma_list==2:#((judge_extrem(emd_data)<=0 and judge_extrem(emd_data[:-1])>0) or (judge_extrem(emd_data)<0)):#self.de2==1 and emd_data[-1]<emd_data[-2]:#((judge_extrem(emd_data[:-1])>0 and judge_extrem(emd_data)<=0) ):#imf_flag==2:# (  (len(self.boll_de)>1 and self.boll_de[-2]==2 and self.boll_de[-1]!=2) or (len(self.boll_de)>1 and self.boll_de[-2]!=1 and self.boll_de[-1]==1)):#( self.pre_cha_de[-1]==1 or imf_flag==2) :#and (boll_de==2 or self.buy_day>10):#(boll_de==2 or (deltama5<0 and deltama5_before<0)):#emd_data[-1]<emd_data[-2]<emd_data[-3]  and emd_data[-3]>emd_data[-4] :# (r (  emd_data[-3]<emd_data[-2] and emd_data[-1]<emd_data[-2] and emd_data[-1]>0)):#imf_flag==2:#boll_de==2:# self.close_flag[-2]==2 and imf_flag!=2:# and self.boll_de[-1]!=2 and len(self.boll_de)>1 and self.boll_de[-2]==2:#boll_de==2:#((boll_de!=2 and self.boll_de[-2]==2) or ((imf_flag==2) and deltama3_before<0 and deltama3<0))  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
            ...
            if self.fail_flag==0:
                ...
                print "sellsell 1"
                self.buy_price=0
        elif self.buy_price!=0 and ((((self.buy_price-trade_price)/self.buy_price)>0.03 ) )  :#and self.share > 0 :
            print "sell sell 4"
            if self.fail_flag==0:
                if self.buy_price>=trade_price:
                    self.fail_flag=0
...
#            self.last_as=self.total_asset
    ...
    def run(self,emd_data, datafile,current_index,date,emd_file,emd_data2,emd_data3,preflag,chazhi,prechade,kalval,predict):
        ...
        #my_emd = one_dimension_emd(emd_data2,3)
        #(imf, residual) = my_emd.emd_mean_pro(0.03,0.03)
...
#
       ...
       # data1 = ma_me
        ...
        plt.subplot(412).axis([start,len(data1),min(data1[start:]),max(data1[start:])])      
        ...
        plt.plot([i for i in range(len(data1))],data1,'o',[i for i in range(len(data1))],data1,'b',self.peak,peak_value,'yo')             
...
##
    ...
    for i in range(len(close_price)):
        cdiff.append(close_price[i]-close_price[i-1])
    cdiff=list(np.diff(ma_list))
    cdiff.insert(0,0)
...
#####
        ...
        if 0:#ma5<ma10<ma20<ma30:
           ...
           # emd_data.append(open_price[i])
            ...
            zhi10=close_price[i-1]+close_price[i-2]+close_price[i-3]+close_price[i-4]+close_price[i-5]+close_price[i-6]+close_price[i-7]+close_price[i-8]+close_price[i-9]
            zhi10=zhi10+pp2
            zhi5=zhi5+pp2
            ...
            print "ma3 kal %s %s"%(ma3_list[i],k_v[i])
            #emd_data.append(k_v[i])
           # emd_data.append(k_v1[i])

            print "emd data pre cha %s"%chazhi[-10:]

            zhi5=close_price[i-1]+close_price[i-2]+close_price[i-3]+close_price[i-4]
            zhi5=zhi5+close_price[i]
           # zhi5=zhi5+k_v[i]
            zhi2=close_price[i-1]+close_price[i]
           # zhi2=close_price[i-1]+k_v[i]
            rz=zhi2/2.0-zhi5/5.0
