...
##############################################################
        ...
        coef_choice=0
        if coef_list2[2]>0.7 and self.buy_price==0 :
            coef_choice=2
#        elif (self.buy_price==0 and coef_list2[2]<0.85 and coef_list1[1]<coef_list1[0] and coef_list1[0]>0.8):
#            coef_choice=3
#        elif self.buy_price!=0 or (self.buy_price==0 and coef_list2[2]<0.85 and coef_list1[1]>coef_list1[0] ):#and coef_list1[1]>0):#and coef_list1[2]>0:
        elif coef_list2[2]<0.7: 
            coef_choice=1
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    if small_small_imf_max_index[-1]<small_small_imf_min_index[-1]:#len(small_index_max)>0 and  ((price_max_data[-1]-price_max_data[small_index_max[-1]-imf_min_true_index[-1]])/price_max_data[small_index_max[-1]-imf_min_true_index[-1]])>-0.02 :

                        small_small_index_max.append(499)
                    else:
                        small_small_index_min.append(499)
                  ...
                  #          min_index.append(sum(small_index_min)/len(small_index_min))
                    ...
                    if len(small_index_max)>0 and( coef_choice==1 or coef_choice==3):
                        print "enter 1"
                        if len(small_index_max)>2:
                            max_index.append(sum(small_index_max[:2])/len(small_index_max[:2]))
                        else:
                            max_index.append(sum(small_index_max[:])/len(small_index_max[:]))
                        small_index_min = list(filter(lambda n:n>max_index[-1], small_index_min))
                    ...
                    if 0:#len(small_small_index_max)>0 and coef_choice==3:
                        print "enter 3"
                        if len(small_small_index_max)>3: 
                        ...
                        small_small_index_min = list(filter(lambda n:n>max_index[-1], small_small_index_min))
...
#                        imf_extrem_decision6 = 0
            ...
            #            imf_extrem_decision = -1
             ...
             #           residual = price_min_data
                    ...
                    if small_small_imf_min_index[-1]<small_small_imf_max_index[-1]:#len(small_index_max)>0 and  ((price_max_data[-1]-price_max_data[small_index_max[-1]-imf_min_true_index[-1]])/price_max_data[small_index_max[-1]-imf_min_true_index[-1]])>-0.02 :
                        small_small_index_min.append(499)
                    else:
                        small_small_index_max.append(499)
                  ...
                  #          max_index.append(sum(small_index_max)/len(small_index_max))
                    ...
                    if len(small_index_min)>0 and (coef_choice==1 or coef_choice==3):
                        print "enter 1"
                        print small_index_max
                        ...
                        if len(small_index_min)>2:
                            ...
                            min_index.append(sum(small_index_min[:])/len(small_index_min[:]))
                        print min_index
                        small_index_max = list(filter(lambda n:n>min_index[-1], small_index_max))
                        if len(small_index_max)>0:
                            max_index.append(sum(small_index_max[:])/len(small_index_max[:]))
                        if min_index!=[] and max_index!=[]:
                    ...
                    if 0:#len(small_small_index_min)>0 and coef_choice==3:
                        print "enter 3"
                        if len(small_small_index_min)>3:
                            min_index.append(sum(small_small_index_min[:3])/len(small_small_index_min[:3]))
                        else:
                            min_index.append(sum(small_small_index_min[:])/len(small_small_index_min[:]))
                        small_small_index_max = list(filter(lambda n:n>min_index[-1], small_small_index_max))
...
#########
        ...
        if macd_decision2==1 and distance_decision2 ==1 and self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            ...
            self.holdtime = 0
            once_flag = 1
        elif  self.buy_price!=0 and (((self.buy_price-trade_price)/self.buy_price)>0.05 )  and self.share > 0 :
            print "sell sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
