...
######
        ...
        #if deltama5<0 and deltama10<0 and deltama20<0 and deltama25<0 and deltama30<0:
        #    ma_decision=11
        #kdj_de=0
        #if ma_decision==0 and current_k>before_k:
        #    kdj_de=1
        #if ma_decision==11 and current_d>before_d:
        #    kdj_de=1
...
####
        ...
        if wrma3>60:
            wr_de=1
####
        if ma_decision==0 and wr_de==1  and self.buy_price==0 and (boll_de==1 ) and imf_flag==1:
            self.f_d=1
        elif self.buy_price!=0:
            self.f_d=0
        print "selffd %s"%self.f_d
######
        ...
        if np.std(emd_data[-30:])>0.5:
            std_de=1
        print "std_de%s"%std_de
        print "ma_decision %s"%ma_decision
        #if  self.buy_price!=0   and ( (deltama5_before2>deltama5_before>deltama5 ))  and ((current_price-self.buy_price)/self.buy_price)>0.01:
        flag_fail=1
        if self.buy_price!=0 and distance_decision2==1 and ((boll_de==2) or ((imf_flag==2) and deltama3_before<0 and deltama3<0 and current_price<self.close_price[current_price_index-1])):
