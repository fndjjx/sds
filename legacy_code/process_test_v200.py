                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    if max_index != []:
                        ...
                        if min_index!=[]:
                            ...
                            dis = tmp[-1]
                        if (tmp[0] in max_index) and tmp[0]>4 and (tmp[-1] in max_index) and len(data)-max_index[-1]>mean_max_min and len(data)-max_index[-1]<mean_max_min+2:#+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = -1
                        if (tmp[-1] in min_index) and len(data)-min_index[-1]>(mean_min_max)-std_min_max and len(data)-min_index[-1]<(mean_min_max)+std_min_max:#std_max_min and len(data)-max_index[-1]<dis+std_max_min:#+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = 1
                    if min_index != []:
                        #if (tmp[0] in max_index) and tmp[0]>4 and (tmp[-1] in min_index)  and len(data)-min_index[-1]<6:
                        print len(data)-min_index[-1]
                        if (tmp[-1] in min_index) and len(data)-min_index[-1]>(mean_min_max)-std_min_max and len(data)-min_index[-1]<(mean_min_max)+std_min_max:
                            imf_extrem_decision6 = 1
            ...
            #            imf_extrem_decision = -1
             ...
             #           residual = price_min_data
                    ...
                    if max_index != []:
                        ...
                        if (tmp[-1] in max_index) and len(data)-max_index[-1]>(mean_max_min)-1 and len(data)-max_index[-1]<mean_max_min+2 :#+2*std_max_min :#and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
                            imf_extrem_decision6 = -1
                    if min_index != []:
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif  self.buy_price==0 and   imf_extrem_decision6==-1  and power_decision==-1:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
