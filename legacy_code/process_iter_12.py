...
class process_util():
   ...
   #         fp.writelines("\n")
    ...
    def run_predict_price_period(self,imf,current_price_index,residual,imf_next,date,datafile,imf1):
        ...
        close_max_index = argrelextrema(np.array(self.close_price[:current_price_index]),np.greater)[0]
        close_min_index = argrelextrema(np.array(self.close_price[:current_price_index]),np.less)[0]
        if close_max_index[-1]>close_min_index[-1]:
            close_trust = close_min_index[-1]
        else:
            close_trust = close_max_index[-1]

        imf0_trust_max_index = argrelextrema(np.array(imf_next[:close_trust]),np.greater)[0]
        imf0_trust_min_index = argrelextrema(np.array(imf_next[:close_trust]),np.less)[0]

        if imf0_trust_max_index[-1]>imf0_trust_min_index[-1]:
            imf0_trust = imf0_trust_min_index[-1]
        else:
            imf0_trust = imf0_trust_max_index[-1]


        imf1_trust_max_index = argrelextrema(np.array(imf1[:imf0_trust]),np.greater)[0]
        imf1_trust_min_index = argrelextrema(np.array(imf1[:imf0_trust]),np.less)[0]
 
        print "imf0 trust max index %s "%imf0_trust_max_index 
        print "imf0 trust min index %s "%imf0_trust_min_index 

        print "close price max %s "%close_max_index
        print "close price min %s "%close_min_index
        guai_decision = 0
        if abs(close_max_index[-1]-close_min_index[-1]) == 1 and (current_price_index-close_max_index[-1] <= 2 or current_price_index-close_min_index[-1] <= 2): 
            delta1 = imf[current_price_index-3]-imf[current_price_index-2]     
            delta2 = imf[current_price_index-4]-imf[current_price_index-3]     
            delta3 = imf[current_price_index-5]-imf[current_price_index-4]     
            delta4 = imf[current_price_index-6]-imf[current_price_index-5]     
            delta_delta = delta2-delta1
            delta_delta2 = delta3-delta2
            print "delta guai imf %s "%delta2
            print "delta delta guai %s "%delta_delta
            print "delta delta guai before %s "%delta_delta2
            print "guai last imf max %s "%((current_price_index-3)-imf_max_index[-1])
            if delta_delta < 0 and self.delta_guai>0:
                guai_decision = 1
            else:
                guai_decision = 0
            self.delta_guai = delta_delta
        else:
            self.delta_guai = 0
#######################################################
#        if imf1_trust_max_index[-1]>imf1_trust_min_index[-1]:
#            now_last_last_close_extre = current_price_index-imf1_trust_min_index[-1] 
#        else:
#            now_last_last_close_extre = current_price_index-imf1_trust_max_index[-1] 
#        print "now_last_last_close_extre %s "%now_last_last_close_extre
#        print "current price index %s"%current_price_index
#        print "last_last_close_extre %s"%(current_price_index-now_last_last_close_extre)
#        print "imf %s"%imf[current_price_index]
#        print "imf last last extre %s"%imf[current_price_index-now_last_last_close_extre]
#        print "imf last last extre -1 %s"%imf[current_price_index-now_last_last_close_extre-1]
#        delta1 = imf[current_price_index-now_last_last_close_extre-1]-imf[current_price_index-now_last_last_close_extre]     
#        delta2 = imf[current_price_index-now_last_last_close_extre-2]-imf[current_price_index-now_last_last_close_extre-1]     
#        delta3 = imf[current_price_index-now_last_last_close_extre-3]-imf[current_price_index-now_last_last_close_extre-2]     
#        delta_delta = delta2-delta1
#        delta_delta2 = delta3-delta2
#        print "delta guai imf %s "%delta1
#        print "delta delta guai %s "%delta_delta
#        print "delta delta guai before %s "%delta_delta2
#        last_imf_max_guai = ((current_price_index-now_last_last_close_extre)-imf_max_index[-1])
#        print "guai last imf max %s "%((current_price_index-now_last_last_close_extre)-imf_max_index[-1])
#        if delta1<0 and delta2>0:
#            guai_decision = 1
#        else:
#            guai_decision = 0
#        self.delta_guai = delta1
#
...
#        elif self.vol[current_price_index] < 2*self.vol[current_price_index-1] and power_decision != 1 and (now_position_last_min_delta == 1 or now_position_last_min_delta == 2) and (self.ma(self.close_price, 10, 0, current_price_index)-self.ma(self.close_price, 10, -1, current_price_index))>0 and self.vol[current_price_index]>vol_ma5 and self.open_price[current_price_index]<self.close_price[current_price_index] and imf_power_decision == -1  and self.money>0 :#and self.vol[current_price_index]>self.vol[current_price_index-1] and self.money>0:
        elif guai_decision == 1 and self.money>0:
            print "buy buy 1"
            self.share = self.buy(self.money,trade_price)
            self.money = 0
            self.buy_price = trade_price
...
#            tomorrow_will = -1
   ...
   #         imf.pop()
    ...
    #    imf = imf_without
        ...
        self.run_predict_price_period(imf,current_index,residual,imf_raw,date,datafile,imf_raw1)
        endtime = datetime.datetime.now()
        print "run time"
        print (endtime - starttime).seconds
