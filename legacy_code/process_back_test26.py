...
####
        ...
        for i in diffren100:
            if i<0:
                down_count+=1
        prechabig0=0
        if len(self.pre_cha)>2 and self.pre_cha[-3]>0 and self.pre_cha[-2]<0:
            prechabig0=1
        ...
        if   self.buy_price!=0 and ((len(self.pre_cha_de)>1 and self.pre_cha_de[-1]!=-1 and self.pre_cha_de[-2]==-1 ) or (self.pre_cha[-2]>0)):
