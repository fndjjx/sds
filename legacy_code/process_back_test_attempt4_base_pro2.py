...
##
        ...
        ma5=ma_func2(self.close_price,5) 
        p,m,s=montecarlo_simulate(self.close_price[current_price_index-20:current_price_index+1],100)
        ...
        mpp,mpm,mps=montecarlo_simulate(mp_list[current_price_index-20:current_price_index+1],100)
...
###
        ...
        sel = 1
        if sel==1:
            if  diffmp[-1]<0 and mean_de==1 and   len(self.mclist2)>0 and self.mclist2[-1]>0 and self.mclist5[-1]>0 :#and self.mclist3[-1]>self.close_price[current_price_index]:#and (( self.fail_flag==0) or (self.fail_flag==1 and rdde==1)):
                jede=1
            if diffmp[-1]>0 and  mean_de==2:
                jede=2
        elif 0:#sel==2:
            ...
            if ma3_mp_diff[-1]>0:#np.mean(ma3_mp_diff[-10:]) :#and np.mean(ma3_mp_diff[-3:])>0:#self.close_price[current_price_index]>mp and  mean_de==2:
                jede=2
        elif sel==2 :#and not (self.vol[current_price_index]<self.vol[current_price_index-1] and self.close_price[current_price_index]<self.open_price[current_price_index]):#self.sel%2==1:#s1<s2:#np.std(rp1)/np.mean(rp1)>np.std(rp)/np.mean(rp):
