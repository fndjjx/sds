            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    if 0:#len(small_index_max)>0 and  ((price_max_data[-1]-price_max_data[small_index_max[-1]-imf_min_true_index[-1]])/price_max_data[small_index_max[-1]-imf_min_true_index[-1]])>-0.02 :
                  ...
                  #          min_index.append(sum(small_index_min)/len(small_index_min))
                    ...
                    if len(small_index_max)>0 and percentage_choice==3:
                        if len(small_index_max)>2 :
                            max_index.append(sum(small_index_max[:2])/len(small_index_max[:2]))
                        elif len(small_index_max)<=2 :
                            max_index.append(sum(small_index_max)/len(small_index_max))
                    ...
                    if len(small_index_max)>0 and percentage_choice==2:
                        max_index.append(sum(small_index_max[:])/len(small_index_max[:]))

                    if len(small_index_max)>0 and percentage_choice==1:
            ...
            #            imf_extrem_decision = -1
             ...
             #           residual = price_min_data
                  ...
                  #          max_index.append(sum(small_index_max)/len(small_index_max))
                    ...
                    if len(small_index_min)>0 and percentage_choice==3:
                        if len(small_index_min)>2 :
                            min_index.append(sum(small_index_min[:2])/len(small_index_min[:2]))
                    ...
                    if len(small_index_min)>0 and percentage_choice==2:
                        min_index.append(sum(small_index_min[:])/len(small_index_min[:]))

                    if len(small_index_min)>0 and percentage_choice==1:
                        min_index.append(small_index_min[0])
                        small_index_max = list(filter(lambda n:n>min_index[-1]+2, small_imf_max_index))
                        if len(small_index_max)>0:
                            max_index.append(small_index_max[0])
...
#########
        ...
        if self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((trade_price-self.buy_price)/self.buy_price) >0:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif   small_imf_decision3 == -1 and self.buy_price==0 and   imf_extrem_decision6==-1 :# and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2] :#and small_imf_decision1 == -1:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
