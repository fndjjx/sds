...
#
        ...
        #    ma_ex_de=2
        if small_num>0 and (large_num/float(small_num))>3:
            ma_ex_de=1
...
########
        ...
        boll_high1_b3 = k2*np.std(self.close_price[current_price_index-period-3:current_price_index-2])+np.mean(self.close_price[current_price_index-period-3:current_price_index-2])
        boll_high1_b4 = k2*np.std(self.close_price[current_price_index-period-4:current_price_index-3])+np.mean(self.close_price[current_price_index-period-4:current_price_index-3])
        ...
        boll_low1_b3 = k1*(-np.std(self.close_price[current_price_index-period-3:current_price_index-2]))+np.mean(self.close_price[current_price_index-period-3:current_price_index-2])
        boll_low1_b4 = k1*(-np.std(self.close_price[current_price_index-period-4:current_price_index-3]))+np.mean(self.close_price[current_price_index-period-4:current_price_index-3])
        ...
        print "widthb3 %s"%(boll_high1_b3-boll_low1_b3)
        print "widthb4 %s"%(boll_high1_b4-boll_low1_b4)
        self.close_price=tmp
        current_price=self.close_price[current_price_index]


        width_b4=boll_high1_b4-boll_low1_b4
        width_b3=boll_high1_b3-boll_low1_b3
        width_b2=boll_high1_b2-boll_low1_b2
        width_b1=boll_high1_b-boll_low1_b
        width=boll_high1-boll_low1


        mean1=(width+width_b1+width_b2)/3.0
        mean2=(width_b1+width_b2+width_b3)/3.0
        print "mean %s %s"%(mean1,mean2)

        kd_de=0
        #if ((deltama3+deltama3_before)<(deltama5+deltama5_before) or (deltama5+deltama5_before)<(deltama9+deltama9_before)) and mean1>mean2:
        #if ((deltama3+deltama3_before+deltama3_before2)<(deltama5+deltama5_before+deltama5_before2)) and mean1>mean2:
        if ((deltama3+deltama3_before)<(deltama5+deltama5_before)) and mean1>mean2:
        #if (deltama5+deltama5_before)<(deltama10+deltama10_before) and mean1>mean2:
            kd_de=1
...
######
        ...
        flag_fail=1
        if  self.buy_price!=0  and (boll_de==2 or ((imf_flag==2) and deltama3_before<0 and deltama3<0 and current_price<self.close_price[current_price_index-1]))  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
