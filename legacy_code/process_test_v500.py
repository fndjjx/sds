...
#                count = 10
   ...
   #         return -1
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,train_data_dir,train_number,hidden_number,result_number):
        ...
        coef_list=[]
        price_recent = self.close_price[current_price_index-49:current_price_index+1]
        for i in range(len(imf_list)):
            coef_list.append(np.corrcoef(price_recent,imf_list[i][-50:])[0][1])
            print "imf%s data coef %s"%(i,np.corrcoef(price_recent,imf_list[i][-50:])[0][1])
...
##############################################################
        ...
        if len(list(price_max_index)) > 0 and len(list(price_min_index)) > 0 :
            ...
            if chazhi[-1]<5 or chazhi[-2]<5 or chazhi[-3]<5:
                chazhi_decision=0
...
################
                    ...
                    if np.corrcoef(price_max_min_list,imf_max_min_list)[0][1]>0 and np.corrcoef(price_max_min_list2,price_min_max_list2)[0][1]<0 and 499-imf_max_true_index[-1]>mean_max_min-1 and 499-imf_max_true_index[-1]<mean_max_min+2*std_max_min:
                    ...
                    print "price price coef %s"%np.corrcoef(price_max_min_list[:length],price_min_max_list[:length])[0][1]
                    print "imf_extrem_decision9 %s"%imf_extrem_decision9
...
#####################################################################################################################
        ...
        this_buy = 0
        if  imf_extrem_decision9 == -1:#  (current_price-ma5) > (self.close_price[current_price_index-1]-self.ma(self.close_price, 5, -1, current_price_index)) and ( imf_extrem_decision6 == -1):
            this_buy = -1
        if this_buy == -1:
            self.buy_decision=-1
        ...
        if self.buy_price!=0 and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            ...
            self.holdtime = 0
            once_flag = 1
        elif self.buy_price!=0 and (((self.buy_price-trade_price)/self.buy_price)>0.02 )  and self.share > 0 :
            print "sell sell 1"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif   coef_decision1==1 and  self.buy_price==0 and   self.buy_decision == -1 :#and   power_ma5_decision==-1 :#and power_ma5_decision==-1:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
            print "buy buy 1"
