...
###################
        ...
        last_50_price = map(lambda x:x-np.mean(last_50_price),last_50_price)
        ...
        self.imf_std.append((np.std(last_50_price)))
        #std_mean = (np.std(last_50_price)/np.mean(last_50_price))
        std_mean = (np.std(last_50_price))
        mean_std_mean = (np.mean(self.imf_std))
        std_std_mean = (np.std(self.imf_std))
        last_100_price = self.close_price[current_price_index-20:current_price_index+1]
        last_100_price = map(lambda x:x-np.mean(last_100_price),last_100_price)
        #std_mean_100 = (np.std(last_100_price)/np.mean(last_100_price))
        std_mean_100 = (np.std(last_100_price))

        print "100 std/mean %s"%(np.std(last_100_price))
        
        print "std mean %s"%(np.mean(self.imf_std))
        print "std std %s"%(np.std(self.imf_std))
        choice=0
        if ((std_mean-mean_std_mean)/mean_std_mean)<-0.1 or ((std_mean-mean_std_mean)/mean_std_mean)>0.2:
        #if ((std_mean-std_mean_100)/std_mean_100)<-0.1:
            choice=1 
            ...
            if 1:#self.buy_price == 0:# and abs(self.last_true_max-imf_max_true_index[-1])<10 and abs(self.last_true_min-imf_min_true_index[-1])<10:#(buyhigh/buyhigh2)>0.3 and (buyhigh/buyhigh2)<3:
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                 ...
                 #   print "choice %s"%choice
                    ...
                    if imf_min_true_index[-1]-small_index_max_remove[-1]<3:
                  ...
                  #          min_index.append(sum(small_index_min)/len(small_index_min))
                    ...
                    if len(small_index_max)>0 and (choice==1 or choice==2):
                        ...
                        if len(small_index_min)==1 :#and small_index_min[0]<498:
                            small_index_min = list(filter(lambda n:n>=max_index[-1], small_imf_min_index))
            ...
            #            imf_extrem_decision = -1
             ...
             #           residual = price_min_data
                  ...
                  #  print "choice %s"%choice
                    ...
                    if imf_max_true_index[-1]-small_index_min_remove[-1]<3:
                  ...
                  #          max_index.append(sum(small_index_max)/len(small_index_max))
                    ...
                    if len(small_index_min)>0 and (choice==1 or choice==2):
                        ...
                        if len(small_index_max)==1 :#and small_index_max[0]<498:
                            small_index_max = list(filter(lambda n:n>=min_index[-1], small_imf_max_index))
