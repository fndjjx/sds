...
class process_util():
    ...
    def __init__(self,open_price,close_price,macd,date):
   ...
   #         fp.writelines("\n")
    ...
    def run_predict_price_period(self,imf,current_price_index,residual,imf_next,datafile):
...
#######################################
        ...
        if self.macd[current_price_index-1]<self.macd[current_price_index-2] and self.macd[current_price_index-2]<self.macd[current_price_index-3] and self.macd[current_price_index-1]<self.macd[current_price_index]:
            macd_decision1 = 1
        elif self.macd[current_price_index-1]>self.macd[current_price_index-2] and self.macd[current_price_index-2]<self.macd[current_price_index-3] and self.macd[current_price_index-1]<self.macd[current_price_index]:
            macd_decision1 = 1
        elif self.macd[current_price_index-1]>self.macd[current_price_index-2] and self.macd[current_price_index-2]>self.macd[current_price_index-3] and self.macd[current_price_index-1]<self.macd[current_price_index]:
            macd_decision1 = 1
        else:
        ...
        std_list = self.close_price[current_price_index-19:current_price_index+1]
        std_list_before = self.close_price[current_price_index-20:current_price_index]
        std_list_before2 = self.close_price[current_price_index-21:current_price_index-1]
        print "std 20 %s"%np.std(std_list)
        print "std 20 before %s"%np.std(std_list_before)
        print "std 20 before2 %s"%np.std(std_list_before2)


        std_decision = 0
        if np.std(std_list) > 0.5:
            std_decision = 1
        else:
            std_decision = 0
...
#            self.sell_y_index.append(current_price)
        tomorrow_will = 0
        if rsi_decision1 == 1 and macd_decision3 == 1 and extrem_decision == 1 and self.buy_price < self.close_price[current_price_index] and  ex_decision5 == 1 :
            print "sell 1"
            tomorrow_will = 1
        if macd_decision4 == 1  :
            print "sell 2"
...
#            self.last_buy_flag = 1
        ...
        elif std_decision == 1 and macd_decision7 == 1 and macd_decision6 == 1 and macd_decision5 == 1 and ma_decision9 == 1 and (now_position_last_min_delta == 1 or now_position_last_min_delta == 2) and extrem_decision == -1 and ex_decision1 == 1  and mix_decision1 == 1 : #and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 5, -1, current_price_index)) :#and imf_power_decision == -1 and power_decision == -1 and self.money>0 :
            print "buy 1"
            tomorrow_will = -1
        elif  std_decision == 1 and power_decision == -1 and imf_power_decision == -1 and (now_position_last_min_delta == 1 or now_position_last_min_delta == 2) and extrem_decision == -1 and ex_decision1 == 1 and macd_decision1 == 1 and macd_5 > -0.28: #and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 5, -1, current_price_index)) :#and imf_power_decision == -1 and power_decision == -1 and self.money>0 :
            print "buy 2"
            tomorrow_will = -1
...
#            self.last_buy_flag = 1
        ...
        if os.path.exists("decision"):
            ...
            fp = open("decision",'w')
            lines.append("\n")
            lines.append(self.date[current_price_index])
            lines.append(" ")
  ...
  #      power_next = result - imf_ma3_next
   ...
   #         imf.pop()
        ...
        self.run_predict_price_period(imf,current_index,residual,imf,datafile)
...
#    open_price_raw = table.col_values(1)
#    close_price_raw = table.col_values(4)
#    macd_raw = table.col_values(19)
#    date = date[34:] 
#    open_price_raw = open_price_raw[34:]
#    close_price_raw = close_price_raw[34:]
#    macd_raw = macd_raw[34:]
#
#    open_price = [float(i) for i in open_price_raw]
#    close_price = [float(i) for i in close_price_raw]
#    macd = [float(i) for i in macd_raw]
#
#    print "len open price %s"%len(open_price)
#    print "len close price %s"%len(close_price)
#    print "len macd %s"%len(macd)
...
#        macd.append(float(eachline.split("\n")[0]))
    process = process_util(open_price,close_price,macd,date)
    os.popen("rm -r ../../emd_data")
    os.popen("mkdir ../../emd_data")
    f = os.popen("wc -l %s"%datafile)
    row_num = f.readline().split(" ")[0]
    f.close()
    begin = int(row_num)-3
...
#    #    print "emd_data %s"%emd_data
        process.run(emd_data,datafile,i-1,date)
    process.peilv()
    process.hold_time()
