...
##############################################################
        ...
        for i in range(len(imf_list)):
            coef_list.append(np.corrcoef(price_recent,imf_list[i][-5:])[0][1])
        ...
        for i in range(len(imf_list)):
            coef_list.append(np.corrcoef(price_recent,imf_list[i][-10:])[0][1])
        ...
        coef_decision1 = 0
        if ((np.corrcoef(price_recent,imf_list[2][-20:])[0][1])>0.6 and (np.corrcoef(self.close_price[current_price_index-9:current_price_index+1],imf_list[1][-10:])[0][1])>0.5 and (np.corrcoef(self.close_price[current_price_index-9:current_price_index+1],imf_list[2][-10:])[0][1])>0) or ((np.corrcoef(price_recent,imf_list[2][-20:])[0][1])>0.7) or ((np.corrcoef(self.close_price[current_price_index-9:current_price_index+1],imf_list[2][-10:])[0][1])>0.7):
            coef_decision1 = 1
        elif (np.corrcoef(price_recent,imf_list[2][-20:])[0][1])>0.5 and (np.corrcoef(self.close_price[current_price_index-9:current_price_index+1],imf_list[1][-10:])[0][1])<0.5:
            coef_decision1 = 2
                ...
                if imf_max_true_index[-1]<imf_min_true_index[-1]:# and 499-imf_min_true_index[-1]>(mean_min_max):# and imf_min_true_index[-1]>465:# and 499-imf_max_true_index[-1]<(mean_min_max+mean_max_min):# and (499-imf_min_true_index[-1])>(mean_max_min+std_max_min)) :
                   ...
                   #     data = [rawdata[i+1]-rawdata[i] for i in range(len(rawdata)-1)]
                    ...
                    if coef_decision1==2:
                        ...
                        max_index=imf_max_remove_index
                        min_index=imf_min_remove_index
                        max_index = list(filter(lambda n:n<496, max_index))
                        min_index = list(filter(lambda n:n<496, min_index))
                    ...
                    if tmp!=[] and max_index != []:
                        if (tmp[0] in max_index) and tmp[0]>4 and (tmp[-1] in max_index) and 499-max_index[-1]>mean_max_min-1 and 499-max_index[-1]<mean_max_min+2:#+2*std_max_min:# and len(data)-max_index[-1]<mean_max_min+2*std_max_min:
...
#####################################################################################################################
        ...
        if self.buy_price!=0 and (imf_extrem_decision6==1 ) and ((current_price-self.buy_price)/self.buy_price) >0.01:#and self.sell_decision==-1 and   ((self.macd[current_price_index]>0 and self.macd[current_price_index]<self.macd[current_price_index-1]<self.macd[current_price_index-2]) or (self.macd[current_price_index]<self.macd[current_price_index-1] and self.macd[current_price_index]<0)) and self.share > 0 :
            print "sell sell 1"
            ...
            self.holdtime = 0
            once_flag = 1
        elif  self.buy_price!=0 and (((self.buy_price-trade_price)/self.buy_price)>0.03 )  and imf_extrem_decision6!=-1 and self.share > 0 :
            print "sell sell 2"
            self.money = self.sell(self.share,trade_price)
            self.share = 0
            self.buy_price = 0
...
#            once_flag = 1
  ...
  #          self.holdtime = 0
  #          once_flag = 1
        elif coef_decision1==1 and fall_decision1==1 and ( self.ma_count2>=9 or self.ma_count>=3) and  self.buy_price==0 and   imf_extrem_decision6==-1  and  up_decision1 == 1 :#and ma10>ma30:#imf_extrem_decision8 == 1 and buy_decision2 == -1:# and  self.close_price[current_price_index-1]<self.close_price[current_price_index]:# and imf_below_0_decision == -1:# and imf_power_decision == -1:
