...
class process_util():
    ...
    def run_predict1(self,imf_list,current_price_index,date,datafile,residual,imf_open,imf_macd,sel_flag,emd_data,emd_data2,use):
        ...
        for i in range(len(imf_list)):
            ...
            jiange.append(np.mean(np.diff(tmp)))
            print "imf %s"%i
            print "max %s"%max_index
            print "min %s"%min_index
        print "hahahahaha%s"%(calc_SNR(emd_data,imf_list)[1]-1)
        
        #if (calc_SNR(emd_data,imf_list)[1]-1)==1:
        #    use=1
        #if (calc_SNR(emd_data,imf_list)[1]-1)==2:
        #    use=2
        ###if jiange[2]<6:
        #if (calc_SNR(emd_data,imf_list)[1]-1)==3:
        #    use=3
        #if (calc_SNR(emd_data,imf_list)[1]-1)==4:
        #    use=4
####
...
#       #     (imf_process,extenflag1,ex_num) = matchlist3(imf_process)
        ...
        imf2_max_index = list(argrelextrema(np.array(imf_list[use]),np.greater)[0])
        imf2_min_index = list(argrelextrema(np.array(imf_list[use]),np.less)[0])
        ...
        if i==1 :#or tmp[-2]<990:
            (imf_process,extenflag1,ex_num,cha,cha2) = matchlist1(imf_list[use],3,emd_data)
        elif i==2:
            (imf_process,extenflag1,ex_num,cha,cha2) = matchlist2(imf_list[use],3,emd_data)
        else:
            (imf_process,extenflag1,ex_num,cha,cha2) = matchlist3(imf_list[use],3,emd_data)
...
###########################
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,residual,emd_data,imf_open,imf_macd,emd_std,preflag,emd_data2):
        ...
        power_imf =  self.imf_percentage(imf_list[-20:],residual,self.close_price[current_price_index-19:current_price_index+1])
        print power_imf
        ma5 = []
        kkk=10
        for i in range(kkk):
...
#
        ...
        print "ma5 %s"%ma5[-10:]
        ma_max_index = list(argrelextrema(np.array(ma5[-30:]),np.greater)[0])
        print " ma max index %s"%ma_max_index
        ma_min_index = list(argrelextrema(np.array(ma5[-30:]),np.less)[0])
        ...
        tmp.sort()
        print tmp
        print "mean diff%s"%np.mean(np.diff(tmp))
        use=0
        if len(ma_max_index)+len(ma_min_index)>3:
            use=2
        else:
            use=3

        if power_imf[2]>power_imf[3]:
            use=2
        else:
            use=3
        (imf_close_flag,last_max)=self.run_predict1(imf_list,current_price_index,date,datafile,residual,imf_open,imf_macd,2,emd_data,emd_data2,use)
...
####
    ...
    def run(self,emd_data, datafile,current_index,date,emd_file,emd_data2,emd_data3,preflag):
        ...
        my_emd = one_dimension_emd(emd_data,5)
