...
###
    ...
    #        ma_list=self.buy_ma
        ...
        sel=0
        if self.buy_price==0:
            if np.std(diffmp[-20:])>0.15:#self.pimf1[-1][1]>0.4:#self.cu[-1]>0.6 or (len(self.cu)>1 and self.cu[-1]>self.cu[-2]):
                sel=1
            else:
                sel=2
            self.sel=sel
      ...
      #  print c2/float(c1)
       ...
       # self.radio1.append(c2/float(c1))
        ...
        mean_de=0
        if np.mean(diffmp[-3:])<0:#>np.mean(diffmp[-10:]) :#and np.mean(diffmp[-5:])>np.mean(diffmp[-6:-1]):
            mean_de=1
        else:
            mean_de=2
        #sel=0
...
###
        ...
        if sel==1:#self.sel%2==1:#s1<s2:#np.std(rp1)/np.mean(rp1)>np.std(rp)/np.mean(rp):
            if self.close_price[current_price_index]<mp :#and self.close_price[current_price_index-1]<mp1:
                jede=1
...
#        data11 = self.je
        ...
        plt.plot([i for i in range(len(data2))],data2,'o',[i for i in range(len(data2))],data2,'b',self.buy_x_index,data2_buy_value,'yo',self.sell_x_index,data2_sell_value,'ko')             
