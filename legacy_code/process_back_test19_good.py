...
####
        ...
        #    imf1_or_flag=1
        #elif imf1_min_index[-1]<imf1_max_index[-1] and imf1_max_index[-1]>996:
        #    imf1_or_flag=2
...
###
        ...
        imf2_or_flag=0
        if imf2_min_index[-1]>imf2_max_index[-1] and imf2_min_index[-1]>496 :#and imf2_min_index[-1]-imf2_max_index[-1]>3:
            imf2_or_flag=1
        if imf2_min_index[-1]<imf2_max_index[-1] and imf2_max_index[-1]>=496 :#and imf2_max_index[-1]-imf2_min_index[-1]>3:
            imf2_or_flag=2
...
##########
        ...
        ##if i==1 :#or tmp[-2]<990:
        ##    (imf_process,extenflag1,ex_num,cha) = matchlist1(imf_list[3],5)
        ##elif i==2:
        ##    (imf_process,extenflag1,ex_num,cha) = matchlist2(imf_list[3],5)
        ##else:
        ##    (imf_process,extenflag1,ex_num,cha) = matchlist3(imf_list[3],5)
        ...
        ##imf1_flag=0
        ##if (imf_min_index[-1]>imf_max_index[-1] and imf_min_index[-1]>996 and imf1_or_flag!=2) or (imf1_or_flag==1) :#and ex_flag==1:#and imf_process[-1]<0 :
        ##    imf1_flag=1
        ##if (imf_min_index[-1]<imf_max_index[-1] and imf_max_index[-1]>996 ) or (imf1_or_flag==2):
        ##    imf1_flag=2
...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if imf2_flag==1 or imf2_or_flag==1:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
#       #     if (imf2_flag==1 and self.open_flag==1) or (imf2_flag==1 and imf2_macd_flag==1) or (imf2_macd_flag==1 and self.close_flag==1) or (imf2_macd_flag==1 and imf2_flag==1):#and imf2_macd_flag==1:#and power_de==1:#and imf3_flag==1:
        ...
        #    #if (imf2_flag==2 or imf2_macd_flag==2) and  imf2_or_flag==2 :#and imf2_open_flag!=2:
            if imf2_flag==2 or imf2_or_flag==2:#and imf2_open_flag==2 :
...
#
        ...
        ma_ex_de=0
        if ma_max_index[-1]<ma_min_index[-1] :#and len(ma5)-ma_min_index[-1]<=5 :#and ma_min_index[-1]-ma_max_index[-1]>=2:#and abs(ma_max_index[-1]-last_max)<2:
            ma_ex_de=1
...
#######
        ...
        deltama20_before = self.ma(self.close_price, 20, -1, current_price_index)-self.ma(self.close_price, 20, -2, current_price_index)
...
############
        ...
        print "ma9 %s"%ma9
        print "ma10 %s"%ma10
        print "delta ma10 %s %s"%(deltama10_before,deltama10)
        print "ma20 %s"%ma20
        print "delta ma20 %s %s"%(deltama20_before,deltama20)
...
#######
        ...
        if len(self.buy_x_index)>0 and current_price_index-self.buy_x_index[-1]<10:
...
#######
        ...
        period=5
        mean_p =(self.open_price[current_price_index]+self.high_price[current_price_index]+self.low_price[current_price_index]+self.close_price[current_price_index])/4.0
        mean_p_b =(self.open_price[current_price_index-1]+self.high_price[current_price_index-1]+self.low_price[current_price_index-1]+self.close_price[current_price_index-1])/4.0
        mean_p_b2 =(self.open_price[current_price_index-2]+self.high_price[current_price_index-2]+self.low_price[current_price_index-2]+self.close_price[current_price_index-2])/4.0
        mean_p_b3 =(self.open_price[current_price_index-3]+self.high_price[current_price_index-3]+self.low_price[current_price_index-3]+self.close_price[current_price_index-3])/4.0
        mean_p_b4 =(self.open_price[current_price_index-4]+self.high_price[current_price_index-4]+self.low_price[current_price_index-4]+self.close_price[current_price_index-4])/4.0
        boll_low = (-np.std(self.close_price[current_price_index-period:current_price_index+1]))+np.mean(self.close_price[current_price_index-period:current_price_index+1])
        boll_3low = (-np.std(self.close_price[current_price_index-3:current_price_index+1]))+np.mean(self.close_price[current_price_index-3:current_price_index+1])
        ma5_high=self.ma(self.high_price, 5, 0, current_price_index)
        ma3_high=self.ma(self.high_price, 3, 0, current_price_index)
        ma5_low=self.ma(self.low_price, 5, 0, current_price_index)
        ma3_low=self.ma(self.low_price, 3, 0, current_price_index)
        mean_pp=(mean_p+mean_p_b+mean_p_b2+mean_p_b3+mean_p_b4)/5.0
        mean_pp3=(mean_p+mean_p_b+mean_p_b2)/3.0
        print "meanpp %s"%mean_p
        print "mean3pp %s"%mean_pp3
        print "highpp %s"%ma5_high
        print "3highpp %s"%ma3_high
        print "3lowpp %s"%ma3_low
        print "lowpp %s"%ma5_low
        print (ma5_high+ma5_low)/2.0
        print "ma5 %s"%ma5
        print "bolllow %s"%boll_low
        print "boll3low %s"%boll_3low
        #if self.high_price[current_price_index]<ma5:
        #if mean_pp<ma5:
        if self.close_price[current_price_index]>=self.open_price[current_price_index]:
            upshadow=self.high_price[current_price_index]-self.close_price[current_price_index]
            downshadow=self.open_price[current_price_index]-self.low_price[current_price_index]
        else:
            upshadow=self.high_price[current_price_index]-self.open_price[current_price_index]
            downshadow=self.close_price[current_price_index]-self.low_price[current_price_index]
        print "shadow%s %s"%(upshadow,downshadow)
        #if ma5_low<boll_low :#or upshadow>downshadow:
        #if (deltama10>0 and self.high_price[current_price_index]<ma5 and deltama3>0) or (deltama10>0 and deltama3<0 and (self.high_price[current_price_index]<ma5 or ma3_low<boll_3low)) or (deltama10<0 and (ma3_low<boll_3low or self.high_price[current_price_index]<ma5)):
        if (deltama10>0 and ma3_low<boll_3low) or (deltama10>0 and deltama3<0 and ma3_low<boll_3low):
            self.mean_low.append(1)
        else:
            self.mean_low.append(0)
        print "mean low%s"%self.mean_low[-10:]
...
########
        ...
        if current_price<boll_low1:
            self.below_count.append(1)
        else:
            self.below_count.append(0)


        below_de=0
        if sum(self.below_count[-5:])>=2:
            below_de=1
        else:
            below_de=0

        print "below %s"%self.below_count[-10:]
        boll_de=0
        if current_price>boll_high1 :#and current_price>boll_low2:
            boll_de=2
        #elif (below_de==0 and current_price<boll_low1) or (below_de==1 and self.close_price[current_price_index-1]<boll_low1_b and current_price>boll_low1) :#and :
        elif current_price<boll_low1:
            boll_de=1
...
#################
        ...
        wrong_count=0
        #if (deltama5<0 and deltama3<deltama3_before ):
        #    wrong_count+=1
        #if (deltama3<0 and deltama5<deltama5_before):
        #    wrong_count+=1 
       # if  self.high_price[current_price_index]<=ma5 :
       #     wrong_count+=1
...
#################
        if self.de_flag_day>0 and self.de_flag_day<=1:
            self.de_flag_day+=1
        elif self.de_flag_day>0 and self.de_flag_day>1:
            self.de_flag_day=0
            self.de_flag=0

        dee=0
        if (len(self.below_count)>2 and self.below_count[-3]==1 and self.below_count[-2]==0 and self.below_count[-1]==0) :
            dee=3
        if (len(self.below_count)>0 and self.below_count[-1]==1) :
            dee=1

        if (dee==1 ) and imf_flag==1 :
            self.de_flag=1
            self.de_flag_day=1
...
###############
        ...
        ma_de=1
        if deltama3<deltama3_before and deltama5<deltama5_before:
            ma_de=0
        ...
        if  self.buy_price!=0  and ((self.s_f==2 and current_price<self.close_price[current_price_index-1]) or (deltama3<0 and deltama3_before>=0 )) :#(imf_flag==2) and deltama3<deltama3_before  :# ((current_price-self.buy_price)/self.buy_price)>0.01:#((deltamacd10<0 and deltamacd10_b>0)  ) and ((current_price-self.buy_price)/self.buy_price)>0.01:#(current_price<ma5 )and distance_decision2==1 and ((current_price-self.buy_price)/self.buy_price)>0.01:
            ...
            if 1:#self.fail_flag==0:
                print "sell sell 1"
                self.money = self.sell(self.share,trade_price)+self.money
                self.share = 0
