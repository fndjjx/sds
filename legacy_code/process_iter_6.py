...
class process_util():
    ...
    def __init__(self,open_price,close_price,macd):
...
#            self.last_buy_flag = 1
        ...
        elif std_decision == 1 and macd_decision7 == 1 and macd_decision6 == 1 and macd_decision5 == 1 and ma_decision9 == 1 and (now_position_last_min_delta == 1 or now_position_last_min_delta == 2) and extrem_decision == -1 and ex_decision1 == 1 and self.money>0 and mix_decision1 == 1 : #and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 5, -1, current_price_index)) :#and imf_power_decision == -1 and power_decision == -1 and self.money>0 :
            print "buy 1"
            self.share = self.buy(self.money,trade_price)
            self.money = 0
            self.buy_price = trade_price
            ...
            self.last_buy_flag = 1
            tomorrow_will = -1
        elif  std_decision == 1 and power_decision == -1 and imf_power_decision == -1 and (now_position_last_min_delta == 1 or now_position_last_min_delta == 2) and extrem_decision == -1 and ex_decision1 == 1 and self.money>0 and macd_decision1 == 1 and macd_5 > -0.28: #and (self.ma(self.close_price, 5, 0, current_price_index) > self.ma(self.close_price, 5, -1, current_price_index)) :#and imf_power_decision == -1 and power_decision == -1 and self.money>0 :
            print "buy 2"
            self.share = self.buy(self.money,trade_price)
            self.money = 0
            self.buy_price = trade_price
...
#        macd.append(float(eachline.split("\n")[0]))
    process = process_util(open_price,close_price,macd)
