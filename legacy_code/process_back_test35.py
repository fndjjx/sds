...
###########
        ...
        #    #if ((imf2_flag==1 and imf2_or_flag!=2) or imf2_or_flag==1):# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
            if  imf1_or_flag==1:# and imf2_or_flag!=2 :#and imf2_flag!=2 and imf1_flag!=2:# and imf1_flag!=2:#and imf1_flag==1:# and len(self.imf_flag_list)>1 and self.imf_flag_list[-1]==1:#and  imf2_or_flag!=2:# and imf1_flag==1:#and imf2_flag6==1:#or self.close_flag==1:#and imf2_open_flag!=2:
...
#       #     if (imf2_flag==1 and self.open_flag==1) or (imf2_flag==1 and imf2_macd_flag==1) or (imf2_macd_flag==1 and self.close_flag==1) or (imf2_macd_flag==1 and imf2_flag==1):#and imf2_macd_flag==1:#and power_de==1:#and imf3_flag==1:
        ...
        #    #if (imf2_flag==2 or imf2_macd_flag==2) and  imf2_or_flag==2 :#and imf2_open_flag!=2:
            if imf1_or_flag==2:#and imf2_open_flag==2 :
...
###########################
    ...
    def run_predict(self,imf_list,current_price_index,date,datafile,residual,emd_data,imf_open,imf_macd,emd_std,preflag,emd_data2,prechade,kal_val):
        print "prechade%s"%prechade
        self.pre_cha_de.append(prechade)
        print "current price index%s"%current_price_index
      
        self.kalp.append(kal_val)
...
############
        ...
        if self.buy_price==0:
            ...
            date = date.replace(" ","")
            trade_price=getanytime(self.mindata,date,"13:31:00")
...
####
        pp2=leastsqt_predict(self.close_price[current_price_index-9:current_price_index+1],1)[0]
        zhi5_base=self.close_price[current_price_index-1]+self.close_price[current_price_index-2]+self.close_price[current_price_index-3]+self.close_price[current_price_index]
        zhi5=zhi5_base+pp2
        zhi2=self.close_price[current_price_index]+pp2
        predict=zhi2/2.0-zhi5/5.0

        zhi5=zhi5_base+kal_val
        zhi2=self.close_price[current_price_index]+kal_val
        rz=zhi2/2.0-zhi5/5.0


      #  print "this date%s"%self.date[current_price_index]
        cdate = self.date[current_price_index+1].replace("/","-")
        cdate = cdate.replace(" ","")
      #  print date
      #  nearprice=self.getDatanearClose(self.mindata,date)
        print "rz %s"%rz
        print "chazhi %s"%(predict-rz)
        print "9:45 %s 10:00 %s"%(float(getanytime(self.mindata,cdate,"09:45:00")),float(getanytime(self.mindata,cdate,"10:00:00")))
        print "self pre cha%s"%self.pre_cha[-10:]
        print "kal %s"%self.kalp[-10:]
        print "close %s"%self.close_price[-10:]

###3
        prechabig0=0
        if len(self.pre_cha)>1 and self.pre_cha[-2]>0 and (predict-rz)<0:
            prechabig0=1
        ...
        elif self.buy_price==0 and  imf_flag==1 and distance_decision==1 :#and prechabig0==1:#boll_de==1:#and deltama3>deltama3_before :#and deltama3>deltama3_before:#and pvpower_de==1 :#emd_data[-1]>emd_data[-2] and emd_data[-2]<emd_data[-3]:#self.close_price[current_price_index]>self.close_price[current_price_index-1]:#and (deltama3_before<deltama3 and deltama3_before<0 and deltama3>0) :#and ma_ex_de==1:#and distance_decision==1 and deltama3_before<deltama3 and deltama3_before<deltama3_before2 :#and deltama3_before2<deltama3_before3 :#and ma_ex_de==1:# and distance_decision==1 :# and self.close_price[current_price_index]>self.close_price[current_price_index-1]:#deltama3_before<deltama3:#self.macd[current_price_index-1]<self.macd[current_price_index] :#and (self.macd[current_price_index-2]<0 or self.macd[current_price_index-1]<0 or self.macd[current_price_index]<0):#and deltama5>=0 :#and deltama3_before<deltama3 ) or (deltama3>0 and deltama3_before<0)):# and  mama3>mama3_b and mama3_b<mama3_b2 and distance_decision==1:#and deltama5<0:#and self.close_price[current_price_index]>self.close_price[current_price_index-1] and deltama5_before<deltama5<0: #and deltama3_before<0 and deltama3>0 and deltama5_before<deltama5<0:#and ma_decision==1 :#and distance_decision==1:#deltama3_before<deltama3:# (deltama3_before>deltama3 or deltama3<0):
...
#            self.last_as=self.total_asset
    ...
    def run(self,emd_data, datafile,current_index,date,emd_file,emd_data2,emd_data3,preflag,chazhi,prechade,kal_val):
        self.pre_cha=chazhi
...
#        (imf_macd, residual) = my_emd3.emd(0.03,0.03)
        ...
        print current_index
        self.run_predict(imf,current_index,date,datafile,residual,emd_data,imf_open,imf_macd,std,preflag,emd_data2,prechade,kal_val)
...
###
    ...
    for i in range(begin,len(date)):
        ...
        #p=float(getanytime(mindata,cdate,"09:31:00"))
        #near_list.append(p)

    print halfclose_list
    print kalman_simple(open_price[begin:],halfclose_list) 
    print open_price[begin]
    print halfclose_list[0]
    print kalman_simple(open_price[begin:],halfclose_list)[0]
    k_v = [0]*(begin)
    k_v = k_v+list(kalman_simple(halfclose_list,close_price[begin:]))
    print k_v
    print len(k_v)
...
##
            ...
            halfprice=float(getanytime(mindata,c_date,"11:30:00"))
            print "real half %s"%halfprice
            print "begin open%s"%open_price[i]
            halfprice=k_v[i]
            print "kalman %s"%halfprice
            halfprice=float(getDataHalfClose(mindata,c_date))
...
###
            ...
            nearzhi=zhi2/2.0-zhi5/5.0

            #emd_data.append(nearzhi)
            emd_data.append(k_v[i])
            
            chazhi.append(zhi-nearzhi)

            print "chazhinear%s"%(zhi-nearzhi)
###
            prechade=0
            if len(chazhi)>0 :
                pdata2=3
...
####
        process.run(emd_data,datafile,i-1,date[-1],emd_file,emd_data2,emd_data3,pre_flag,chazhi,prechade,k_v[i])
